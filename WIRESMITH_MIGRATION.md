...
dependency — mimir's own protos no longer use it, but three consumer-side needs
...
- **Any hints (gogo-registry payloads).** The deprecated `hints` fields carry
  ...
  rejected (compiler limitation — `option_customtype.go` "not supported on oneof
...
explicit `(wiresmith.options.jsontag)` matching the exact gogoproto tag** —
...
## Deferred Any-using protos (7m6) — RESOLVED
