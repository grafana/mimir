...
loop:
	...
	infoSeries, ws, err := ev.fetchInfoSeries(ctx, res, ignoreSeries, sel.InfoSelectHints)
	annots.Merge(ws)
	if err != nil {
		return res, annots, err
	}

	series, ws := ev.combineWithInfoSeries(res, infoSeries, ignoreSeries, sel.Offset, sel.InfoSelectHints)
