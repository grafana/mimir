...
func (*evaluator) enrichVectorWithResourceAttrs(base Vector, rq storage.ResourceQuerier, timestamp int64, enh *EvalNodeHelper, dataLabelMatchers map[string][]*labels.Matcher, mappings *attrNameMappings) Vector {
	...
	for _, bs := range base {
		...
		if !found || rv == nil {
			...
			// If there's a filter that doesn't match empty string, skip this series
			if hasNonEmptyMatcher(dataLabelMatchers) {
				continue
			}
			// Otherwise keep the original sample
...
// hasNonEmptyMatcher returns true if any matcher in dataLabelMatchers doesn't match the empty string.
func hasNonEmptyMatcher(dataLabelMatchers map[string][]*labels.Matcher) bool {
	for _, matchers := range dataLabelMatchers {
		for _, m := range matchers {
			if !m.Matches("") {
				return true
			}
