...
func (ev *evaluator) fetchInfoSeries(ctx context.Context, mat Matrix, ignoreSeries map[uint64]struct{}, dataLabelMatchers map[string][]*labels.Matcher, selectHints storage.SelectHints, excludeTargetInfo bool) (Matrix, annotations.Annotations, error) {
	...
	if len(idLblValues) == 0 {
		// Remove __name__ from dataLabelMatchers since it's not a data label
		// selector (it selects which info metrics to consider). Without this,
		// combineWithInfoVector would incorrectly exclude series when only
		// __name__ is specified in the selector.
		delete(dataLabelMatchers, labels.MetricName)
		return nil, nil, nil
	}
	...
	for name, ms := range dataLabelMatchers {
		...
		if len(filtered) > 0 {
			dataLabelMatchers[name] = filtered
		} else {
			delete(dataLabelMatchers, name)
		}
	}
	if len(nameMatchers) > 0 {
		infoLabelMatchers = append(nameMatchers, infoLabelMatchers...)
	} else {
	...
	if excludeTargetInfo {
		...
		infoLabelMatchers = append(infoLabelMatchers, labels.MustNewMatcher(labels.MatchNotEqual, labels.MetricName, targetInfo))
	}
