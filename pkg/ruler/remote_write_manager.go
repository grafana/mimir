...
// EnterpriseTenantManagerFactory creates an ManagerFactory with a
// custom storage.Appendable.
// FYI: this signature can be changed, just try to keep return type the same
// or we will end up forking upstream code
func RWTenantManagerFactory(
	cfg Config,
	p storage.Appendable,
	...
	return func(
		...
		var wrappedQueryFunc promRules.QueryFunc
		...
		return promRules.NewManager(&promRules.ManagerOptions{
			Appendable:                 p,
			...
			NotifyFunc:                 promRules.SendAlerts(notifier, cfg.ExternalURL.URL.String()),
