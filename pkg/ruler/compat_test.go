...
func TestDefaultManagerFactory_CorrectQueryableUsed(t *testing.T) {
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			managerFactory := DefaultTenantManagerFactory(cfg, pusher, federatedQueryable, queryFunc, afero.NewMemMapFs(), &NoopMultiTenantConcurrencyController{}, options.limits, nil)
...
func TestDefaultManagerFactory_ShouldNotWriteRecordingRuleResultsWhenDisabled(t *testing.T) {
	...
	for _, writeEnabled := range []bool{false, true} {
		t.Run(fmt.Sprintf("write enabled: %t", writeEnabled), func(t *testing.T) {
			...
			factory := DefaultTenantManagerFactory(cfg, pusher, queryable, queryFunc, afero.NewMemMapFs(), &NoopMultiTenantConcurrencyController{}, options.limits, nil)
...
func TestDefaultManagerFactory_ShouldInjectReadConsistencyToContextBasedOnRuleDetail(t *testing.T) {
	...
	for testName, testData := range tests {
		t.Run(testName, func(t *testing.T) {
			...
			managerFactory := DefaultTenantManagerFactory(cfg, pusher, queryable, rules.EngineQueryFunc(eng, queryable), afero.NewMemMapFs(), &NoopMultiTenantConcurrencyController{}, options.limits, nil)
			manager := managerFactory(context.Background(), userID, notifierManager, options.logger, nil)
...
func writeRuleGroupToFiles(t *testing.T, path string, logger log.Logger, userID string, ruleGroup rulespb.RuleGroupDesc) []string {
	_, files, err := newMapper(path, afero.NewMemMapFs(), logger).MapRules(userID, map[string][]rulefmt.RuleGroup{
