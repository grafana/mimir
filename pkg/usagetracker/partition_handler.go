...
func (p *partitionHandler) loadAllSnapshotShards(ctx context.Context, files []string) error {
			...
			for j, data := range snapshot.data {
				wg.Go(func() {
					if err := p.store.loadSnapshot(data, time.Now()); err != nil {
						select {
						case errs <- errors.Wrapf(err, "failed to load snapshot data shard %d from file %q", j, snapshot.filename):
						default:
							level.Info(p.logger).Log("msg", "failed to load snapshot data, and error was already sent", "filename", snapshot.filename, "shard", j, "err", err)
						}
						return
					}
				})
	...
	go func() {
		...
		for _, filename := range files {
			...
			level.Info(p.logger).Log("msg", "downloaded snapshot file", "filename", filename, "file_download_time", time.Since(fileT0), "total_elapsed", time.Since(t0), "bytes", n)
			select {
...
func (p *partitionHandler) loadSnapshot(ctx context.Context, r *usagetrackerpb.SnapshotEvent, buf *bytes.Buffer) {
	...
	for i, data := range file.Data {
		// Each one of the datas belongs to a different shard.
		// Most of the write requests touch all shards.
		...
		wg.Go(func() {
			...
			level.Debug(p.logger).Log("msg", "loaded snapshot shard", "filename", r.Filename, "shard_index", i, "bytes", len(data), "elapsed_shard", time.Since(ts), "elapsed_load", time.Since(tl))
		})
	}
	wg.Wait()
