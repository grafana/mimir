...
func splitAndMergeCompactorFactory(ctx context.Context, cfg Config, logger log.Logger, reg prometheus.Registerer) (Compactor, Planner, error) {
	...
	compactor, err := tsdb.NewLeveledCompactor(ctx, reg, logger, cfg.BlockRanges.ToMilliseconds(), downsample.NewPool(), nil, false) // TODO(jesus.vazquez) Review this false enable overlapping compaction. I'm assuming it needs to be false because Mimir only pushes lvl 1 blocks to storage.
	if err != nil {
		return nil, nil, err
	}
