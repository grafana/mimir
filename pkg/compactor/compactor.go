...
func (cfg *Config) RegisterFlags(f *flag.FlagSet, logger log.Logger) {
	...
	f.DurationVar(&cfg.MaxCompactionTime, "compactor.max-compaction-time", 2*time.Hour, "Max time for starting compactions for a single tenant. After this time no new compactions for the tenant are started before next compaction cycle. This can help in multi-tenant environments to avoid single tenant using all compaction time, but also in single-tenant environments to force new discovery of blocks more often. 0 = disabled.")
