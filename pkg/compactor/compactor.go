...
func (cfg *Config) RegisterFlags(f *flag.FlagSet, logger log.Logger) {
	...
	f.DurationVar(&cfg.MaxLookback, "compactor.max-lookback", 0*time.Second, "Blocks uploaded before the lookback will not be considered in compactor cycles. This option should be larger than all values `-blocks-storage.tsdb.block-ranges-period`. The value 0s means all blocks will be considered regardless of their upload time.")
