	...
	for _, c := range []struct {
		...
		if !t.Run(c.name, func(t *testing.T) {
			t.Run("from meta", func(t *testing.T) {
				...
				assert.Equal(t, c.expected, plan)
				assert.Equal(t, c.expectedMarks, promtest.ToFloat64(marked)-lastMarkValue)
			...
			t.Run("from bkt", func(t *testing.T) {
				...
				for _, m := range metasByMinTime {
					require.NoError(t, bkt.Upload(context.Background(), filepath.Join(m.ULID.String(), block.IndexFilename), bytes.NewReader(make([]byte, m.Thanos.Files[0].SizeBytes))))
					m.Thanos = metadata.Thanos{}
				}
