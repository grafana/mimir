...
func TestBucketCompactor_FilterOwnJobs(t *testing.T) {
	...
	for testName, testCase := range tests {
		t.Run(testName, func(t *testing.T) {
			bc, err := NewBucketCompactor(log.NewNopLogger(), nil, nil, nil, "", nil, 2, false, testCase.ownJob, nil, 0, 0, false, 4, 4, m, 32, cfg, 8, 0)
...
func TestBlockMaxTimeDeltas(t *testing.T) {
	...
	bc, err := NewBucketCompactor(log.NewNopLogger(), nil, nil, nil, "", nil, 2, false, nil, nil, 0, 0, false, 4, 4, metrics, 32, cfg, 8, 0)
	require.NoError(t, err)
