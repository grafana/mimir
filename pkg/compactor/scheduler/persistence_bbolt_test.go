...
func TestDeserializeCompactionJob_RecordWrittenBeforePlannerStats(t *testing.T) {
	...
	// Absent stats leave the job spanning nothing, so it lands in the urgent lane.
	require.Zero(t, job.value.Duration())
	require.Equal(t, compactionUrgentLane, newUrgencyLanePolicy(testUrgencyConfig()).LaneForJob(job))
}
