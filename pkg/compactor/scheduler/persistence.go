...
type JobPersistenceManager interface {
	...
	RecoverAll(allowlist *util.AllowList, clk clock.Clock, jobTrackerFactory func(tenant string, persister JobPersister) *JobTracker) (map[string]*JobTracker, error)
	Close() error
}
...
func (m *BboltJobPersistenceManager) RecoverAll(allowedTenants *util.AllowList, clk clock.Clock, compactionTrackerFactory func(tenant string, persister JobPersister) *JobTracker) (jobTrackers map[string]*JobTracker, err error) {
	...
	err = m.db.Update(func(tx *bbolt.Tx) error {
		return tx.ForEach(func(name []byte, b *bbolt.Bucket) error {
			...
			jobs := jobsFromTenantBucket(tenant, b, clk, m.logger)
...
func jobsFromTenantBucket(tenant string, bucket *bbolt.Bucket, clk clock.Clock, logger log.Logger) []TrackedJob {
	jobs := make([]TrackedJob, 0, 10)
	c := bucket.Cursor()
	for k, v := c.First(); k != nil; k, v = c.Next() {
		job, err := deserializeJob(k, v, clk)
		if err != nil {
...
func (n *NopJobPersistenceManager) RecoverAll(allowList *util.AllowList, clk clock.Clock, jobTrackerFactory func(string, JobPersister) *JobTracker) (jobTrackers map[string]*JobTracker, err error) {
