...
func NewTenantDiscoverer(
	...
	s := &TenantDiscoverer{
		...
		repeatedFailureThreshold: cfg.RepeatedFailureThreshold,
...
func (s *TenantDiscoverer) discoverTenants(ctx context.Context) error {
	...
	for _, tenant := range tenants {
		...
		if _, ok := s.knownTenants[tenant]; !ok {
			...
			tracker := NewJobTracker(persister, tenant, s.clock, s.maxLeases, s.repeatedFailureThreshold, s.metrics.newTrackerMetricsForTenant(tenant))
