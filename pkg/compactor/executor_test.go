...
func TestParseLaneRequests(t *testing.T) {
	...
	cases := map[string]struct {
		...
		urgencies [][]compactorschedulerpb.CompactionUrgency
	}{
		"no lanes rejected": {
			input:   flagext.StringSliceCSV{},
			wantErr: true,
		...
		"compaction urgencies split across workers": {
			input: flagext.StringSliceCSV{"compact-urgent+plan", "compact-defer"},
			workers: [][]compactorschedulerpb.JobType{
				{compaction, planning},
				{compaction},
			},
			urgencies: [][]compactorschedulerpb.CompactionUrgency{
				{compactorschedulerpb.COMPACTION_URGENCY_URGENT, compactorschedulerpb.COMPACTION_URGENCY_ANY},
				{compactorschedulerpb.COMPACTION_URGENCY_DEFER},
			},
		},
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			for i, worker := range tc.urgencies {
				urgencies := make([]compactorschedulerpb.CompactionUrgency, len(result[i]))
				for j, r := range result[i] {
					urgencies[j] = r.CompactionUrgency
				}
				require.Equal(t, worker, urgencies)
			}
		})
	}
}
