...
func (cfg *SchedulerClientConfig) RegisterFlags(f *flag.FlagSet) {
	...
	f.Var(&cfg.Lanes, flagPrefix+"lanes", "Lanes to request for each worker goroutine. Each entry is a '+'-separated list of lanes, in the order they should be served. Valid lanes: plan, compact, compact-p1, compact-p2. The compact-p1 and compact-p2 lanes only differ from compact when the scheduler's lane policy separates compaction by urgency.")
...
func parseLaneRequests(configuredLanes flagext.StringSliceCSV) ([][]*compactorschedulerpb.LaneRequest, error) {
	...
	for _, workerLane := range configuredLanes {
		...
		for _, lane := range split {
			...
			case "compact-p1":
				requests = append(requests, &compactorschedulerpb.LaneRequest{JobType: compactorschedulerpb.JOB_TYPE_COMPACTION, CompactionUrgency: compactorschedulerpb.COMPACTION_URGENCY_P1})
			case "compact-p2":
				requests = append(requests, &compactorschedulerpb.LaneRequest{JobType: compactorschedulerpb.JOB_TYPE_COMPACTION, CompactionUrgency: compactorschedulerpb.COMPACTION_URGENCY_P2})
			default:
