...
func (c *Config) RegisterFlags(f *flag.FlagSet) {
	...
	f.StringVar(&c.KafkaPartitions, "aggregator.kafka-partitions", "0", "Kafka partitions to t consume metrics from, separated by \",\". Can be a list of integers or ranges of integers (e.g. 0-3,5,7-10). If not set, computed from -aggregator.kafka-partitions-total and -aggregator.kafka-aggregators-total.")
	...
	f.IntVar(&c.KafkaAggregatorsTotal, "aggregator.kafka-aggregators-total", 0, "Total number of aggregators. Used to compute which partitions this aggregator should be reading.")
...
func (c *Config) Validate() error {
	...
	if len(c.KafkaPartitions) == 0 {
		if c.KafkaPartitionsTotal <= 0 || c.KafkaAggregatorsTotal <= 0 || c.KafkaReaderID == "" {
			return errors.New("either aggregator.kafka-partitions or (aggregator.kafka-partitions-total and aggregator.kafka-aggregators-total) must be set")
		}
		...
		if id < 0 || id >= c.KafkaAggregatorsTotal {
			return fmt.Errorf("invalid aggregator.kafka-reader-id: %s (aggregators: %d)", c.KafkaReaderID, c.KafkaAggregatorsTotal)
		}

		partitionsPerReader := make([][]int, c.KafkaAggregatorsTotal)
		for p := 0; p < c.KafkaPartitionsTotal; p++ {
			r := p % c.KafkaAggregatorsTotal
