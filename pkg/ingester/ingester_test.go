...
func TestIngesterFilesystemOperationsFailedMetrics(t *testing.T) {
	...
	tempDir := t.TempDir()
	tsdbDir := filepath.Join(tempDir, "tsdb")
	// make /data/tsdb a file, shutdown marker check will fail and ingester won't start
	require.NoError(t, os.WriteFile(tsdbDir, []byte("invalid"), 0644))

	limitsCfg := defaultLimitsTestConfig()
	i, _, err := prepareIngesterWithBlocksStorageAndLimits(t, cfg, limitsCfg, nil, tsdbDir, reg)
	require.NoError(t, err)

	err = services.StartAndAwaitRunning(context.Background(), i)
	require.Error(t, err)

	// check `filesystemOpShutdownMarkerExists` incremented
	ct := testutil.ToFloat64(i.metrics.filesystemOperationsFailed.WithLabelValues(filesystemOpShutdownMarkerExists))
	require.Greater(t, ct, 0.0)
}
