...
func TestIngesterActiveSeries(t *testing.T) {
	...
	metricNames := []string{
		"cortex_ingester_active_series",
		"cortex_ingester_active_series_otlp",
		...
		"successful push, should count active series": {
			test: func(t *testing.T, ingester *Ingester, gatherer prometheus.Gatherer) {
				...
				expectedMetrics := `
					...
					# HELP cortex_ingester_active_series_otlp Number of currently active series per user ingested via OTLP.
					# TYPE cortex_ingester_active_series_otlp gauge
					cortex_ingester_active_series_otlp{user="other_test_user"} 4
					cortex_ingester_active_series_otlp{user="test_user"} 4
