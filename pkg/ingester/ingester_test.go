...
func TestIngester_StartPushRequest(t *testing.T) {
	...
	setupIngester := func(tc testCase) *failingIngester {
		...
		if tc.failingIngester {
			...
			failingCause = ingestererr.NewUnavailableError(expectedState)
		} else {
	...
	testCases := map[string]testCase{
		"fail if ingester is not available for push": {
			...
			verifyErr: func(err error) {
				require.ErrorAs(t, err, &ingestererr.UnavailableError{})
			},
		},
		...
		"fail if instance limit is reached": {
			...
			expectedInflightPushRequestsBytes: 0,
			verifyErr: func(err error) {
				require.ErrorAs(t, err, &ingestererr.InstanceLimitReachedError{})
			},
		},
...
func TestIngester_StartReadRequest(t *testing.T) {
	...
	testCases := map[string]testCase{
		...
		"fail if ingester is overloaded, and do not acquire a permit": {
			...
			expectedAcquiredPermitCount: 0,
			verifyErr: func(err error) {
				require.ErrorIs(t, err, ingestererr.ErrTooBusy)
			},
		},
...
func TestIngester_Push(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"should soft fail if histogram has a bucket count vs overall count mismatch": {
			...
			expectedErr: ingestererr.NewErrorWithStatus(ingestererr.WrapOrAnnotateWithUser(ingestererr.NewNativeHistogramValidationError(globalerror.NativeHistogramCountMismatch, fmt.Errorf("21 observations found in buckets, but the Count field is 22: histogram's observation count should equal the number of observations found in the buckets (in absence of NaN)"), model.Time(10), []mimirpb.LabelAdapter{metricLabelAdapters[0]}), userID), codes.FailedPrecondition),
			expectedMetrics: `
				# HELP cortex_ingester_ingested_samples_total The total number of samples ingested per user.
				# TYPE cortex_ingester_ingested_samples_total counter
		...
		"should soft fail if histogram has a bucket count higher than overall count and sum NaN": {
			...
			// Expect the error string instead of constructing the error to catch if Prometheus changes the error message.
			expectedErr: ingestererr.NewErrorWithStatus(ingestererr.WrapOrAnnotateWithUser(ingestererr.NewNativeHistogramValidationError(globalerror.NativeHistogramCountNotBigEnough, fmt.Errorf("21 observations found in buckets, but the Count field is 20: histogram's observation count should be at least the number of observations found in the buckets"), model.Time(10), []mimirpb.LabelAdapter{metricLabelAdapters[0]}), userID), codes.FailedPrecondition),
			expectedMetrics: `
				# HELP cortex_ingester_ingested_samples_total The total number of samples ingested per user.
				# TYPE cortex_ingester_ingested_samples_total counter
				cortex_ingester_ingested_samples_total{user="test"} 0
		...
		"should soft fail if histogram has different number of buckets then encoded in spans": {
			...
			// Expect the error string instead of constructing the error to catch if Prometheus changes the error message.
			expectedErr: ingestererr.NewErrorWithStatus(ingestererr.WrapOrAnnotateWithUser(ingestererr.NewNativeHistogramValidationError(globalerror.NativeHistogramSpansBucketsMismatch, fmt.Errorf("positive side: spans need 5 buckets, have 4 buckets: histogram spans specify different number of buckets than provided"), model.Time(10), []mimirpb.LabelAdapter{metricLabelAdapters[0]}), userID), codes.FailedPrecondition),
			expectedMetrics: `
				# HELP cortex_ingester_ingested_samples_total The total number of samples ingested per user.
				# TYPE cortex_ingester_ingested_samples_total counter
				cortex_ingester_ingested_samples_total{user="test"} 0
		...
		"should soft fail if histogram has a negative bucket count": {
			...
			// Expect the error string instead of constructing the error to catch if Prometheus changes the error message.
			expectedErr: ingestererr.NewErrorWithStatus(ingestererr.WrapOrAnnotateWithUser(ingestererr.NewNativeHistogramValidationError(globalerror.NativeHistogramNegativeBucketCount, fmt.Errorf("negative side: bucket number 2 has observation count of -98: histogram has a bucket whose observation count is negative"), model.Time(10), []mimirpb.LabelAdapter{metricLabelAdapters[0]}), userID), codes.FailedPrecondition),
			expectedMetrics: `
				# HELP cortex_ingester_ingested_samples_total The total number of samples ingested per user.
				# TYPE cortex_ingester_ingested_samples_total counter
				cortex_ingester_ingested_samples_total{user="test"} 0
		...
		"should soft fail on existing histogram series if all exemplars are out of order": {
			...
			expectedErr: ingestererr.NewErrorWithStatus(ingestererr.WrapOrAnnotateWithUser(ingestererr.NewTSDBIngestExemplarErr(storage.ErrOutOfOrderExemplar, model.Time(1500), []mimirpb.LabelAdapter{metricLabelAdapters[0]}, []mimirpb.LabelAdapter{{Name: "traceID", Value: "333"}}), userID), codes.FailedPrecondition),
			expectedIngested: model.Matrix{
				&model.SampleStream{Metric: metricLabelSet, Histograms: []model.SampleHistogramPair{
					{Histogram: mimirpb.FromHistogramToPromHistogram(util_test.GenerateTestHistogram(1)), Timestamp: 9},
					{Histogram: mimirpb.FromHistogramToPromHistogram(util_test.GenerateTestHistogram(2)), Timestamp: 10}},
		...
		"should soft fail on sample out-of-order": {
			...
			expectedErr: ingestererr.NewErrorWithStatus(ingestererr.WrapOrAnnotateWithUser(ingestererr.NewSampleOutOfOrderError(model.Time(9), metricLabelAdapters), userID), codes.FailedPrecondition),
			expectedIngested: model.Matrix{
		...
		"should soft fail on all samples out of bound in a write request": {
			...
			expectedErr: ingestererr.NewErrorWithStatus(ingestererr.WrapOrAnnotateWithUser(ingestererr.NewSampleTimestampTooOldError(model.Time(1575043969-(86400*1000)), metricLabelAdapters), userID), codes.FailedPrecondition),
			expectedIngested: model.Matrix{
		...
		"should soft fail on all samples with histograms out of bound in a write request": {
			...
			expectedErr: ingestererr.NewErrorWithStatus(ingestererr.WrapOrAnnotateWithUser(ingestererr.NewSampleTimestampTooOldError(model.Time(1575043969-(86800*1000)), metricLabelAdapters), userID), codes.FailedPrecondition),
			expectedIngested: model.Matrix{
				&model.SampleStream{Metric: metricLabelSet, Values: []model.SamplePair{{Value: 2, Timestamp: 1575043969}}},
			},
			expectedMetrics: `
		...
		"should soft fail on some samples with timestamp too far in future in a write request": {
			...
			expectedErr: ingestererr.NewErrorWithStatus(ingestererr.WrapOrAnnotateWithUser(ingestererr.NewSampleTimestampTooFarInFutureError(model.Time(now.UnixMilli()+(86400*1000)), metricLabelAdapters), userID), codes.FailedPrecondition),
			expectedIngested: model.Matrix{
		...
		"should soft fail on some exemplars with timestamp too far in future in a write request": {
			...
			expectedErr: ingestererr.NewErrorWithStatus(ingestererr.WrapOrAnnotateWithUser(ingestererr.NewExemplarTimestampTooFarInFutureError(model.Time(now.UnixMilli()+(86400*1000)), metricLabelAdapters, []mimirpb.LabelAdapter{{Name: "traceID", Value: "222"}}), userID), codes.FailedPrecondition),
			expectedIngested: model.Matrix{
				&model.SampleStream{
		...
		"should soft fail on two different sample values at the same timestamp": {
			...
			expectedErr: ingestererr.NewErrorWithStatus(ingestererr.WrapOrAnnotateWithUser(ingestererr.NewSampleDuplicateTimestampError(model.Time(1575043969), metricLabelAdapters), userID), codes.FailedPrecondition),
			expectedIngested: model.Matrix{
				&model.SampleStream{Metric: metricLabelSet, Values: []model.SamplePair{{Value: 2, Timestamp: 1575043969}}},
			},
			expectedMetrics: `
		...
		"should soft fail on exemplar with unknown series": {
			...
			expectedErr:              ingestererr.NewErrorWithStatus(ingestererr.WrapOrAnnotateWithUser(ingestererr.NewExemplarMissingSeriesError(model.Time(1000), metricLabelAdapters, []mimirpb.LabelAdapter{{Name: "traceID", Value: "123"}}), userID), codes.FailedPrecondition),
...
func Test_Ingester_LabelNames(t *testing.T) {
	...
	t.Run("limited due to resource utilization", func(t *testing.T) {
		...
		require.Equal(t, ingestererr.ErrTooBusy.Error(), stat.Message())
...
func TestIngester_Push_ShouldNotCreateTSDBIngesterServiceIsNotInRunningState(t *testing.T) {
	for _, grpcLimitEnabled := range []bool{false, true} {
		t.Run(fmt.Sprintf("gRPC limit enabled: %t", grpcLimitEnabled), func(t *testing.T) {
			...
			assert.EqualError(t, err, ingestererr.NewUnavailableError(services.Stopping).Error())
			assert.Nil(t, res)
	...
	for name, tc := range map[string]struct {
		...
		"should not allow to flush blocks with flush API endpoint if ingester is not in Running state": {
			...
			action: func(t *testing.T, i *Ingester, _ *prometheus.Registry) {
				...
				require.NoError(t, err)
				assert.Equal(t, ingestererr.NewUnavailableError(services.Terminated).Error(), string(body))
			},
		},
	} {
...
func TestIngesterPushErrorDuringForcedCompaction(t *testing.T) {
	...
	ctx := user.InjectOrgID(context.Background(), userID)
	_, err = i.Push(ctx, req)
	expectedErr := ingestererr.NewErrorWithStatus(ingestererr.WrapOrAnnotateWithUser(errTSDBForcedCompaction, userID), codes.Internal)
...
func TestIngester_PushGrpcMethod_Disabled(t *testing.T) {
	...
	_, err = i.Push(ctx, req)
	require.ErrorIs(t, err, ingestererr.ErrPushGrpcDisabled)
}
...
func TestIngester_PushWithSampledErrors(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"should soft fail on sample out-of-order": {
			...
			expectedErrs: []globalerror.ErrorWithStatus{
				ingestererr.NewErrorWithStatus(ingestererr.WrapOrAnnotateWithUser(ingestererr.NewSampleOutOfOrderError(model.Time(9), metricLabelAdapters), users[0]), codes.FailedPrecondition),
				ingestererr.NewErrorWithStatus(ingestererr.WrapOrAnnotateWithUser(ingestererr.NewSampleOutOfOrderError(model.Time(9), metricLabelAdapters), users[1]), codes.FailedPrecondition),
			},
		...
		"should soft fail on all samples out of bound in a write request": {
			...
			expectedErrs: []globalerror.ErrorWithStatus{
				ingestererr.NewErrorWithStatus(ingestererr.WrapOrAnnotateWithUser(ingestererr.NewSampleTimestampTooOldError(model.Time(1575043969-(86400*1000)), metricLabelAdapters), users[0]), codes.FailedPrecondition),
				ingestererr.NewErrorWithStatus(ingestererr.WrapOrAnnotateWithUser(ingestererr.NewSampleTimestampTooOldError(model.Time(1575043969-(86400*1000)), metricLabelAdapters), users[1]), codes.FailedPrecondition),
			},
			expectedSampling: true,
			expectedMetrics: `
				# HELP cortex_discarded_samples_total The total number of samples that were discarded.
		...
		"should soft fail on all samples with histograms out of bound in a write request": {
			...
			expectedErrs: []globalerror.ErrorWithStatus{
				ingestererr.NewErrorWithStatus(ingestererr.WrapOrAnnotateWithUser(ingestererr.NewSampleTimestampTooOldError(model.Time(1575043969-(86800*1000)), metricLabelAdapters), users[0]), codes.FailedPrecondition),
				ingestererr.NewErrorWithStatus(ingestererr.WrapOrAnnotateWithUser(ingestererr.NewSampleTimestampTooOldError(model.Time(1575043969-(86800*1000)), metricLabelAdapters), users[1]), codes.FailedPrecondition),
			},
			expectedSampling: true,
			expectedMetrics: `
				# HELP cortex_discarded_samples_total The total number of samples that were discarded.
		...
		"should soft fail on some samples with timestamp too far in future in a write request": {
			...
			expectedErrs: []globalerror.ErrorWithStatus{
				ingestererr.NewErrorWithStatus(ingestererr.WrapOrAnnotateWithUser(ingestererr.NewSampleTimestampTooFarInFutureError(model.Time(now.UnixMilli()+(86400*1000)), metricLabelAdapters), users[0]), codes.FailedPrecondition),
				ingestererr.NewErrorWithStatus(ingestererr.WrapOrAnnotateWithUser(ingestererr.NewSampleTimestampTooFarInFutureError(model.Time(now.UnixMilli()+(86400*1000)), metricLabelAdapters), users[1]), codes.FailedPrecondition),
			},
			expectedSampling: true,
			expectedMetrics: `
				# HELP cortex_discarded_samples_total The total number of samples that were discarded.
		...
		"should soft fail on some exemplars with timestamp too far in future in a write request": {
			...
			expectedErrs: []globalerror.ErrorWithStatus{
				ingestererr.NewErrorWithStatus(ingestererr.WrapOrAnnotateWithUser(ingestererr.NewExemplarTimestampTooFarInFutureError(model.Time(now.UnixMilli()+(86400*1000)), metricLabelAdapters, []mimirpb.LabelAdapter{{Name: "traceID", Value: "222"}}), users[0]), codes.FailedPrecondition),
				ingestererr.NewErrorWithStatus(ingestererr.WrapOrAnnotateWithUser(ingestererr.NewExemplarTimestampTooFarInFutureError(model.Time(now.UnixMilli()+(86400*1000)), metricLabelAdapters, []mimirpb.LabelAdapter{{Name: "traceID", Value: "222"}}), users[1]), codes.FailedPrecondition),
			},
		...
		"should soft fail on two different sample values at the same timestamp": {
			...
			expectedErrs: []globalerror.ErrorWithStatus{
				ingestererr.NewErrorWithStatus(ingestererr.WrapOrAnnotateWithUser(ingestererr.NewSampleDuplicateTimestampError(model.Time(1575043969), metricLabelAdapters), users[0]), codes.FailedPrecondition),
				ingestererr.NewErrorWithStatus(ingestererr.WrapOrAnnotateWithUser(ingestererr.NewSampleDuplicateTimestampError(model.Time(1575043969), metricLabelAdapters), users[1]), codes.FailedPrecondition),
			},
			expectedSampling: true,
			expectedMetrics: `
				# HELP cortex_discarded_samples_total The total number of samples that were discarded.
		...
		"should soft fail on exemplar with unknown series": {
			...
			expectedErrs: []globalerror.ErrorWithStatus{
				ingestererr.NewErrorWithStatus(ingestererr.WrapOrAnnotateWithUser(ingestererr.NewExemplarMissingSeriesError(model.Time(1000), metricLabelAdapters, []mimirpb.LabelAdapter{{Name: "traceID", Value: "123"}}), users[0]), codes.FailedPrecondition),
				ingestererr.NewErrorWithStatus(ingestererr.WrapOrAnnotateWithUser(ingestererr.NewExemplarMissingSeriesError(model.Time(1000), metricLabelAdapters, []mimirpb.LabelAdapter{{Name: "traceID", Value: "123"}}), users[1]), codes.FailedPrecondition),
			},
			expectedSampling: false,
		},
	}
