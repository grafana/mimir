...
func TestRingConfig_CustomConfigToLifecyclerConfig(t *testing.T) {
	...
	cfg.TokenGenerationCanJoinEnabled = true
	...
	tokenGenerator, err := ring.NewSpreadMinimizingTokenGenerator(expected.ID, expected.Zone, cfg.SpreadMinimizingZones, cfg.TokenGenerationCanJoinEnabled, logger)
...
func TestRingConfig_Validate(t *testing.T) {
	tests := map[string]struct {
		...
		tokenGenerationCanJoinEnabled bool
		...
		"spread-minimizing and token-generation-can-join-check-enabled pass validation": {
			zone:                          instanceZone,
			tokenGenerationStrategy:       spreadMinimizingTokenGeneration,
			spreadMinimizingZones:         spreadMinimizingZones,
			tokenGenerationCanJoinEnabled: true,
		},
		...
		"random and token-generation-can-join-check-enabled don't pass validation": {
			zone:                          instanceZone,
			tokenGenerationStrategy:       randomTokenGeneration,
			tokenGenerationCanJoinEnabled: true,
			expectedError:                 fmt.Errorf("%q must be false in %q token generation strategy", tokenGenerationCanJoinEnabledFlag, randomTokenGeneration),
		},
	...
	for _, testData := range tests {
		...
		cfg.TokenGenerationCanJoinEnabled = testData.tokenGenerationCanJoinEnabled
...
func TestRingConfig_TokenGenerationCanJoinCheckEnabled(t *testing.T) {
	tests := map[string]struct {
		zone                          string
		tokenGenerationStrategy       string
		tokenGenerationCanJoinEnabled bool
		...
		"spread-minimizing and token-generation-can-join-check-enabled allow can join": {
			zone:                          instanceZone,
			tokenGenerationStrategy:       spreadMinimizingTokenGeneration,
			tokenGenerationCanJoinEnabled: true,
			expectedCanJoinEnabled:        true,
		},
		"spread-minimizing without token-generation-can-join-check-enabled doesn't allow can join": {
			zone:                          instanceZone,
			tokenGenerationStrategy:       spreadMinimizingTokenGeneration,
			tokenGenerationCanJoinEnabled: false,
	...
	for _, testData := range tests {
		...
		assert.NotNil(t, tokenGenerator)
		assert.Equal(t, testData.tokenGenerationCanJoinEnabled, tokenGenerator.CanJoinEnabled())
	}
}
