...
func collectAll(t *testing.T, vs *ingesterSearcherValueSet) ([]mimirstorage.SearchResult, annotations.Annotations, error) {
	t.Helper()
	var results []mimirstorage.SearchResult
	for vs.Next() {
...
func producerOf(values ...string) func(ch chan<- mimirstorage.SearchResult) (annotations.Annotations, error) {
	return func(ch chan<- mimirstorage.SearchResult) (annotations.Annotations, error) {
...
func TestIngesterSearcherValueSet_ProducerError(t *testing.T) {
	...
	produce := func(ch chan<- mimirstorage.SearchResult) (annotations.Annotations, error) {
		ch <- mimirstorage.SearchResult{Value: "aaa"}
...
func TestIngesterSearcherValueSet_ProducerWarning(t *testing.T) {
	...
	produce := func(ch chan<- mimirstorage.SearchResult) (annotations.Annotations, error) {
		ch <- mimirstorage.SearchResult{Value: "aaa"}
		ch <- mimirstorage.SearchResult{Value: "bbb"}
		a := annotations.New()
		a.Add(warn)
		return *a, nil
	}
...
func TestIngesterSearcherValueSet_CloseUnblocksProducer(t *testing.T) {
	...
	produce := func(ch chan<- mimirstorage.SearchResult) (annotations.Annotations, error) {
		for i := range mimirstorage.SearchValueSetChannelSize * 3 {
			ch <- mimirstorage.SearchResult{Value: string(rune('a' + i%26))}
		}
		return nil, nil
	}
	vs := newingesterSearcherValueSet(produce, nil, 0, 0, nil)
