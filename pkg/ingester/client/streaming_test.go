...
func TestSeriesChunksStreamReader_HappyPaths(t *testing.T) {
	...
	for name, testCase := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			reader := NewSeriesChunksStreamReader(ctx, mockClient, "ingester", 5, limiter.NewQueryLimiter(0, 0, 0, 0, false, nil), cleanup, log.NewNopLogger())
...
func TestSeriesChunksStreamReader_AbortsWhenParentContextCancelled(t *testing.T) {
	...
	reader := NewSeriesChunksStreamReader(parentCtx, mockClient, "ingester", 3, limiter.NewQueryLimiter(0, 0, 0, 0, false, nil), cleanup, log.NewNopLogger())
	cancel()
	reader.StartBuffering()
...
func TestSeriesChunksStreamReader_ReadingSeriesOutOfOrder(t *testing.T) {
	...
	cleanup := func() {}
	reader := NewSeriesChunksStreamReader(ctx, mockClient, "ingester", 1, limiter.NewQueryLimiter(0, 0, 0, 0, false, nil), cleanup, log.NewNopLogger())
	reader.StartBuffering()
...
func TestSeriesChunksStreamReader_ReceivedFewerSeriesThanExpected(t *testing.T) {
	...
	reader := NewSeriesChunksStreamReader(ctx, mockClient, "ingester", 3, limiter.NewQueryLimiter(0, 0, 0, 0, false, nil), cleanup, log.NewNopLogger())
	reader.StartBuffering()
...
func TestSeriesChunksStreamReader_ChunksLimits(t *testing.T) {
	...
	for name, testCase := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			reader := NewSeriesChunksStreamReader(ctx, mockClient, "ingester", 1, limiter.NewQueryLimiter(0, testCase.maxChunkBytes, testCase.maxChunks, 0, false, queryMetrics), cleanup, log.NewNopLogger())
			reader.StartBuffering()
