...
func FromLabelValuesRequest(req *LabelValuesRequest) (string, int64, int64, []*labels.Matcher, *storage.LabelHints, error) {
	var err error
	var matchers []*labels.Matcher
	var labelHints *storage.LabelHints
	...
	if req.Limit != 0 {
		labelHints = &storage.LabelHints{Limit: int(req.Limit)}
	}

	return req.LabelName, req.StartTimestampMs, req.EndTimestampMs, matchers, labelHints, nil
}

// ToLabelNamesRequest builds a LabelNamesRequest proto
func ToLabelNamesRequest(from, to model.Time, labelHints *storage.LabelHints, matchers []*labels.Matcher) (*LabelNamesRequest, error) {
	ms, err := ToLabelMatchers(matchers)
	if err != nil {
		return nil, err
	}
...
func FromLabelNamesRequest(req *LabelNamesRequest) (int64, int64, []*labels.Matcher, *storage.LabelHints, error) {
	var err error
	var matchers []*labels.Matcher
	var labelHints *storage.LabelHints
	if req.Matchers != nil {
	...
	return req.StartTimestampMs, req.EndTimestampMs, matchers, labelHints, nil
}
