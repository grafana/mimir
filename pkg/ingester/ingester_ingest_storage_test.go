...
func TestIngester_Start(t *testing.T) {
	...
	t.Run("should replay the partition at startup (after a restart) and then join the ingesters and partitions ring", func(t *testing.T) {
		...
		writer := ingest.NewWriter(cfg.IngestStorageConfig.KafkaConfig, ingest.CompartmentsConfig{}, log.NewNopLogger(), nil)
		...
		require.NoError(t, err)
		require.NoError(t, writer.WriteSync(ctx, 0, partitionID, userID, &mimirpb.WriteRequest{Timeseries: []mimirpb.PreallocTimeseries{series1}, Source: mimirpb.API}))
		...
		require.NoError(t, writer.WriteSync(ctx, 0, partitionID, userID, &mimirpb.WriteRequest{Timeseries: []mimirpb.PreallocTimeseries{series2}, Source: mimirpb.API}))
