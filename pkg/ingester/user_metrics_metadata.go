...
func (mm *userMetricsMetadata) add(metric string, metadata *mimirpb.MetricMetadata) error {
	...
	if !ok {
		...
		if !mm.limiter.IsWithinMaxMetricsWithMetadataPerUser(mm.userID, len(mm.metricToMetadata)) {
			...
			return mm.limiter.samplers.WrapMaxMetadataPerUserLimitExceededError(formatMaxMetadataPerUserError(mm.limiter.limits, mm.userID))
		}
	...
	if !mm.limiter.IsWithinMaxMetadataPerMetric(mm.userID, len(set)) {
		...
		return mm.limiter.samplers.WrapMaxMetadataPerMetricLimitExceededError(formatMaxMetadataPerMetricError(mm.limiter.limits, labels.FromStrings(labels.MetricName, metric), mm.userID))
	}
