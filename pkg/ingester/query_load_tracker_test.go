...
func TestQueryLoadTracker_AttributeRoutesByHint(t *testing.T) {
	tr := newQueryLoadTracker()

	// Nil hint goes to unnamed.
	tr.attribute(nil, 100)
	// Named hint with partition 0 should be a real partition (not unnamed).
	tr.attribute(&client.QueryAttributionHint{PartitionId: 0}, 200)
	// Other partition.
	tr.attribute(&client.QueryAttributionHint{PartitionId: 7}, 300)
	// Negative samples are ignored (defensive).
	tr.attribute(&client.QueryAttributionHint{PartitionId: 7}, -10)

	// Drive a tick so EWMA seeds from the just-accumulated counts.
	tr.tick()
...
func TestQueryLoadTracker_PartitionZeroIsNotUnnamed(t *testing.T) {
	tr := newQueryLoadTracker()
...
func TestQueryLoadTracker_EWMASmoothing(t *testing.T) {
	tr := newQueryLoadTracker()

	// First tick: only partition 5 has samples; tick seeds rate.
	...
	// Second tick with no new events should pull rate toward zero
	// by exactly alpha (lastRate += alpha*(0 - lastRate)).
	tr.tick()
	snap2 := tr.Snapshot()
	second := snap2.PerPartition[0].SamplesEWMA
	assert.Less(t, second, first, "EWMA should decay when no new events arrive")

	// Many ticks of no traffic — should approach zero, but slowly.
	for i := 0; i < 20; i++ {
		tr.tick()
	}
	snap3 := tr.Snapshot()
...
func TestQueryLoadTracker_PrometheusCollect(t *testing.T) {
	tr := newQueryLoadTracker()
	...
	for _, fam := range families {
		switch fam.GetName() {
		case "cortex_ingester_query_samples_named_ewma":
			hasNamed = true
		...
		case "cortex_ingester_query_samples_unnamed_ewma":
			hasUnnamed = true
