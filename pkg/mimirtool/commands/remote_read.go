...
func (c *RemoteReadCommand) Register(app *kingpin.Application, envVars EnvVarNames) {
	remoteReadCmd := app.Command("remote-read", "Inspect stored series in Mimir using the remote read API.")
	...
	for _, cmd := range []*kingpin.CmdClause{exportCmd, dumpCmd, statsCmd} {
		cmd.Flag("address", "Address of the mimir cluster, alternatively set "+envVars.Address+".").
		...
		cmd.Flag("id", "Mimir tenant id, alternatively set "+envVars.TenantID+".").
			Envar(envVars.TenantID).
			Default("").
			StringVar(&c.tenantID)
		cmd.Flag("key", "Api key to use when contacting mimir, alternatively set "+envVars.APIKey+".").
