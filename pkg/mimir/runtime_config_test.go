...
`,
			checks: map[string]func(t *testing.T, l *validation.Limits){
				...
				"tenant-a:source=test-run": func(t *testing.T, l *validation.Limits) {
					assert.Equal(t, 1000, l.MaxGlobalSeriesPerUser)
					assert.Equal(t, 50000, l.MaxActiveSeriesPerUser)
					assert.Equal(t, float64(5000), l.IngestionRate)
				},
				"tenant-b:source=test-run": func(t *testing.T, l *validation.Limits) {
					assert.Equal(t, 2000, l.MaxGlobalSeriesPerUser)
					assert.Equal(t, 50000, l.MaxActiveSeriesPerUser)
					assert.Equal(t, float64(5000), l.IngestionRate)
				},
		...
		{
			name: "explicit tenant:key=value inherits from tenant:source and applies its overrides",
			yaml: `
overrides:
  tenant-a:
    max_global_series_per_user: 1000
  ...
  ":test-run":
    max_active_series_per_user: 50000
    ingestion_rate: 5000
  "tenant-a:run-id=specific-id:source=test-run":
...
`,
			checks: map[string]func(t *testing.T, l *validation.Limits){
				...
				"tenant-a:run-id=specific-id:source=test-run": func(t *testing.T, l *validation.Limits) {
					assert.Equal(t, 1000, l.MaxGlobalSeriesPerUser)
		...
		{
			name: "explicit tenant:source override takes precedence over expansion",
			yaml: `
overrides:
  tenant-a:
    max_global_series_per_user: 1000
