...
func (t *Mimir) initQueryFrontendTripperware() (serv services.Service, err error) {
	...
	case querier.PrometheusEngine:
		eng = NewUnlimitedMemoryTrackerPromqlEngine(promql.NewEngine(promOpts))
	case querier.MimirEngine:
		...
		if t.Cfg.Frontend.EnableQueryEngineFallback {
			eng = streamingpromqlcompat.NewEngineWithFallback(t.QueryFrontendStreamingEngine, NewUnlimitedMemoryTrackerPromqlEngine(promql.NewEngine(promOpts)), mqeOpts.CommonOpts.Reg, util_log.Logger)
		} else {
