...
// jobIDForSpec builds a stable ID for a job based on start offsets.
// In compartments mode, the id for each cluster is included in the returned jobID.
func jobIDForSpec(spec *schedulerpb.JobSpec) string {
	if !spec.IsCompartmentEncoding() {
		// Non compartments case (clusterID ID not included).
		return fmt.Sprintf("%s/%d/%d", spec.Topic, spec.Partition, spec.StartOffset)
	}

	// In compartments mode, sort by clusterID id so ID is deterministic.
