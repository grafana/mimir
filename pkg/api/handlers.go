...
func NewQuerierHandler(
	...
	router.Path(path.Join(promPrefix, "/search/metric_names")).Methods("GET", "POST").Handler(searchQueryStats.Wrap(querier.SearchMetricNamesHandler(queryable, limits, logger)))
	router.Path(path.Join(promPrefix, "/search/label_names")).Methods("GET", "POST").Handler(searchQueryStats.Wrap(querier.SearchLabelNamesHandler(queryable, limits, logger)))
	router.Path(path.Join(promPrefix, "/search/label_values")).Methods("GET", "POST").Handler(searchQueryStats.Wrap(querier.SearchLabelValuesHandler(queryable, limits, logger)))
