...
// Provenance-includes-location: https://github.com/prometheus-community/parquet-common/blob/main/convert/convert.go
...
//type tsdb.BlockReader interface {
...
func ConvertTSDBBlock(
	...
	tsdbRowReader, err := NewTSDBRowReader(ctx, mint, maxt, cfg.colDuration.Milliseconds(), blocks, cfg)
	if err != nil {
		return 0, err
	}
	defer func() { _ = tsdbRowReader.Close() }()

	w := NewShardedWrite(tsdbRowReader, tsdbRowReader.Schema(), bkt, &cfg)
	return w.currentShard, errors.Wrap(w.Write(ctx), "error writing block")
}

var _ parquet.RowReader = &BufferedRowReader{}
...
func NewBufferedRowReader(
	ctx context.Context,
	rr parquet.RowReader,
	bufferRows int,
) *BufferedRowReader {
	br := &BufferedRowReader{
		rr:    rr,
		ctx:   ctx,
		c:     make(chan []parquet.Row, bufferRows),
		errCh: make(chan error, 1),
		rowPool: zeropool.New[[]parquet.Row](func() []parquet.Row {
			return make([]parquet.Row, bufferRows)
		}),
	}
...
func (b *BufferedRowReader) ReadRows(rows []parquet.Row) (int, error) {
	if b.current == nil {
		select {
		case next, ok := <-b.c:
			if !ok {
...
func (b *BufferedRowReader) readRows() {
	for {
		select {
		case <-b.ctx.Done():
			b.errCh <- b.ctx.Err()
