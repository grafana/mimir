...
func (c *RemoteIndexCache) StorePostingsOffsetsForMatcher(
	...
	key := postingsOffsetsForMatcherCacheKey(tenantID, blockID.String(), labelName, m, invertMatch)
...
func postingsOffsetsForMatcherCacheKey(tenantID, blockID, labelName string, m *labels.Matcher, invertMatcher bool) string {
	...
	// Preallocate the byte slice used to store the cache key.
	encodedHashLen := base64.RawURLEncoding.EncodedLen(hashLen)
	expectedLen := len(prefix) + len(tenantID) + 1 + ulid.EncodedSize + 1 + len(labelName) + 1 + encodedHashLen
	key := make([]byte, expectedLen)
	offset := 0
	...
	offset += copy(key[offset:], blockID)
	offset += copy(key[offset:], separator)
	offset += copy(key[offset:], labelName)
	offset += copy(key[offset:], separator)
	base64.RawURLEncoding.Encode(key[offset:], lblHash[:hashLen])
	offset += encodedHashLen
...
func matcherStr(m *labels.Matcher) string {
	if m == nil {
		return ""
	}
	return m.String()
...
// matcherStr returns the canonical string form of m, treating a nil matcher
// as a valid sentinel (used by callers to mean "all label values").
