...
func NewTSDBMetrics(r prometheus.Registerer, logger log.Logger) *TSDBMetrics {
	m := &TSDBMetrics{
		...
		tsdbShardedPostingsSubfiltered: prometheus.NewDesc(
			...
			"Total number of TSDB head ShardedPostings calls for a power-of-two shard count larger than the shard bucket count, served by sub-filtering the single candidate bucket.",
			nil, nil),
		tsdbShardedPostingsFallback: prometheus.NewDesc(
			"tsdb_head_sharded_postings_fallback_total",
			"Total number of TSDB head ShardedPostings calls served by per-series filtering because the shard count is not a power of two or the shard bucket index is disabled.",
			nil, nil),
