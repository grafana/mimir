...
// Provenance-includes-location: https://github.com/grafana/mimir/blob/26344c3ec7409713fcf52a9c41cd0dce537b3100/pkg/storage/tsdb/bucketindex/parquet_loader.go
...
func (l *ParquetLoader) GetIndex(ctx context.Context, userID string) (*ParquetIndex, error) {
	l.indexesMx.RLock()
	...
	if err != nil {
		...
		if errors.Is(err, ErrIndexNotFound) {
			...
			// } else if errors.Is(err, bucket.ErrCustomerManagedKeyAccessDenied) { // TODO
...
func (l *ParquetLoader) cacheIndex(userID string, idx *ParquetIndex, err error) {
	...
	l.indexes[userID] = newParquetCachedIndex(idx, err)
}
