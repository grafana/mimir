...
const (
	...
	// the ID of a sharded block generated by the split-and-merge compactor. If a block hasn't this label, it means the
	// block hasn't been split.
...
// IsOutOfOrder returns true if the block was created from out-of-order samples.
func (m Meta) IsOutOfOrder() bool {
	if m.Compaction.FromOutOfOrder() {
		return true
	}
	// Compactor doesn't preserve the "FromOutOfOrder" hints. We check for the external labels,
	// that ingester/block-builder set for a fallback.
