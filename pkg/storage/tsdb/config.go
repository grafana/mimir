...
type BucketStoreConfig struct {
	...
	ChunkRangesPerSeries int `yaml:"chunk_ranges_per_series" category:"advanced"`
}
...
func (cfg *BucketStoreConfig) RegisterFlags(f *flag.FlagSet, logger log.Logger) {
	...
	f.IntVar(&cfg.ChunkRangesPerSeries, "blocks-storage.bucket-store.chunk-ranges-per-series", 1, "This option controls into how many ranges the chunks of each series from each block are split. This value is effectively the number of chunks cache items per series per block.")
}
