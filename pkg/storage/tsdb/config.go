...
func (cfg *TSDBConfig) RegisterFlags(f *flag.FlagSet) {
	...
	f.IntVar(&cfg.HeadPostingsForMatchersCacheMaxItems, "blocks-storage.tsdb.head-postings-for-matchers-cache-size", math.MaxInt, "Maximum number of entries in the cache for postings for matchers in the Head and OOOHead when TTL is greater than 0.")
	...
	f.IntVar(&cfg.BlockPostingsForMatchersCacheMaxItems, "blocks-storage.tsdb.block-postings-for-matchers-cache-size", math.MaxInt, "Maximum number of entries in the cache for postings for matchers in each compacted block when TTL is greater than 0.")
