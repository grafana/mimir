...
type TSDBConfig struct {
	...
	HeadChunksMaxFileSize     int           `yaml:"head_chunks_max_file_size"`
...
func (cfg *TSDBConfig) RegisterFlags(f *flag.FlagSet) {
	...
	f.IntVar(&cfg.HeadChunksWriteBufferSize, "blocks-storage.tsdb.head-chunks-write-buffer-size-bytes", chunks.DefaultWriteBufferSize, "The write buffer size used by the head chunks mapper. Lower values reduce memory utilisation on clusters with a large number of tenants at the cost of increased disk I/O operations. The buffer size can't be larger than the head chunks max file size.")
	f.IntVar(&cfg.HeadChunksMaxFileSize, "blocks-storage.tsdb.head-chunks-max-file-size-bytes", chunks.DefaultMaxHeadChunkFileSize, "The max size of head chunks file on disk. Once this size is reached, a new file is cut.")
...
func (cfg *TSDBConfig) Validate() error {
	...
	if cfg.HeadChunksWriteBufferSize < chunks.MinWriteBufferSize || cfg.HeadChunksWriteBufferSize > cfg.HeadChunksMaxFileSize || cfg.HeadChunksWriteBufferSize%1024 != 0 {
		return errors.Errorf("head chunks write buffer size must be a multiple of 1024 between %d and %d", chunks.MinWriteBufferSize, cfg.HeadChunksMaxFileSize)
	}
