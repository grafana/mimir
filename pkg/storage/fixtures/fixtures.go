...
func AppendTestSeries(seriesCount int) func(testing.TB, func() storage.Appender) {
...
func SetupTestBlock(tb testing.TB, appendFuncs ...AppendFunc) *BucketTestBlock {
	...
	// Dir for writing/compacting the temp block before bucket upload, analogous to ingester or block-builder storage
	...
	// Dir to represent the local store-gateway storage for index-headers
	...
	assert.NoError(tb, err)
	_, err = block.Upload(context.Background(), log.NewNopLogger(), bkt, compactedBlockDir, nil)
	assert.NoError(tb, err)
