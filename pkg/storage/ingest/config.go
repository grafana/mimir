...
var (
	...
	ErrSASLOauthbearerReauthWithoutFile  = fmt.Errorf("the OAuth reauth file path can only be set when the OAuth file path is also configured")
...
type KafkaAuthConfig struct {
	...
	OauthbearerReadTimeout time.Duration `yaml:"sasl_oauthbearer_read_timeout"`

	OauthbearerReauthRequestFilePath     string        `yaml:"sasl_oauthbearer_reauth_request_file_path"`
	OauthbearerReauthRequestWriteTimeout time.Duration `yaml:"sasl_oauthbearer_reauth_request_write_timeout"`
}
...
func (cfg *KafkaAuthConfig) RegisterFlagsWithPrefix(prefix string, f *flag.FlagSet) {
	...
	f.DurationVar(&cfg.OauthbearerReadTimeout, prefix+"oauthbearer-read-timeout", 10*time.Second, "Timeout for reading the token from the oauthbearer file path after writing the reauth signal.")
	f.StringVar(&cfg.OauthbearerReauthRequestFilePath, prefix+"oauthbearer-reauth-request-file-path", "", "Path to a file (typically a named pipe) to signal reauthentication. When set, before reading the token from the file at oauthbearer-file-path, the string \"reauth\\n\" is written to this file. This can be used to trigger an external process to refresh the token.")
	f.DurationVar(&cfg.OauthbearerReauthRequestWriteTimeout, prefix+"oauthbearer-reauth-request-write-timeout", 10*time.Second, "Timeout for writing the reauth signal to the reauth file.")
}
...
func (cfg *KafkaAuthConfig) Validate() error {
	...
	case SASLMechanismOauthbearer:
		...
		if cfg.OauthbearerReauthRequestFilePath != "" && cfg.OauthbearerFilePath == "" {
			return ErrSASLOauthbearerReauthWithoutFile
		}
