...
type KafkaConfig struct {
	...
	// Together with the producer linger, it determines the maximum Produce latency the client
	// can absorb before back-pressuring producers (linger * inflight = absorbable latency).
	// 0 uses the default.
	MaxInflightProduceRequests int `yaml:"max_inflight_produce_requests"`
}
...
func (cfg *KafkaConfig) RegisterFlagsWithPrefix(prefix string, f *flag.FlagSet) {
	...
	f.IntVar(&cfg.MaxInflightProduceRequests, prefix+"max-inflight-produce-requests", 0, "The maximum number of in-flight Produce requests per broker. Together with the producer linger this determines the maximum Produce latency the client can absorb before back-pressuring producers. 0 uses the default.")
