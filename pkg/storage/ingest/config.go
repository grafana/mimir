...
func (cfg *KafkaConfig) RegisterFlagsWithPrefix(prefix string, f *flag.FlagSet) {
	...
	// The rack is also ignored when consuming a partition with concurrent fetching enabled, because the concurrent
	// fetchers build their own Fetch requests and always read from the partition leader. See SingleClusterPartitionReader.
	f.StringVar(&cfg.ClientRack, prefix+"client-rack", "", "The rack identifier for this Kafka client. Corresponds to the Kafka client.rack setting. Only supported when "+prefix+"backend=kafka. It is ignored when consuming a partition with concurrent fetching enabled ("+prefix+"fetch-concurrency-max greater than 0), because concurrent fetching always reads from the partition leader.")
