...
func (cfg *KafkaConfig) RegisterFlagsWithPrefix(prefix string, f *flag.FlagSet) {
	...
	// It also only applies to the read path when concurrent fetching is disabled, because the concurrent fetchers build
	// their own Fetch requests and always read from the partition leader. See NewKafkaReaderClient().
	f.StringVar(&cfg.ClientRack, prefix+"client-rack", "", "The rack identifier for this Kafka client. Corresponds to the Kafka client.rack setting. Only supported when "+prefix+"backend=kafka. On the read path it has no effect unless "+prefix+"fetch-concurrency-max is set to 0, because concurrent fetching always reads from the partition leader.")
