...
func TestIngesterZonalPartition(t *testing.T) {
	t.Run("with zones", func(t *testing.T) {
		actual, err := IngesterZonalPartition("ingester-zone-a-0")
		require.NoError(t, err)
		assert.EqualValues(t, 0, actual)

		actual, err = IngesterZonalPartition("ingester-zone-b-0")
		require.NoError(t, err)
		assert.EqualValues(t, 1, actual)

		actual, err = IngesterZonalPartition("ingester-zone-c-0")
		require.NoError(t, err)
		assert.EqualValues(t, 2, actual)

		actual, err = IngesterZonalPartition("ingester-zone-a-1")
		require.NoError(t, err)
		assert.EqualValues(t, 4, actual)

		actual, err = IngesterZonalPartition("ingester-zone-b-1")
		require.NoError(t, err)
		assert.EqualValues(t, 5, actual)

		actual, err = IngesterZonalPartition("ingester-zone-c-1")
		require.NoError(t, err)
		assert.EqualValues(t, 6, actual)

		actual, err = IngesterZonalPartition("ingester-zone-a-2")
		require.NoError(t, err)
		assert.EqualValues(t, 8, actual)

		actual, err = IngesterZonalPartition("ingester-zone-b-2")
		require.NoError(t, err)
		assert.EqualValues(t, 9, actual)

		actual, err = IngesterZonalPartition("ingester-zone-c-2")
		require.NoError(t, err)
		assert.EqualValues(t, 10, actual)

		actual, err = IngesterZonalPartition("mimir-write-zone-a-1")
		require.NoError(t, err)
		assert.EqualValues(t, 4, actual)

		actual, err = IngesterZonalPartition("mimir-write-zone-b-1")
		require.NoError(t, err)
		assert.EqualValues(t, 5, actual)

		actual, err = IngesterZonalPartition("mimir-write-zone-c-1")
		require.NoError(t, err)
		assert.EqualValues(t, 6, actual)
	})

	t.Run("without zones", func(t *testing.T) {
		actual, err := IngesterZonalPartition("ingester-0")
		require.NoError(t, err)
		assert.EqualValues(t, 0, actual)

		actual, err = IngesterZonalPartition("ingester-1")
		require.NoError(t, err)
		assert.EqualValues(t, 4, actual)

		actual, err = IngesterZonalPartition("mimir-write-0")
		require.NoError(t, err)
		assert.EqualValues(t, 0, actual)

		actual, err = IngesterZonalPartition("mimir-write-1")
		require.NoError(t, err)
		assert.EqualValues(t, 4, actual)
	})

	t.Run("should return error if the ingester ID has a non supported format", func(t *testing.T) {
		_, err := IngesterZonalPartition("unknown")
		require.Error(t, err)

		_, err = IngesterZonalPartition("ingester-zone-X-0")
		require.Error(t, err)

		_, err = IngesterZonalPartition("ingester-zone-a-")
		require.Error(t, err)
