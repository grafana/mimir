...
func TestConcurrentFetchers(t *testing.T) {
	...
	t.Run("starting to run against a broken broker fails creating the fetchers", func(t *testing.T) {
		...
		metrics := NewReaderMetrics(partitionID, reg, noopReaderMetricsSource{}, topicName, nil)
...
func createConcurrentFetchers(ctx context.Context, t *testing.T, client *kgo.Client, topic string, partition int32, startOffset int64, concurrency int, maxInflightBytes int32, start bool) (*ConcurrentFetchers, prometheus.Gatherer) {
	...
	reg := prometheus.NewPedanticRegistry()
	metrics := NewReaderMetrics(partition, reg, noopReaderMetricsSource{}, topic, nil)
