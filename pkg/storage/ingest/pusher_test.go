...
func TestPusherConsumer(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		"unsupported record version": {
			...
			expErr:      "",
			expectedLogLines: []string{
				"level=error msg=\"failed to parse write request; skipping\" err=\"parsing ingest consumer write request: received a record with an unsupported version: 101, max supported version: 3\"",
			},
		},
