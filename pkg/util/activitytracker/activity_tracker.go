...
func NewActivityTracker(cfg Config, reg prometheus.Registerer) (*ActivityTracker, error) {
	...
	tracker := &ActivityTracker{
		...
		prependTimestamp: !cfg.skipTimestamp,
...
//
// Note that timestamp of Insert call is prepended before the tracked activity.
func (t *ActivityTracker) Insert(activityGenerator func() string) (activityIndex int) {
	...
	case i := <-t.freeIndexQueue:
		...
		startTime := ""
		maxSize := entrySize
		if t.prependTimestamp {
			startTime = time.Now().UTC().Format(time.RFC3339Nano)
			maxSize -= len(startTime) + len(timeSeparator)
		}

		entry = trimEntryToSize(entry, maxSize)

		ix := i * entrySize
		if t.prependTimestamp {
			for _, s := range []string{startTime, timeSeparator, entry} {
				ix += copy(t.fileBytes[ix:], s)
			}
		} else {
			copy(t.fileBytes[ix:], entry)
		}
		return i
	default:
		t.failedInserts.Inc()
