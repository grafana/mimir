...
// HeapAlloc is encoded as uint64 value returned by runtime.MemStats.HeapAlloc.
const heapAllocLength = 8
...
func (t *ActivityTracker) Insert(activityGenerator func() string) (activityIndex int) {
	...
	case i := <-t.freeIndexQueue:
		...
		binary.BigEndian.PutUint64(t.fileBytes[ix+timestampLength:], ms.HeapAlloc)

		activity = trimEntryToSize(activity, entrySize-timestampLength-heapAllocLength)

		copy(t.fileBytes[ix+timestampLength+heapAllocLength:], activity)
		return i
	default:
...
func LoadUnfinishedEntries(file string) ([]Entry, error) {
	...
	for n, err = io.ReadFull(fd, buf); err == nil; _, err = io.ReadFull(fd, buf) {
		...
		heapAlloc := binary.BigEndian.Uint64(buf[timestampLength:])

		s = s[timestampLength+heapAllocLength:]
		s = strings.ReplaceAll(s, "\x00", "")

		results = append(results, Entry{
			Timestamp:         ts,
			HeapAllocAtInsert: heapAlloc,
			Activity:          s,
		})
	}
