...
type BlockedQuery struct {
	...
	TimeRangeShorterThan  model.Duration `yaml:"time_range_shorter_than,omitempty" doc:"description=Block queries with time range shorter than this duration. Set to 0 to disable."`
}
...
func (lq *BlockedQueriesConfig) ExampleDoc() (comment string, yaml interface{}) {
	return `The following configuration shows various ways to block queries: by pattern, by time range, or by combining both. ` +
			...
			`Time range filtering blocks queries outside the acceptable duration window.`,
		[]BlockedQuery{
			{
