...
type Limits struct {
	...
	S3SSECEncryptionKeyPath   string `yaml:"s3_sse_c_encryption_key_path" json:"s3_sse_c_encryption_key_path" doc:"nocli|description=Filepath of the S3 server-side encryption customer key. Ignored if the SSE type override is not set." category:"experimental"`
...
// S3SSECEncryptionKeyPath returns the per-tenant file path of the S3 SSE-C encryption key.
func (o *Overrides) S3SSECEncryptionKeyPath(user string) string {
	return o.getOverridesForUser(user).S3SSECEncryptionKeyPath
}
