...
func TestHandler_otlpWriteNoCompression(t *testing.T) {
	...
	resp := httptest.NewRecorder()
	handler := OTLPHandler(100000, nil, &mockSeparateMetricsLabelGetter{separateMetricsLabel: "abc"}, false, nil, verifyWritePushFunc(t, mimirpb.API))
	handler.ServeHTTP(resp, req)
	assert.Equal(t, 200, resp.Code)
}
...
func TestHandler_otlpDroppedMetricsPanic(t *testing.T) {
	...
	resp := httptest.NewRecorder()
	handler := OTLPHandler(100000, nil, &mockSeparateMetricsLabelGetter{separateMetricsLabel: "abc"}, false, nil, func(ctx context.Context, pushReq *Request) (response *mimirpb.WriteResponse, err error) {
...
func TestHandler_otlpWriteRequestTooBigNoCompression(t *testing.T) {
	...
	handler := OTLPHandler(30, nil, &mockSeparateMetricsLabelGetter{separateMetricsLabel: "abc"}, false, nil, readBodyPushFunc(t))
	handler.ServeHTTP(resp, req)
...
func TestHandler_otlpWriteRequestTooBigWithCompression(t *testing.T) {
	...
	handler := OTLPHandler(140, nil, &mockSeparateMetricsLabelGetter{separateMetricsLabel: "abc"}, false, nil, readBodyPushFunc(t))
	handler.ServeHTTP(resp, req)
	assert.Equal(t, http.StatusRequestEntityTooLarge, resp.Code)
...
func TestHandler_otlpWriteRequestWithUnSupportedCompression(t *testing.T) {
	...
	resp := httptest.NewRecorder()
	handler := OTLPHandler(100000, nil, &mockSeparateMetricsLabelGetter{separateMetricsLabel: "abc"}, false, nil, readBodyPushFunc(t))
	handler.ServeHTTP(resp, req)
...
type mockSeparateMetricsLabelGetter struct {
	separateMetricsLabel string
}

func (m *mockSeparateMetricsLabelGetter) SeparateMetricsLabel(userID string) string {
	return m.separateMetricsLabel
}
