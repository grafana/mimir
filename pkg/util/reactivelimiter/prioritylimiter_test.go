...
func TestPriorityLimiter_PriorityRejectionWithIntPriority(t *testing.T) {
	...
	// Medium priority (250) might be allowed or rejected due to random granular priority (200-299 range)
	// We'll test with a higher priority that should definitely be allowed
	canAcquire300 := limiter.CanAcquirePermitWithPriority(300)
	require.True(t, canAcquire300)
