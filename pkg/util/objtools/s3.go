...
func (c *S3ClientConfig) RegisterFlags(prefix string, f *flag.FlagSet) {
	...
	f.BoolVar(&c.DisableMultipart, prefix+"disable-multipart", true, "If false (default), use multipart uploads. If true, disable multipart uploads.")
	f.Uint64Var(&c.PartSize, prefix+"part-size", 0, "if positive, override default multipart upload part size. If zero, use default multipart upload part size (16MiB).")
}
...
func (c *S3ClientConfig) ToBucket() (Bucket, error) {
	...
	return &s3Bucket{
		...
		DisableMultipart: c.DisableMultipart,
		PartSize:         c.PartSize,
	}, nil
}
...
func (bkt *s3Bucket) Upload(ctx context.Context, objectName string, reader io.Reader, contentLength int64) error {
	_, err := bkt.client.PutObject(ctx, bkt.bucketName, objectName, reader, contentLength, minio.PutObjectOptions{
		DisableMultipart: bkt.DisableMultipart,
