...
func ExampleInitLogger() {
	...
	log.InitLogger(&cfg, false, rateLimitedCfg)
...
func ExampleNewDefaultLogger() {
	...
	var lvl dslog.Level
	_ = lvl.Set("info")
	logger := log.NewDefaultLogger(lvl, dslog.LogfmtFormat)

	level.Info(logger).Log("msg", "log.Logger", "test", 1)
	level.Debug(logger).Log("msg", "log.Logger", "test", 2)

	// Output:
	// ts=1970-01-01T00:00:00Z level=info msg=log.Logger test=1
...
func BenchmarkDebugLog(b *testing.B) {
	...
	log.InitLogger(&cfg, false, log.RateLimitedLoggerCfg{})
	b.ResetTimer()
