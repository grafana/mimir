...
// appendSample is a helper to call Append for float samples using the new AppenderV2 interface.
func appendSample(t *testing.T, ca *MimirAppender, ls labels.Labels, metaFamilyName string, meta metadata.Metadata, ct, ts int64, v float64, es []exemplar.Exemplar) {
	_, err := ca.Append(0, ls, ct, ts, v, nil, nil, storage.AppendV2Options{
		MetricFamilyName: metaFamilyName,
		Metadata:         meta,
		Exemplars:        es,
	})
...
// appendHistogram is a helper to call Append for histogram samples using the new AppenderV2 interface.
func appendHistogram(t *testing.T, ca *MimirAppender, ls labels.Labels, metaFamilyName string, meta metadata.Metadata, ct, ts int64, h *histogram.Histogram, es []exemplar.Exemplar) {
