...
func newOTLPParser(
	...
	return func(ctx context.Context, r *http.Request, maxRecvMsgSize int, buffers *util.RequestBuffers, req *mimirpb.PreallocWriteRequest, logger log.Logger) (int, error) {
		...
		if warnings := otlpConverter.TranslationWarnings(); len(warnings) > 0 {
			...
			// and independently of the logging limit, so the discovery signal stays
			// stable however the logging is configured.
...
// unbounded distinct-message cardinality) deliberately stay unlogged, as
// before, until their volume is reviewed.
