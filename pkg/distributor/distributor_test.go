...
func TestDistributor_DropSeriesByLabelValueCombinations(t *testing.T) {
	type testcase struct {
		...
		dropSeries            validation.DropSeries
		expectIngestedMetrics []string
		expectedMetrics       string
	}

	unmarshalDropSeries := func(encoded []byte) (result validation.DropSeries) {
		err := result.UnmarshalJSON(encoded)
		require.NoError(t, err)
		return
	}
	...
	testcases := []testcase{
		{
			name:                  "send two metrics, drop nothing",
			request:               makeWriteRequest(123456789000, 5, 5, false, "metric1", "metric2"),
			dropSeries:            nil,
			expectIngestedMetrics: []string{"metric1", "metric2"},
			expectedMetrics: `
			# HELP cortex_distributor_received_samples_total The total number of received samples, excluding rejected, forwarded and deduped samples.
			# TYPE cortex_distributor_received_samples_total counter
			...
			# HELP cortex_distributor_received_metadata_total The total number of received metadata, excluding rejected.
			# TYPE cortex_distributor_received_metadata_total counter
			cortex_distributor_received_metadata_total{user="user"} 10
			# HELP cortex_distributor_samples_in_total The total number of samples that have come in to the distributor, including rejected, forwarded or deduped samples.
			# TYPE cortex_distributor_samples_in_total counter
			cortex_distributor_samples_in_total{user="user"} 10
			# HELP cortex_distributor_metadata_in_total The total number of metadata the have come in to the distributor, including rejected.
			# TYPE cortex_distributor_metadata_in_total counter
			cortex_distributor_metadata_in_total{user="user"} 10
`,
		}, {
			name:                  "send two metrics, drop one of them based on metric name",
			request:               makeWriteRequest(123456789000, 5, 5, false, "metric1", "metric2"),
			dropSeries:            unmarshalDropSeries([]byte(`[{"name":"__name__", "value":"metric1"}]`)),
			expectIngestedMetrics: []string{"metric2"},
			expectedMetrics: `
			# HELP cortex_distributor_received_samples_total The total number of received samples, excluding rejected, forwarded and deduped samples.
			# TYPE cortex_distributor_received_samples_total counter
...
`,
			...
			name:                  "send 3 metrics, drop all of them based on a label  that isn't the metric name",
			request:               makeWriteRequest(123456789000, 5, 5, false, "metric1", "metric2", "metric3"),
			dropSeries:            unmarshalDropSeries([]byte(`[{"name":"bar", "value":"baz"}]`)), // all samples generated by makeWriteRequest have bar=baz.
			expectIngestedMetrics: []string{},
			expectedMetrics: `
			# HELP cortex_distributor_received_samples_total The total number of received samples, excluding rejected, forwarded and deduped samples.
			# TYPE cortex_distributor_received_samples_total counter
			...
			# HELP cortex_distributor_received_metadata_total The total number of received metadata, excluding rejected.
			# TYPE cortex_distributor_received_metadata_total counter
			cortex_distributor_received_metadata_total{user="user"} 15
			# HELP cortex_distributor_samples_in_total The total number of samples that have come in to the distributor, including rejected, forwarded or deduped samples.
			# TYPE cortex_distributor_samples_in_total counter
			cortex_distributor_samples_in_total{user="user"} 15
			# HELP cortex_distributor_metadata_in_total The total number of metadata the have come in to the distributor, including rejected.
			# TYPE cortex_distributor_metadata_in_total counter
			cortex_distributor_metadata_in_total{user="user"} 15
`,
		}, {
			name:                  "send 1 metric and 5 metadata entries, drop 2 of the metadata entries",
			request:               makeWriteRequest(123456789000, 5, 5, false, "metric1"),
			dropSeries:            unmarshalDropSeries([]byte(`[{"name":"__name__", "value":"metric_0"},{"name":"__name__", "value":"metric_1"}]`)),
			expectIngestedMetrics: []string{"metric1"},
			expectedMetrics: `
			# HELP cortex_distributor_received_samples_total The total number of received samples, excluding rejected, forwarded and deduped samples.
			# TYPE cortex_distributor_received_samples_total counter
			cortex_distributor_received_samples_total{user="user"} 5
			# HELP cortex_distributor_received_metadata_total The total number of received metadata, excluding rejected.
			# TYPE cortex_distributor_received_metadata_total counter
			cortex_distributor_received_metadata_total{user="user"} 3
			# HELP cortex_distributor_samples_in_total The total number of samples that have come in to the distributor, including rejected, forwarded or deduped samples.
			# TYPE cortex_distributor_samples_in_total counter
...
`,
	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			...
			sort.StringSlice(tc.expectIngestedMetrics).Sort()
			sort.StringSlice(ingestedMetrics).Sort()
