...
func TestDistributor_Push(t *testing.T) {
	...
	for name, tc := range map[string]struct {
		...
		"A push exceeding burst size should fail": {
			...
			metadata:       5,
			expectedError:  httpgrpc.Errorf(http.StatusTooManyRequests, distributorerror.NewIngestionRateLimitedError(validation.IngestionRateLimitedMsgFormat, 20, 20).Error()),
			metricNames:    []string{lastSeenTimestamp},
			expectedMetrics: `
				# HELP cortex_distributor_latest_seen_sample_timestamp_seconds Unix timestamp of latest received sample per user.
				# TYPE cortex_distributor_latest_seen_sample_timestamp_seconds gauge
	...
	tests := map[string]struct {
		...
		"request limit should be evenly shared across distributors": {
			...
			pushes: []testPush{
				{expectedError: nil},
				{expectedError: nil},
				{expectedError: httpgrpc.Errorf(http.StatusTooManyRequests, distributorerror.NewRequestRateLimitedError(validation.RequestRateLimitedMsgFormat, 4, 2).Error())},
			},
		},
		...
		"request burst should set to each distributor": {
			...
			pushes: []testPush{
				...
				{expectedError: nil},
				{expectedError: nil},
				{expectedError: httpgrpc.Errorf(http.StatusTooManyRequests, distributorerror.NewRequestRateLimitedError(validation.RequestRateLimitedMsgFormat, 2, 3).Error())},
			},
		},
		"request limit is reached return 529 when enable service overload error set to true": {
			...
			pushes: []testPush{
				{expectedError: nil},
				{expectedError: nil},
				{expectedError: httpgrpc.Errorf(distributorerror.StatusServiceOverloaded, distributorerror.NewRequestRateLimitedError(validation.RequestRateLimitedMsgFormat, 4, 2).Error())},
			},
		},
	}
	...
	tests := map[string]struct {
		...
		"evenly share the ingestion limit across distributors": {
			...
			pushes: []testPush{
				...
				{samples: 2, metadata: 1, expectedError: httpgrpc.Errorf(http.StatusTooManyRequests, distributorerror.NewIngestionRateLimitedError(validation.IngestionRateLimitedMsgFormat, 10, 5).Error())},
				{samples: 2, expectedError: nil},
				{samples: 1, expectedError: httpgrpc.Errorf(http.StatusTooManyRequests, distributorerror.NewIngestionRateLimitedError(validation.IngestionRateLimitedMsgFormat, 10, 5).Error())},
				{metadata: 1, expectedError: httpgrpc.Errorf(http.StatusTooManyRequests, distributorerror.NewIngestionRateLimitedError(validation.IngestionRateLimitedMsgFormat, 10, 5).Error())},
			},
		},
		"for each distributor, set an ingestion burst limit.": {
			...
			pushes: []testPush{
				...
				{samples: 5, metadata: 1, expectedError: httpgrpc.Errorf(http.StatusTooManyRequests, distributorerror.NewIngestionRateLimitedError(validation.IngestionRateLimitedMsgFormat, 10, 20).Error())},
				{samples: 5, expectedError: nil},
				{samples: 1, expectedError: httpgrpc.Errorf(http.StatusTooManyRequests, distributorerror.NewIngestionRateLimitedError(validation.IngestionRateLimitedMsgFormat, 10, 20).Error())},
				{metadata: 1, expectedError: httpgrpc.Errorf(http.StatusTooManyRequests, distributorerror.NewIngestionRateLimitedError(validation.IngestionRateLimitedMsgFormat, 10, 20).Error())},
			},
		},
	}
...
func TestHaDedupeMiddleware(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			next := func(ctx context.Context, pushReq *push.Request) error {
				nextCallCount++
...
func TestInstanceLimitsBeforeHaDedupe(t *testing.T) {
	...
	mockPush := func(ctx context.Context, pushReq *push.Request) error {
	...
	err := wrappedMockPush(ctx, push.NewParsedRequest(writeReqReplica1))
	...
	err = wrappedMockPush(ctx, push.NewParsedRequest(writeReqReplica2))
	require.NoError(t, err)
...
func TestSeriesAreShardedToCorrectIngesters(t *testing.T) {
	...
	distrib := d[0]
	err := distrib.push(ctx, push.NewParsedRequest(req))
	require.NoError(t, err)
