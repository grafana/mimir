		...
		t.Run(testName, func(t *testing.T) {
			...
			for _, ingestStorageEnabled := range []bool{false, true} {
				...
				t.Run(fmt.Sprintf("ingest storage enabled: %t", ingestStorageEnabled), func(t *testing.T) {
					...
					require.NoError(t, err)
					assertMatchingLabelSlices(t, testData.expectedResult, metrics)
		...
		t.Run(testName, func(t *testing.T) {
			...
			for scenarioName, scenarioData := range scenarios {
				...
				t.Run(scenarioName, func(t *testing.T) {
					...
					require.NoError(t, err)
					assertMatchingLabelSlices(t, testData.expectedSeries, series)
...
// Match output of labels.Labels.String().
func labelAdaptersToString(ls []mimirpb.LabelAdapter) string {
	var b bytes.Buffer
...
func assertMatchingLabelSlices(tb testing.TB, a [][]mimirpb.LabelAdapter, b []labels.Labels) {
	tb.Helper()
	mapA := map[string]struct{}{}
	for i := range a {
		mapA[labelAdaptersToString(a[i])] = struct{}{}
	}
	for i := range b {
		s := b[i].String()
