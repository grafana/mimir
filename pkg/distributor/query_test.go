		...
		t.Run(name, func(t *testing.T) {
			for _, minimizeIngesterRequests := range []bool{true, false} {
				t.Run(fmt.Sprintf("request minimization enabled: %v", minimizeIngesterRequests), func(t *testing.T) {
					...
					queryRes, err := ds[0].QueryStream(queryCtx, queryMetrics, math.MinInt32, math.MaxInt32, nil, allSeriesMatchers...)
					require.NoError(t, err)
					...
					_, err = ds[0].QueryStream(queryCtx, queryMetrics, math.MinInt32, math.MaxInt32, nil, allSeriesMatchers...)
					require.Error(t, err)
...
func TestDistributor_QueryStream_ShouldReturnErrorIfMaxChunkBytesPerQueryLimitIsReached(t *testing.T) {
	...
	queryMetrics := stats.NewQueryMetrics(reg[0])
	chunkSizeResponse, err := ds[0].QueryStream(ctx, queryMetrics, math.MinInt32, math.MaxInt32, nil, allSeriesMatchers...)
	require.NoError(t, err)
	...
	// exceed it), we expect a query running on all series to succeed.
	queryRes, err := ds[0].QueryStream(ctx, queryMetrics, math.MinInt32, math.MaxInt32, nil, allSeriesMatchers...)
	require.NoError(t, err)
	...
	finalResp, err := ds[0].QueryStream(ctx, queryMetrics, math.MinInt32, math.MaxInt32, nil, allSeriesMatchers...)
	require.NoError(t, err)
		...
		t.Run(fmt.Sprintf("ingest storage enabled: %t", ingestStorageEnabled), func(t *testing.T) {
			...
			for i := 1; i <= numQueries; i++ {
				t.Run(fmt.Sprintf("Query #%d", i), func(t *testing.T) {
					...
					res, err := distributors[0].QueryStream(ctx, queryMetrics, math.MinInt32, math.MaxInt32, nil, matchers)
					require.NoError(t, err)
