...
type nameValidationEngine struct {
	engine promql.QueryEngine
	limits *validation.Overrides
}

// NameValidationEngine creates a new promql.QueryEngine that wraps engine and overrides query options
// with the name validation scheme from limits.
func NameValidationEngine(engine promql.QueryEngine, limits *validation.Overrides) promql.QueryEngine {
	return &nameValidationEngine{engine: engine, limits: limits}
}
...
func (e nameValidationEngine) NewInstantQuery(ctx context.Context, q storage.Queryable, opts promql.QueryOpts, qs string, ts time.Time) (promql.Query, error) {
...
func (e nameValidationEngine) NewRangeQuery(ctx context.Context, q storage.Queryable, opts promql.QueryOpts, qs string, start, end time.Time, interval time.Duration) (promql.Query, error) {
	validationScheme, err := e.getValidationScheme(ctx)
	if err != nil {
		return nil, err
	}
...
func (e nameValidationEngine) getValidationScheme(ctx context.Context) (model.ValidationScheme, error) {
