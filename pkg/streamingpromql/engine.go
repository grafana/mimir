...
func NewEngineWithCache(opts EngineOpts, metrics *stats.QueryMetrics, planner *QueryPlanner, intermediateCache *cache.CacheFactory) (*Engine, error) {
	...
	nodeMaterializers := map[planning.NodeType]planning.NodeMaterializer{
		...
		planning.NODE_TYPE_TIME_RANGE_SPLIT:                 splitandcache.NewTimeRangeSplitMaterializer(opts.CommonOpts.Reg),
		planning.NODE_TYPE_CACHE:                            splitandcache.NewCacheMaterializer(opts.RangeQuerySplittingAndCaching.CacheClient, opts.CachePrefixGenerator, opts.Limits, opts.CommonOpts.Reg),
	}
