...
func (f *InfoFunction) combineSeriesMetadata(innerMetadata []types.SeriesMetadata, ignoreSeries map[int]struct{}, dataLabelMatchers types.Matchers) ([]types.SeriesMetadata, error) {
	...
	// Use a slice to support multiple matchers for the same label name (intersection semantics).
	...
	for _, labelSets := range labelSetsMap {
		...
		for _, infoLabels := range labelSets {
			...
			infoLabels.Range(func(l labels.Label) {
				...
				// All matchers for the same label must match (intersection semantics).
				if len(dataLabelMatchersMap) > 0 {
					if matchers, ok := dataLabelMatchersMap[l.Name]; !ok || !matchersMatch(matchers, l.Value) {
						return
					}
				}
		...
		for labelName, matchers := range dataLabelMatchersMap {
			if _, saved := savedLabels[labelName]; !saved && !matchersMatch(matchers, "") {
				shouldSkip = true
				break
			}
		}
