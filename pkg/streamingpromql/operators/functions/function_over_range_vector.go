	...
	if m.Func.GenerateFunc != nil {
		if x, ok := m.Inner.(cacheHelper); ok {
			...
			if rng > minDurationToCache {
				...
				m.intermediateResults = fakeCache("user", m.Func.Name, "selector", m.timeRange.StartT, rng)
				if len(m.intermediateResults) == int(rng/intermediateCacheBlockLength) {
