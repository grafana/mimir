...
func TestBucketQuantile_ForcedMonotonicity(t *testing.T) {
	...
	for name, tc := range map[string]struct {
		getInput       func() promql.Buckets // The buckets can be modified in-place so return a new one each time.
		expectedForced bool
		expectedFixed  bool
		expectedValues map[float64]float64
	}{
		"simple - monotonic": {
			getInput: func() promql.Buckets {
		...
		t.Run(name, func(t *testing.T) {
			for q, v := range tc.expectedValues {
				res, forced, fixed := promql.BucketQuantile(q, tc.getInput())
