...
func TestOneToOneVectorVectorBinaryOperation_ClosesInnerOperatorsAsSoonAsPossible(t *testing.T) {
	...
	for name, testCase := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			memoryConsumptionTracker := limiting.NewMemoryConsumptionTracker(0, nil)
			o, err := NewOneToOneVectorVectorBinaryOperation(left, right, vectorMatching, parser.ADD, false, memoryConsumptionTracker, types.NewAnnotations(), posrange.PositionRange{}, timeRange)
			require.NoError(t, err)
			...
			require.Equal(t, uint64(0), memoryConsumptionTracker.CurrentEstimatedMemoryConsumptionBytes.Load())
		})
	}
}
