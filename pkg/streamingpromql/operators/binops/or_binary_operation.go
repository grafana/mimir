...
func (o *OrBinaryOperation) SeriesMetadata(ctx context.Context) ([]types.SeriesMetadata, error) {
	...
	if len(leftMetadata) == 0 && len(rightMetadata) == 0 {
		// Nothing to return.
		leftMetadata = types.SeriesMetadataSlicePool.Put(leftMetadata, o.MemoryConsumptionTracker)
		rightMetadata = types.SeriesMetadataSlicePool.Put(rightMetadata, o.MemoryConsumptionTracker)
...
func (g *orGroup) Close(memoryConsumptionTracker *limiter.MemoryConsumptionTracker) {
	g.leftSamplePresence = types.BoolSlicePool.Put(g.leftSamplePresence, memoryConsumptionTracker)
}
