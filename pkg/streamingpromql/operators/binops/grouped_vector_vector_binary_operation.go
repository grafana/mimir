...
func (s *oneSide) Close(memoryConsumptionTracker *limiter.MemoryConsumptionTracker) {
	...
	if s.matchGroup != nil {
		s.matchGroup.presence = types.IntSlicePool.Put(s.matchGroup.presence, memoryConsumptionTracker)
	}
}
...
func (g *GroupedVectorVectorBinaryOperation) SeriesMetadata(ctx context.Context) ([]types.SeriesMetadata, error) {
	...
	if len(allMetadata) == 0 {
		_ = types.SeriesMetadataSlicePool.Put(allMetadata, g.MemoryConsumptionTracker)
		_ = types.BoolSlicePool.Put(oneSideSeriesUsed, g.MemoryConsumptionTracker)
		_ = types.BoolSlicePool.Put(manySideSeriesUsed, g.MemoryConsumptionTracker)
		g.Close()
		return nil, nil
	}
...
func (g *GroupedVectorVectorBinaryOperation) updateOneSidePresence(side *oneSide, data []types.InstantVectorSeriesData) error {
	...
	if matchGroup.oneSideCount == 0 {
		matchGroup.presence = types.IntSlicePool.Put(matchGroup.presence, g.MemoryConsumptionTracker)
	}
...
func (g *GroupedVectorVectorBinaryOperation) Close() {
	...
	g.oneSideMetadata = types.SeriesMetadataSlicePool.Put(g.oneSideMetadata, g.MemoryConsumptionTracker)
	g.manySideMetadata = types.SeriesMetadataSlicePool.Put(g.manySideMetadata, g.MemoryConsumptionTracker)
