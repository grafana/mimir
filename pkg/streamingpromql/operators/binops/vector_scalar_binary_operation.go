...
func (v *VectorScalarBinaryOperation) PrepareCompleted(ctx context.Context) error {
	if err := v.Scalar.PrepareCompleted(ctx); err != nil {
		return err
	}

	return v.Vector.PrepareCompleted(ctx)
}
