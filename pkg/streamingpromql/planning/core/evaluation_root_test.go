...
// TestEvaluationRootNode_PlanningAndRoundTrip confirms that the __evaluation_root__ marker function is
...
// spin-off tests.
func TestEvaluationRootNode_Planning(t *testing.T) {
	opts := streamingpromql.NewTestEngineOpts()
	planner, err := streamingpromql.NewQueryPlannerWithoutOptimizationPasses(opts, streamingpromql.NewMaximumSupportedVersionQueryPlanVersionProvider())
	require.NoError(t, err)

	ctx := context.Background()
	plan, err := planner.NewQueryPlan(ctx, `__evaluation_root__(sum(foo))`, types.NewInstantQueryTimeRange(time.Now()), streamingpromql.DefaultLookbackDelta, false, streamingpromql.NoopPlanningObserver{})
	require.NoError(t, err)
