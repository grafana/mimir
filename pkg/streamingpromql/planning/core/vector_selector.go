...
func (v *VectorSelector) Describe() string {
	d := describeSelector(v.Matchers, v.Timestamp, v.Offset, nil, v.SkipHistogramBuckets, false, v.Smoothed, v.ProjectionLabels, v.ProjectionInclude)
...
func (v *VectorSelector) EquivalentToIgnoringHintsAndChildren(other planning.Node) bool {
	...
	return ok &&
		...
		v.ProjectionInclude == otherVectorSelector.ProjectionInclude &&
		slices.Equal(v.ProjectionLabels, otherVectorSelector.ProjectionLabels)
}
