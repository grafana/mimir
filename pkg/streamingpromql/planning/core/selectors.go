...
func describeSelector(matchers []*LabelMatcher, ts *time.Time, offset time.Duration, rng *time.Duration, skipHistogramBuckets bool, eagerLoad bool) string {
	...
	if eagerLoad {
		builder.WriteString(", eager load")
	}
