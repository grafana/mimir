...
func TestMemoryConsumptionTracker_Unlimited(t *testing.T) {
	...
	require.Equal(t, uint64(128), tracker.CurrentEstimatedMemoryConsumptionBytes.Load())
	require.Equal(t, uint64(128), tracker.PeakEstimatedMemoryConsumptionBytes.Load())
	...
	require.Equal(t, uint64(130), tracker.CurrentEstimatedMemoryConsumptionBytes.Load())
	require.Equal(t, uint64(130), tracker.PeakEstimatedMemoryConsumptionBytes.Load())
	...
	require.Equal(t, uint64(2), tracker.CurrentEstimatedMemoryConsumptionBytes.Load())
	require.Equal(t, uint64(130), tracker.PeakEstimatedMemoryConsumptionBytes.Load())
	...
	require.Equal(t, uint64(10), tracker.CurrentEstimatedMemoryConsumptionBytes.Load())
	require.Equal(t, uint64(130), tracker.PeakEstimatedMemoryConsumptionBytes.Load())
	...
	require.Equal(t, uint64(131), tracker.CurrentEstimatedMemoryConsumptionBytes.Load())
	require.Equal(t, uint64(131), tracker.PeakEstimatedMemoryConsumptionBytes.Load())
...
func TestMemoryConsumptionTracker_Limited(t *testing.T) {
	...
	require.NoError(t, tracker.IncreaseMemoryConsumption(8))
	require.Equal(t, uint64(8), tracker.CurrentEstimatedMemoryConsumptionBytes.Load())
	require.Equal(t, uint64(8), tracker.PeakEstimatedMemoryConsumptionBytes.Load())
	assertRejectedQueriesCount(t, reg, 0)

	// Add some more memory consumption beneath the limit.
	require.NoError(t, tracker.IncreaseMemoryConsumption(1))
	require.Equal(t, uint64(9), tracker.CurrentEstimatedMemoryConsumptionBytes.Load())
	require.Equal(t, uint64(9), tracker.PeakEstimatedMemoryConsumptionBytes.Load())
	assertRejectedQueriesCount(t, reg, 0)
	...
	for i := 0; i < 3; i++ {
		require.NoError(t, tracker.IncreaseMemoryConsumption(1))
		require.Equal(t, uint64(9+i), tracker.CurrentEstimatedMemoryConsumptionBytes.Load())
		require.Equal(t, uint64(9+i), tracker.PeakEstimatedMemoryConsumptionBytes.Load())
	}
	...
	require.Equal(t, uint64(11), tracker.CurrentEstimatedMemoryConsumptionBytes.Load())
	require.Equal(t, uint64(11), tracker.PeakEstimatedMemoryConsumptionBytes.Load())
	assertRejectedQueriesCount(t, reg, 1)
