...
func (b *InstantVectorDuplicationBuffer) SeriesMetadata(ctx context.Context, selector *types.Selector) ([]types.SeriesMetadata, error) {
	if b.seriesMetadataCount == 0 {
		// Haven't loaded series metadata yet, load it now.
		var err error
		b.seriesMetadata, err = b.Inner.SeriesMetadata(ctx, selector)
		if err != nil {
			return nil, err
		}
	}
...
func (d *InstantVectorDuplicationConsumer) SeriesMetadata(ctx context.Context, selector *types.Selector) ([]types.SeriesMetadata, error) {
	return d.Buffer.SeriesMetadata(ctx, selector)
}
