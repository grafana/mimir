...
var testCasesPropagateMatchersWithoutData = map[string]string{
	...
	`count(not_aggregated_wrapper(kube_pod_container_info{namespace="ns",pod=~"a|b|c"}) and on (cluster, namespace, pod, container) not_aggregated_wrapper(kube_pod_container_status_ready) == 1)`: `count(not_aggregated_wrapper(kube_pod_container_info{namespace="ns",pod=~"a|b|c"}) and on (cluster, namespace, pod, container) not_aggregated_wrapper(kube_pod_container_status_ready{pod=~"a|b|c", namespace="ns"}) == 1)`,
}
