...
func TestOperatorEvaluationStats_TrackSamplesForRangeVectorSelector_InstantQuery(t *testing.T) {
	...
	samplesReadIfFirstStep.requireChange(t, stats.allSeries.samplesReadIfFirstStep, 2+EquivalentFloatSampleCount(h))
...
func TestOperatorEvaluationStats_Add_WithSubsets(t *testing.T) {
	...
	t.Run("receiver has subsets, other does not", func(t *testing.T) {
		...
		require.Equal(t, []int64{4, 5, 0}, withoutSubsets.allSeries.newSamplesReadPerStep)
...
func TestOperatorEvaluationStats_ComputeForSubquery(t *testing.T) {
	...
	t.Run("range query, outer ranges overlap", func(t *testing.T) {
		...
		require.Equal(t, []int64{samplesReadIfFirstStepAt(1) + newSamplesReadAt(2) + newSamplesReadAt(3), samplesReadIfFirstStepAt(3) + newSamplesReadAt(4) + newSamplesReadAt(5)}, result.allSeries.samplesReadIfFirstStep)
	...
	t.Run("range query, outer ranges don't use all inner steps", func(t *testing.T) {
		...
		require.Equal(t, []int64{samplesReadIfFirstStepAt(1) + newSamplesReadAt(2), samplesReadIfFirstStepAt(4) + newSamplesReadAt(5)}, result.allSeries.samplesReadIfFirstStep)
	...
	t.Run("range query with @", func(t *testing.T) {
		...
		samplesReadIfFirstStep := samplesReadIfFirstStepAt(2) + newSamplesReadAt(3) + newSamplesReadAt(4)
	...
	t.Run("range query with @ and offset", func(t *testing.T) {
		...
		samplesReadIfFirstStep := samplesReadIfFirstStepAt(1) + newSamplesReadAt(2) + newSamplesReadAt(3)
		require.Equal(t, []int64{samplesReadIfFirstStep, samplesReadIfFirstStep}, result.allSeries.samplesReadIfFirstStep)
	...
	t.Run("instant query", func(t *testing.T) {
		...
		require.Equal(t, []int64{samplesReadIfFirstStepAt(3) + newSamplesReadAt(4) + newSamplesReadAt(5)}, result.allSeries.samplesReadIfFirstStep)
	...
	t.Run("instant query with @", func(t *testing.T) {
		...
		require.Equal(t, []int64{samplesReadIfFirstStepAt(2) + newSamplesReadAt(3) + newSamplesReadAt(4)}, result.allSeries.samplesReadIfFirstStep)
...
func TestOperatorEvaluationStats_ComputeForSubquery_WithSubsets(t *testing.T) {
	...
	subsetNewSamplesReadAt := func(t int) int64 { return int64(1 + t*100) }
	...
	t.Run("range query, outer ranges overlap", func(t *testing.T) {
		...
		require.Equal(t, []int64{subsetSamplesReadIfFirstStepAt(1) + subsetNewSamplesReadAt(2) + subsetNewSamplesReadAt(3), subsetSamplesReadIfFirstStepAt(3) + subsetNewSamplesReadAt(4) + subsetNewSamplesReadAt(5)}, result.subsets[0].samplesReadIfFirstStep)
	...
	t.Run("instant query", func(t *testing.T) {
		...
		require.Equal(t, []int64{subsetSamplesReadIfFirstStepAt(3) + subsetNewSamplesReadAt(4) + subsetNewSamplesReadAt(5)}, result.subsets[0].samplesReadIfFirstStep)
...
func TestOperatorEvaluationStats_EncodingAndDecoding(t *testing.T) {
	testCases := map[string]func(t *testing.T, memoryConsumptionTracker *limiter.MemoryConsumptionTracker) *OperatorEvaluationStats{
		...
		"range query, with subsets": func(t *testing.T, memoryConsumptionTracker *limiter.MemoryConsumptionTracker) *OperatorEvaluationStats {
			...
			stats.subsets[0].newSamplesReadPerStep[0] = 500
			stats.subsets[0].newSamplesReadPerStep[1] = 501
			stats.subsets[0].newSamplesReadPerStep[2] = 502
			stats.subsets[0].samplesReadIfFirstStep[0] = 600
			...
			stats.subsets[1].newSamplesReadPerStep[0] = 800
			stats.subsets[1].newSamplesReadPerStep[1] = 801
			stats.subsets[1].newSamplesReadPerStep[2] = 802
			stats.subsets[1].samplesReadIfFirstStep[0] = 900
