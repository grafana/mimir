...
func TestLimitingBucketedPool_Unlimited(t *testing.T) {
	...
	require.Equal(t, 128*FPointSize, tracker.CurrentEstimatedMemoryConsumptionBytes.Load())
	require.Equal(t, 128*FPointSize, tracker.PeakEstimatedMemoryConsumptionBytes.Load())
	...
	require.Equal(t, 130*FPointSize, tracker.CurrentEstimatedMemoryConsumptionBytes.Load())
	require.Equal(t, 130*FPointSize, tracker.PeakEstimatedMemoryConsumptionBytes.Load())

	// Put a slice back into the pool, the current stat should be updated but peak should be unchanged.
	p.Put(s100, tracker)
	require.Equal(t, 2*FPointSize, tracker.CurrentEstimatedMemoryConsumptionBytes.Load())
	require.Equal(t, 130*FPointSize, tracker.PeakEstimatedMemoryConsumptionBytes.Load())
	...
	require.Equal(t, 10*FPointSize, tracker.CurrentEstimatedMemoryConsumptionBytes.Load())
	require.Equal(t, 130*FPointSize, tracker.PeakEstimatedMemoryConsumptionBytes.Load())
	...
	require.Equal(t, 266*FPointSize, tracker.CurrentEstimatedMemoryConsumptionBytes.Load())
	require.Equal(t, 266*FPointSize, tracker.PeakEstimatedMemoryConsumptionBytes.Load())
...
func TestLimitingPool_Limited(t *testing.T) {
	...
	require.Equal(t, 8*FPointSize, tracker.CurrentEstimatedMemoryConsumptionBytes.Load())
	require.Equal(t, 8*FPointSize, tracker.PeakEstimatedMemoryConsumptionBytes.Load())
	assertRejectedQueryCount(t, reg, 0)
	...
	require.Equal(t, 9*FPointSize, tracker.CurrentEstimatedMemoryConsumptionBytes.Load())
	require.Equal(t, 9*FPointSize, tracker.PeakEstimatedMemoryConsumptionBytes.Load())
	assertRejectedQueryCount(t, reg, 0)
	...
	for i := 0; i < 3; i++ {
		...
		require.NoError(t, err)
		require.Equal(t, 1, cap(s1))
		require.Equal(t, uint64(9+i)*FPointSize, tracker.CurrentEstimatedMemoryConsumptionBytes.Load())
		require.Equal(t, uint64(9+i)*FPointSize, tracker.PeakEstimatedMemoryConsumptionBytes.Load())
	}

	// Try to get another slice while we're already at the limit.
	_, err = p.Get(1, tracker)
	require.ErrorContains(t, err, expectedError)
	require.Equal(t, 11*FPointSize, tracker.CurrentEstimatedMemoryConsumptionBytes.Load())
	require.Equal(t, 11*FPointSize, tracker.PeakEstimatedMemoryConsumptionBytes.Load())
	assertRejectedQueryCount(t, reg, 1)
}
