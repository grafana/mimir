...
func expHist(buckets []float64, count, sum float64, hint histogram.CounterResetHint) *histogram.FloatHistogram {
...
func TestInterpolateHistograms(t *testing.T) {
	testCases := map[string]struct {
		...
		"returns a copy of the left histogram when t equals t1": {
			h1: expHist([]float64{4, 6}, 10, 100, histogram.UnknownCounterReset),
			h2: expHist([]float64{8, 12}, 20, 200, histogram.UnknownCounterReset),
			t1: 0, t2: 10, t: 0,
			expectedCount:   10,
			expectedSum:     100,
			expectedBuckets: []float64{4, 6},
		},
		"returns a copy of the right histogram when t equals t2": {
			...
			expectedCount:   20,
			expectedSum:     200,
			expectedBuckets: []float64{8, 12},
		},
		"linearly interpolates at the midpoint": {
			// fraction = (5-0)/(10-0) = 0.5, so each value is h1 + (h2-h1)*0.5:
			// count 10 + (20-10)*0.5 = 15, sum 100 + (200-100)*0.5 = 150,
			// buckets [4,6] + ([8,12]-[4,6])*0.5 = [6,9].
			h1: expHist([]float64{4, 6}, 10, 100, histogram.UnknownCounterReset),
			h2: expHist([]float64{8, 12}, 20, 200, histogram.UnknownCounterReset),
			t1: 0, t2: 10, t: 5,
			expectedCount:   15,
			expectedSum:     150,
			expectedBuckets: []float64{6, 9},
		},
		"interpolates gauge data without modelling a reset": {
			// isCounter false, so the reset short-circuit is skipped even though h2 < h1.
			// buckets [12,8] + ([6,4]-[12,8])*0.5 = [9,6], count 15, sum 150.
			h1: expHist([]float64{12, 8}, 20, 200, histogram.GaugeType),
			h2: expHist([]float64{6, 4}, 10, 100, histogram.GaugeType),
			t1: 0, t2: 10, t: 5,
			expectedCount:   15,
			expectedSum:     150,
			expectedBuckets: []float64{9, 6},
		},
		"models a detected counter reset as starting from zero": {
			...
			// h2 * fraction (not interpolated against h1): buckets [8,12]*0.5 = [4,6],
			// count 20*0.5 = 10, sum 200*0.5 = 100.
			h1: expHist([]float64{4, 6}, 10, 100, histogram.UnknownCounterReset),
			h2: expHist([]float64{8, 12}, 20, 200, histogram.CounterReset),
			t1: 0, t2: 10, t: 5,
			isCounter:       true,
			expectedCount:   10,
			expectedSum:     100,
