...
func TestDecodeNodesValidatesFill(t *testing.T) {
	testCases := map[string]struct {
		...
		"group_right fill at V22": {
			version: planning.QueryPlanV22,
			op:      core.BINARY_ADD,
			card:    parser.CardOneToMany,
			lhsType: parser.ValueTypeVector,
			rhsType: parser.ValueTypeVector,
		},
		"group_left fill at V22": {
			version: planning.QueryPlanV22,
			op:      core.BINARY_ADD,
			card:    parser.CardManyToOne,
			lhsType: parser.ValueTypeVector,
		...
		"group_right fill below V22": {
			version:       planning.QueryPlanV20,
			op:            core.BINARY_ADD,
			card:          parser.CardOneToMany,
			lhsType:       parser.ValueTypeVector,
			rhsType:       parser.ValueTypeVector,
			expectedError: "query plan has version 20, but decoded node 2 requires version 22",
		},
