...
func NewQueryPlannerWithTime(opts EngineOpts, versionProvider QueryPlanVersionProvider, timeNow func() time.Time) (*QueryPlanner, error) {
	...
	if opts.EnableRangeVectorCommonSubexpressionElimination && !opts.EnableCommonSubexpressionElimination {
		return nil, errors.New("cannot enable range vector common subexpression elimination without common subexpression elimination")
	}

	if opts.EnableCommonSubexpressionElimination {
		planner.RegisterQueryPlanOptimizationPass(commonsubexpressionelimination.NewOptimizationPass(opts.EnableSubsetSelectorElimination, opts.EnableRangeVectorCommonSubexpressionElimination, opts.CommonOpts.Reg, opts.Logger))
	}
