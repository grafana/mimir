...
func NewQueryPlanner(opts EngineOpts) *QueryPlanner {
	...
	// The following is applied in query pruning middleware for now as we need to run them before query sharding, but we leave most of the code here as we plan to move them here later.
	// planner.RegisterASTOptimizationPass(&ast.ReorderHistogramAggregation{})
