...
// Provenance-includes-location: https://github.com/cortexproject/cortex/blob/master/pkg/ring/lifecycler.go
...
func (cfg *LifecyclerConfig) RegisterFlagsWithPrefix(prefix string, f *flag.FlagSet) {
	...
	f.DurationVar(&cfg.MinReadyDuration, prefix+"min-ready-duration", 1*time.Minute, "Minimum duration to wait after the internal readiness checks have passed but before succeeding the readiness endpoint. This is used to slowdown deployment controllers (eg. Kubernetes) after an instance is ready and before they proceed with a rolling update, to give the rest of the cluster instances enough time to receive ring updates.")
...
func (i *Lifecycler) ClaimTokensFor(ctx context.Context, ingesterID string) error {
	...
	fn := func() {
		...
		claimTokens := func(in interface{}) (out interface{}, retry bool, err error) {
			...
			// Tokens of the leaving ingester may have been generated by an older version of Mimir which
