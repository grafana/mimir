...
type Config struct {
	...
	PrioritizeComponentQueues             bool          `yaml:"prioritize_component_queues" category:"experimental"`
...
func (cfg *Config) RegisterFlags(f *flag.FlagSet, logger log.Logger) {
	...
	f.BoolVar(&cfg.PrioritizeComponentQueues, "query-scheduler.prioritize-component-queues", false, "TODO")
...
func (s *Scheduler) QuerierLoop(querier schedulerpb.SchedulerForQuerier_QuerierLoopServer) error {
	...
	for s.isRunningOrStopping() {
		queueReq, idx, err := s.requestQueue.WaitForRequestForQuerier(querier.Context(), lastUserIndex, querierID, workerID)
		if err != nil {
