...
type Config struct {
	...
	UseIntegratedQueryQueue               bool          `yaml:"use_integrated_query_queue" category:"experimental"`
...
func (cfg *Config) RegisterFlags(f *flag.FlagSet, logger log.Logger) {
	...
	f.BoolVar(&cfg.UseIntegratedQueryQueue, "query-scheduler.use-integrated-query-queue", false, "Use an experimental version of the query queue which has the same behavior as the existing queue, but integrates tenant selection into the tree model. (default false)")
