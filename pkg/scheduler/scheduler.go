...
type Config struct {
	...
	QueueAlgorithm                        string        `yaml:"queue_algorithm" category:"experimental"`
...
func (cfg *Config) RegisterFlags(f *flag.FlagSet, logger log.Logger) {
	...
	f.StringVar(&cfg.QueueAlgorithm, "query-scheduler.queue-algorithm", string(queue.Baseline), "TODO")
...
func NewScheduler(cfg Config, limits Limits, log log.Logger, registerer prometheus.Registerer) (*Scheduler, error) {
	...
	s.requestQueue, err = queue.NewRequestQueue(
		...
		queue.QueueAlgo(cfg.QueueAlgorithm),
...
func (s *Scheduler) QuerierLoop(querier schedulerpb.SchedulerForQuerier_QuerierLoopServer) error {
	...
	for s.isRunningOrStopping() {
		queueReq, idx, err := s.requestQueue.WaitForRequestForQuerier(querier.Context(), lastUserIndex, querierID, resp.GetWorkerID())
		if err != nil {
