...
// InitFrontend initializes frontend V2 (that uses the query scheduler) or no frontend at
	...
	*/
	...
	case cfg.FrontendV2.SchedulerAddress != "" || cfg.FrontendV2.QuerySchedulerDiscovery.Mode == schedulerdiscovery.ModeRing:
		...
		if cfg.FrontendV2.Addr == "" {
			...
			if err != nil {
				return nil, nil, fmt.Errorf("failed to get frontend address: %w", err)
			}
	...
	default:
		return nil, nil, errors.New("failed to configure query-frontend: scheduler address or downstream URL must be set")
	}
}
