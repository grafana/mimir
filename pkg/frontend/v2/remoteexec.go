...
func (g *RemoteExecutionGroupEvaluator) finalizeStream(ctx context.Context, nodeStreamIndex remoteExecutionNodeStreamIndex) (stats.Stats, error) {
	nodeState := g.nodeStreams.streams[nodeStreamIndex]
	if nodeState.finished {
		return stats.Stats{}, fmt.Errorf("can't finalize node stream index %v, as it is already finished", nodeState.nodeIndex)
	}
...
func (r *scalarExecutionResponse) Finalize(ctx context.Context) (stats.Stats, error) {
...
func (r *instantVectorExecutionResponse) Finalize(ctx context.Context) (stats.Stats, error) {
	return r.group.finalizeStream(ctx, r.nodeStreamIndex)
}
...
func (r *rangeVectorExecutionResponse) Finalize(ctx context.Context) (stats.Stats, error) {
	return r.group.finalizeStream(ctx, r.nodeStreamIndex)
}
