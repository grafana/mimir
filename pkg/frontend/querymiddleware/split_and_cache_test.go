...
func TestSplitRequests_storeDownstreamResponses(t *testing.T) {
	tests := map[string]struct {
		...
		"should do nothing on no downstream requests": {
			requests: splitRequests{
				{downstreamRequests: []MetricsQueryRequest{}, downstreamResponses: []Response{}},
				{downstreamRequests: []MetricsQueryRequest{}, downstreamResponses: []Response{}},
			},
			responses: nil,
			expected: splitRequests{
...
func TestSplitAndCacheMiddleware_MemoryConsumptionTrackerFactory_CachedResponsesTracked(t *testing.T) {
	// Verifies that cached responses count against the memory tracker during the Do() call,
	// and that this memory is released when Do() returns.

	cacheBackend := cache.NewInstrumentedMockCache()

	const memoryLimit uint64 = 1024 * 1024
	reg := prometheus.NewPedanticRegistry()
	inflightTracker := limiter.NewInflightMemoryConsumptionTracker(reg, nil)
			...
			Result: []SampleStream{
				{
					...
					Samples: []mimirpb.Sample{{Value: 137, TimestampMs: 1634292000000}},
				},
			},
		},
	}

	var peakMemoryDuringDownstream uint64
	...
	_, err := rc.Do(ctx, req)
	require.NoError(t, err)
	require.Equal(t, uint64(0), peakMemoryDuringDownstream) // No cached data during first call.

	// Second request — extends the time range so we get a cache hit AND a downstream call.
	extendedReq, err := req.WithStartEnd(req.GetStart(), req.GetEnd()+step)
	require.NoError(t, err)

	peakMemoryDuringDownstream = 0
	_, err = rc.Do(ctx, extendedReq)
	require.NoError(t, err)

	// During the downstream call, the cached portion should have been tracked.
	// The exact bytes depend on protobuf encoding size, but it should be non-zero.
	require.Greater(t, peakMemoryDuringDownstream, uint64(0), "cached response bytes should be tracked in the memory consumption tracker during downstream execution")
}
