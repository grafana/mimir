...
func TestSplitAndCacheMiddleware_ResultsCache(t *testing.T) {
	...
	downstreamReqs := 0
	rc := mw.Wrap(MetricsQueryHandlerFunc(func(_ context.Context, req MetricsQueryRequest) (Response, error) {
		downstreamReqs++
...
func TestSplitAndCacheMiddleware_ResultsCache_ShouldNotCacheRequestEarlierThanMaxCacheFreshness(t *testing.T) {
	...
	for testName, testData := range tests {
		t.Run(testName, func(t *testing.T) {
			...
			calls := 0
			rc := mw.Wrap(MetricsQueryHandlerFunc(func(_ context.Context, r MetricsQueryRequest) (Response, error) {
				calls++
	...
	for testName, testData := range tests {
		t.Run(testName, func(t *testing.T) {
			...
			mw := newSplitAndCacheMiddleware(
				...
				log.NewNopLogger(),
				prometheus.NewPedanticRegistry(),
			).Wrap(MetricsQueryHandlerFunc(func(_ context.Context, req MetricsQueryRequest) (Response, error) {
