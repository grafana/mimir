...
func TestSubquerySpinOff_ShouldReturnErrorOnDownstreamHandlerFailure(t *testing.T) {
	...
	spinoffMiddleware := newSpinOffSubqueriesMiddleware(mockLimits{instantQueriesWithSubquerySpinOff: []string{".*"}}, log.NewNopLogger(), newEngine(), nil, []MetricsQueryMiddleware{}, defaultStepFunc)
	...
	for testName, testData := range tests {
		t.Run(testName, func(t *testing.T) {
			...
			spinoffMiddleware := newSpinOffSubqueriesMiddleware(
				...
				engine,
				reg,
				[]MetricsQueryMiddleware{fakeMiddleware},
				defaultStepFunc,
			)
