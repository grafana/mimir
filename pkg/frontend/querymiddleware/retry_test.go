...
func TestRetry(t *testing.T) {
	...
	errBadRequest := httpgrpc.ErrorFromHTTPResponse(&httpgrpc.HTTPResponse{
		...
		Body: []byte("Bad MetricsQueryRequest"),
	})
	...
	for _, tc := range []struct {
	...
	}{
		{
			name:            "retry failures",
			expectedRetries: 4,
			handler: MetricsQueryHandlerFunc(func(_ context.Context, req MetricsQueryRequest) (Response, error) {
				if try.Inc() == 5 {
...
func Test_RetryMiddlewareCancel(t *testing.T) {
	...
	_, err := newRetryMiddleware(log.NewNopLogger(), 5, nil).Wrap(
		MetricsQueryHandlerFunc(func(c context.Context, r MetricsQueryRequest) (Response, error) {
			try.Inc()
			return nil, ctx.Err()
		}),
	).Do(ctx, nil)
