...
func TestQueryMiddleware_Errors(t *testing.T) {
	tests := map[string]struct {
		...
		"err-mimir-max-total-query-length has a correct message": {
			...
			expectedErrorMsg: "the total query time range exceeds the limit (query length: 1h0m0s, limit: 1m0s) (err-mimir-max-total-query-length). To adjust the related per-tenant limit, configure max_total_query_length, or contact your service administrator.",
		},
		"err-mimir-max-query-expression-size-bytes has a correct message": {
			err:              newMaxQueryExpressionSizeBytesError(10, 20),
			expectedErrorMsg: "the raw query size in bytes exceeds the limit (query size: 10, limit: 20) (err-mimir-max-query-expression-size-bytes). To adjust the related per-tenant limit, configure max_query_expression_size_bytes, or contact your service administrator.",
		},
