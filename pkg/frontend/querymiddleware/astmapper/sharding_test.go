...
func TestShardSummer(t *testing.T) {
	for _, tt := range []struct {
	...
	}{
		{
			...
			`max_over_time(` + concatWithSubquery(`(max(hello) > max(hello2))[5m:1m]`) + `[5m])`,
			0,
		},
		// {
		// 	`quantile(0.9,foo)`,
...
func concatWithSubquery(subquery string, queryStrs ...string) string {
	if len(queryStrs) == 0 {
		queryStrs = []string{subquery}
	}
	queries := make([]EmbeddedQuery, len(queryStrs))
	for i, query := range queryStrs {
		queries[i] = NewEmbeddedQuery(query, nil)
		queries[i].SourceSubquery = subquery
	}
	return concatInner(queries...)
}
...
func concatInner(rawQueries ...EmbeddedQuery) string {
	...
	for i, q := range rawQueries {
		...
		queries[i].SourceSubquery = q.SourceSubquery
	}
