			...
			for reqName, reqData := range requestTypes {
				for _, reqMethod := range []string{http.MethodGet, http.MethodPost} {
					t.Run(fmt.Sprintf("%s (%s)", reqName, reqMethod), func(t *testing.T) {
						...
						rt := newRoundTripper(cacheBackend, DefaultCacheKeyGenerator{codec: NewPrometheusCodec(reg, 0*time.Minute, formatJSON)}, limits, downstream, mimirtest.NewTestingLogger(t), reg)
