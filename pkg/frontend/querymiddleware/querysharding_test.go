...
func TestQuerySharding_Correctness(t *testing.T) {
	for _, upstreamRerun := range []bool{false, true} {
		t.Run(fmt.Sprintf("upstreamRerun=%v", upstreamRerun), func(t *testing.T) {
						...
						t.Run(fmt.Sprintf("%T", req), func(t *testing.T) {
							...
							for _, numShards := range []int{2, 4, 8, 16} {
								t.Run(fmt.Sprintf("shards=%d", numShards), func(t *testing.T) {
									...
									if upstreamRerun {
										ctx = context.WithValue(ctx, "fullRangeHandler", fullHandler)
									}
