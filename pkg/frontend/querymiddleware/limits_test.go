...
func TestLimitsMiddleware_MaxQueryLookback(t *testing.T) {
	...
	tests := map[string]struct {
		...
		blockRetentionPeriod time.Duration
		...
		"should not manipulate time range for a query on short time range": {
			...
			blockRetentionPeriod: thirtyDays,
		...
		"should manipulate a query where maxQueryLookback is past the retention period": {
			maxQueryLookback:     thirtyDays,
			blockRetentionPeriod: thirtyDays - (24 * time.Hour),
	...
	for testName, testData := range tests {
		t.Run(testName, func(t *testing.T) {
			...
			limits := mockLimits{maxQueryLookback: testData.maxQueryLookback, compactorBlockRetentionPeriod: testData.blockRetentionPeriod}
...
type mockLimits struct {
	...
	totalShards                   int
	compactorShards               int
	compactorBlockRetentionPeriod time.Duration
...
func (m mockLimits) CompactorBlocksRetentionPeriod(userID string) time.Duration {
	return m.compactorBlockRetentionPeriod
}
