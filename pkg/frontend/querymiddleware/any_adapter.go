...
// AnyAdapter bridges the gogo-generated types.Any to wiresmith-generated code:
// wiresmith cannot embed message types generated by another protobuf runtime,
// so Extent.Response is declared google.protobuf.Any on the wire but carries
// this adapter in Go. Wire format is identical.
//
// results_cache.go packs/unpacks the response with gogo types.MarshalAny /
// types.UnmarshalAny / types.EmptyAny, which resolve payload type URLs through
// the gogo registry (wiresmith's native anypb resolves only through the
// official registry), so the field must stay on gogo's types.Any. Convert at
// the boundary with Any() / FromAny.
type AnyAdapter types.Any

// Any returns the gogo representation. A zero adapter yields nil, matching the
// previous *types.Any field shape where absent meant nil.
