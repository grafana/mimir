...
func (r *remoteReadRoundTripperV2) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	// `QueryDetails` isn't thread-safe, but all queries round asynchronously,
	// so we track them individually and merge them after.
	reqDetails := querydetails.QueryDetailsFromContext(rtCtx)
	subQueryDetails := make([]*querydetails.QueryDetails, len(queries))
	...
	responses, err := collectOrCleanupResponses(rtCtx, len(queries), maxParallelism, func(queryCtx context.Context, queryIdx int) (Response, error) {
		...
		if reqDetails != nil {
			subQueryDetails[queryIdx], queryCtx = querydetails.ContextWithEmptyDetails(queryCtx)
		}
	...
	for _, subDetails := range subQueryDetails {
		reqDetails.Merge(subDetails)
	}
...
var errStreamedResponseClosed = cancellation.NewErrorf("streamed remote read response closed")

func (b *mergedStreamedBody) Close() error {
	err := b.PipeReader.Close()
	b.reader.CancelAll(errStreamedResponseClosed)
	<-b.done
	return err
}
...
func mergeStreamedResponses(responses []Response) (io.ReadCloser, error) {
	...
	return &mergedStreamedBody{PipeReader: pr, reader: chunkReader, done: done}, nil
}
