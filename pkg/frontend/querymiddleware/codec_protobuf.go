...
func (f ProtobufFormatter) EncodeQueryResponse(resp *PrometheusResponse) ([]byte, error) {
	...
	if uint64(size) > f.maxEncodedSize {
		return nil, apierror.Newf(apierror.TypeTooLargeEntry, "Protobuf response (%d bytes) is larger than the maximum allowed (%d bytes)", size, f.maxEncodedSize)
	}

	// The code below is based on the autogenerated Marshal(), but takes advantage of the fact
	// we've already called Size() above.
	data := make([]byte, size)
	n, err := payload.MarshalToSizedBuffer(data[:size])
	if err != nil {
		return nil, err
	}
