...
func testFrontend(t *testing.T, config CombinedFrontendConfig, handler http.Handler, test func(addr string), l log.Logger) {
	...
	worker, err = querier_worker.NewQuerierWorker(workerConfig, httpgrpc_server.NewServer(handler, []httpgrpc_server.Option{httpgrpc_server.WithReturn4XXErrors}...), logger, nil)
	require.NoError(t, err)
