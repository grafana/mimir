...
// DirectlyUnmarshalledByteSlice is an alternative to the default handling of []byte values in protobuf messages.
// Unlike the default protobuf implementation, when unmarshalling, DirectlyUnmarshalledByteSlice holds a reference to the
...
//
// Note that DirectlyUnmarshalledByteSlice will not behave correctly if the protobuf-encoded bytes are reused (eg. if the
...
type DirectlyUnmarshalledByteSlice []byte

func (t *DirectlyUnmarshalledByteSlice) MarshalTo(data []byte) (n int, err error) {
	copy(data, *t)

	return t.Size(), nil
}

func (t *DirectlyUnmarshalledByteSlice) Unmarshal(data []byte) error {
	*t = data

	return nil
}

func (t *DirectlyUnmarshalledByteSlice) Size() int {
	return len(*t)
}

func (t DirectlyUnmarshalledByteSlice) Equal(other DirectlyUnmarshalledByteSlice) bool {
