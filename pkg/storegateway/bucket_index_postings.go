...
func (g rawPostingGroup) filterNonExistingKeys(ctx context.Context, r indexheader.Reader) ([]labels.Label, []index.Range, int64, error) {
	var (
		writeIdx  int
		totalSize int64
		offsets   = make([]index.Range, 0, len(g.keys))
	)
	...
	return g.keys[:writeIdx], offsets, totalSize, nil
}
...
type postingGroup struct {
	...
	// offsets holds the posting-list byte range for each entry in keys, in the
	// same order, as resolved while building the group. It lets ExpandedPostings
	// reuse these ranges instead of resolving them again with PostingsOffset.
	// It is either nil (offsets unknown, e.g. the synthetic all-postings group)
	// or the same length as keys.
	offsets []index.Range
