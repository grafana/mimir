...
// Per-message value-comparison methods (Equal + Compare) for github.com/grafana/mimir/pkg/storegateway/storepb/cache.proto.
//
...
func (this *CachedSeries) Equal(that interface{}) bool {
	...
	for i := range this.Series {
		if !this.Series[i].EqualWiresmith(that1.Series[i]) {
			return false
		}
	}
...
func (this *CachedSeries) Compare(that interface{}) int {
	...
	for i := range this.Series {
		if c := this.Series[i].CompareWiresmith(that1.Series[i]); c != 0 {
			return c
		}
	}
	if c := bytes.Compare(this.DiffEncodedPostings, that1.DiffEncodedPostings); c != 0 {
		return c
	}
	return 0
}
