...
func skipField_Rpc(b []byte, num protowire.Number, typ protowire.Type) (int, error) {
	switch typ {
...
func (m *SeriesRequest) Unmarshal(b []byte) error {
	...
	for len(b) > 0 {
		...
		case 14: // request_hints
			...
			if err := m.RequestHints.Unmarshal(v); err != nil {
				return err
			}
			b = b[n:]
...
func (m *SeriesResponse) Unmarshal(b []byte) error {
	for len(b) > 0 {
		...
		case 2: // warning
			...
			m.Result = &SeriesResponse_Warning{Warning: v}
			b = b[n:]
		case 3: // hints
			if typ != protowire.BytesType {
				n, err := skipField_Rpc(b, num, typ)
...
func (m *LabelNamesResponse) Unmarshal(b []byte) error {
	...
	for len(b) > 0 {
		...
		case 4: // response_hints
			...
			if err := m.ResponseHints.Unmarshal(v); err != nil {
				return err
			}
			b = b[n:]
		default:
