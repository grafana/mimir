...
// so the deprecated hints fields are declared google.protobuf.Any on the wire
// but carry this adapter in Go. Wire format is identical.
//
// The hints payloads are gogo-registry hintspb messages that callers unpack
// with gogo types.MarshalAny / types.UnmarshalAny, so the field must stay on
// gogo's types.Any (wiresmith's native anypb resolves type URLs only through
// the official registry). Convert at the boundary with Any() / FromAny.
