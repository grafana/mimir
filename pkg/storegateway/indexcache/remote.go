...
func postingsCacheKeyBuf(userID string, blockID string, l labels.Label, buf *[]byte) string {
	...
	lblHash, hashLen := postingsCacheKeyLabelHash(l)
	...
	offset += copy(key[offset:], separator)
	base64.RawURLEncoding.Encode(key[offset:], lblHash[0:hashLen])

	// Convert []byte to string with no extra allocation.
	sizedKey := key[:offset+base64.RawURLEncoding.EncodedLen(len(lblHash[0:hashLen]))]
	return *(*string)(unsafe.Pointer(&sizedKey))
}

// postingsCacheKeyLabelHash returns the hash of the input label, to be used
// as part of the cache key generated by postingsCacheKey().
func postingsCacheKeyLabelHash(l labels.Label) (out [blake2b.Size256]byte, _ int) {
	const separator = ":"
	...
	if offset != expectedLen {
		panic(fmt.Sprintf("postingsCacheKeyLabelHash() computed an invalid expected length (expected: %d, actual: %d)", expectedLen, offset))
	}
