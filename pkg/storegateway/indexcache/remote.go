...
func postingsCacheKeyBuf(userID string, blockID string, l labels.Label, buf *[]byte) string {
	...
	sizedKey := key[:offset+base64.RawURLEncoding.EncodedLen(len(lblHash[:hashLen]))]
...
func postingsCacheKeyLabelHash(l labels.Label) (out [blake2b.Size256]byte, outLen int) {
	const separator = ":"
