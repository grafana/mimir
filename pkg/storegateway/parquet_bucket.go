...
// ParquetBucketStore implements the store API backed by a bucket. It loads all index
// files to local disk.
//
...
type ParquetBucketStore struct {
	...
	blockSet *parquetBucketBlockSet
...
// ParquetBucketStoreOption are functions that configure ParquetBucketStore.
type ParquetBucketStoreOption func(s *ParquetBucketStore)

// WithLoggerP sets the ParquetBucketStore logger to the one you pass.
func WithLoggerP(logger log.Logger) ParquetBucketStoreOption {
	return func(s *ParquetBucketStore) {
		s.logger = logger
	}
}

// WithIndexCacheP sets a indexCache to use instead of a noopCache.
func WithIndexCacheP(cache indexcache.IndexCache) ParquetBucketStoreOption {
	return func(s *ParquetBucketStore) {
		s.indexCache = cache
	}
}

// WithQueryGateP sets a queryGate to use instead of a gate.NewNoop().
func WithQueryGateP(queryGate gate.Gate) ParquetBucketStoreOption {
	return func(s *ParquetBucketStore) {
		s.queryGate = queryGate
	}
}

// WithLazyLoadingGateP sets a lazyLoadingGate to use instead of a gate.NewNoop().
func WithLazyLoadingGateP(lazyLoadingGate gate.Gate) ParquetBucketStoreOption {
	return func(s *ParquetBucketStore) {
...
func NewParquetBucketStore(
	...
	seriesHashCache *hashcache.SeriesHashCache,
	metrics *BucketStoreMetrics,
	options ...ParquetBucketStoreOption,
	...
	s := &ParquetBucketStore{
		...
		blockSet:                    newParquetBucketBlockSet(),
...
func (s *ParquetBucketStore) start(context.Context) error {
...
// RemoveBlocksAndClose remove all blocks from local disk and releases all resources associated with the ParquetBucketStore.
...
// Stats returns statistics about the ParquetBucketStore instance.
...
func (s *ParquetBucketStore) syncBlocks(ctx context.Context) error {
	...
	// Start snapshotter in the end of the sync, but do that only once per ParquetBucketStore's lifetime.
...
func (s *ParquetBucketStore) loadBlocks(ctx context.Context, blocks map[ulid.ULID]struct{}) {
	...
	s.blockSet.forEach(func(b *parquetBucketBlock) {
...
func (s *ParquetBucketStore) removeBlock(id ulid.ULID) (returnErr error) {
	...
	// The block has already been removed from ParquetBucketStore, so we track it as removed
...
func (s *ParquetBucketStore) recordRequestAmbientTime(stats *safeQueryStats, requestStart time.Time) {
...
func (s *ParquetBucketStore) sendStreamingSeriesLabelsAndStats(
...
func (s *ParquetBucketStore) sendSeriesChunks(
	req *storepb.SeriesRequest,
	srv storegatewaypb.StoreGateway_SeriesServer,
	seriesSet storepb.SeriesSet,
	stats *safeQueryStats,
...
func (s *ParquetBucketStore) createIteratorForNonChunksStreamingRequest(
	ctx context.Context,
	req *storepb.SeriesRequest,
	blocks []*parquetBucketBlock,
...
func (s *ParquetBucketStore) createIteratorForChunksStreamingLabelsPhase(
	ctx context.Context,
	req *storepb.SeriesRequest,
	blocks []*parquetBucketBlock,
	indexReaders map[ulid.ULID]*bucketIndexReader,
...
func (s *ParquetBucketStore) createIteratorForChunksStreamingChunksPhase(
	ctx context.Context,
	chunkReaders *bucketChunkReaders,
	stats *safeQueryStats,
	chunksLimiter ChunksLimiter,
...
func (s *ParquetBucketStore) getSeriesIteratorFromBlocks(
	ctx context.Context,
	req *storepb.SeriesRequest,
	blocks []*parquetBucketBlock,
	indexReaders map[ulid.ULID]*bucketIndexReader,
...
func (s *ParquetBucketStore) getSeriesIteratorFromPerBlockIterators(perBlockIterators []iterator[seriesChunkRefsSet], chunksLimiter ChunksLimiter, seriesLimiter SeriesLimiter) iterator[seriesChunkRefsSet] {
...
func (s *ParquetBucketStore) recordSeriesCallResult(safeStats *safeQueryStats) {
...
func (s *ParquetBucketStore) recordLabelNamesCallResult(safeStats *safeQueryStats) {
	stats := safeStats.export()
	s.recordPostingsStats(stats)
	s.recordSeriesStats(stats)
	s.recordCachedPostingStats(stats)
...
func (s *ParquetBucketStore) recordLabelValuesCallResult(safeStats *safeQueryStats) {
	stats := safeStats.export()
	s.recordPostingsStats(stats)
	s.recordSeriesStats(stats)
	s.recordStreamingSeriesStats(stats)
...
func (s *ParquetBucketStore) recordPostingsStats(stats *queryStats) {
...
func (s *ParquetBucketStore) recordSeriesStats(stats *queryStats) {
...
func (s *ParquetBucketStore) recordStreamingSeriesStats(stats *queryStats) {
...
func (s *ParquetBucketStore) recordCachedPostingStats(stats *queryStats) {
...
func (s *ParquetBucketStore) recordSeriesHashCacheStats(stats *queryStats) {
...
func (s *ParquetBucketStore) openBlocksForReading(ctx context.Context, skipChunks bool, minT, maxT int64, blockMatchers []*labels.Matcher, stats *safeQueryStats) ([]*parquetBucketBlock, map[ulid.ULID]*bucketIndexReader, map[ulid.ULID]chunkReader) {
	...
	s.blockSet.filter(minT, maxT, blockMatchers, func(b *parquetBucketBlock) {
...
func (s *ParquetBucketStore) LabelNames(ctx context.Context, req *storepb.LabelNamesRequest) (*storepb.LabelNamesResponse, error) {
	...
	s.blockSet.filter(req.Start, req.End, reqBlockMatchers, func(b *parquetBucketBlock) {
		...
		g.Go(func() error {
			...
			result, err := parquetBlockLabelNames(gctx, b, reqSeriesMatchers, seriesLimiter, s.maxSeriesPerBatch, s.logger, stats)
			if err != nil {
...
func parquetBlockLabelNames(ctx context.Context, b *parquetBucketBlock, matchers []*labels.Matcher, seriesLimiter SeriesLimiter, seriesPerBatch int, logger log.Logger, stats *safeQueryStats) ([]string, error) {
	names, ok := fetchCachedLabelNames(ctx, b.indexCache, b.userID, b.meta.ULID, matchers, logger)
	...
	storeCachedLabelNames(ctx, b.indexCache, b.userID, b.meta.ULID, matchers, names, logger)
	return names, nil
}
...
func (s *ParquetBucketStore) LabelValues(ctx context.Context, req *storepb.LabelValuesRequest) (*storepb.LabelValuesResponse, error) {
	...
	s.blockSet.filter(req.Start, req.End, reqBlockMatchers, func(b *parquetBucketBlock) {
		...
		g.Go(func() error {
			...
			result, err := parquetBlockLabelValues(gctx, b, s.postingsStrategy, s.maxSeriesPerBatch, req.Label, reqSeriesMatchers, s.logger, stats)
			if err != nil {
				return errors.Wrapf(err, "block %s", b.meta.ULID)
			}
...
// parquetBlockLabelValues returns sorted values of the label with requested name,
...
func parquetBlockLabelValues(ctx context.Context, b *parquetBucketBlock, postingsStrategy postingsSelectionStrategy, maxSeriesPerBatch int, labelName string, matchers []*labels.Matcher, logger log.Logger, stats *safeQueryStats) ([]string, error) {
...
// parquetBucketBlockSet holds all blocks.
type parquetBucketBlockSet struct {
	// mtx protects the below data strcutures, helping to keep them in sync.
	mtx      sync.RWMutex
	blockSet sync.Map              // Maps block's ulid.ULID to the *parquetBucketBlock.
	blocks   []*parquetBucketBlock // Blocks sorted by mint, then maxt.
}

// newParquetBucketBlockSet initializes a new set with the known downsampling windows hard-configured.
// (Mimir only supports no-downsampling)
// The set currently does not support arbitrary ranges.
func newParquetBucketBlockSet() *parquetBucketBlockSet {
	return &parquetBucketBlockSet{}
}

// add adds a block to the set.
func (s *parquetBucketBlockSet) add(b *parquetBucketBlock) error {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (s *parquetBucketBlockSet) remove(id ulid.ULID) *parquetBucketBlock {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (s *parquetBucketBlockSet) contains(id ulid.ULID) bool {
	_, ok := s.blockSet.Load(id)
	return ok
}

func (s *parquetBucketBlockSet) len() int {
	s.mtx.RLock()
	defer s.mtx.RUnlock()
	return len(s.blocks)
}
...
func (s *parquetBucketBlockSet) filter(mint, maxt int64, blockMatchers []*labels.Matcher, fn func(b *parquetBucketBlock)) {
	if mint > maxt {
		return
	}
	...
	bs := make([]*parquetBucketBlock, 0, len(s.blocks))
...
func (s *parquetBucketBlockSet) forEach(fn func(b *parquetBucketBlock)) {
...
func (s *parquetBucketBlockSet) closeAll() error {
	errs := multierror.New()
	s.blockSet.Range(func(_, val any) bool {
...
func (s *parquetBucketBlockSet) openBlocksULIDs() []ulid.ULID {
...
func (s *parquetBucketBlockSet) allBlockULIDs() []ulid.ULID {
	s.mtx.RLock()
	defer s.mtx.RUnlock()
...
func (s *parquetBucketBlockSet) timerange() (mint, maxt int64) {
	s.mtx.RLock()
	defer s.mtx.RUnlock()
...
// parquetBucketBlock represents a block that is located in a bucket. It holds intermediate
...
func newParquetBucketBlock(
	...
	p blockPartitioners,
) (b *parquetBucketBlock, err error) {
	b = &parquetBucketBlock{
		userID:            userID,
		logger:            logger,
		metrics:           metrics,
		bkt:               bkt,
...
func (b *parquetBucketBlock) indexFilename() string {
	return path.Join(b.meta.ULID.String(), block.IndexFilename)
}

func (b *parquetBucketBlock) indexRangeReader(ctx context.Context, off, length int64) (io.ReadCloser, error) {
...
func (b *parquetBucketBlock) readIndexRange(ctx context.Context, off, length int64) ([]byte, error) {
...
func (b *parquetBucketBlock) chunkRangeReader(ctx context.Context, seq int, off, length int64) (io.ReadCloser, error) {
...
func (b *parquetBucketBlock) chunkReader(ctx context.Context) *bucketChunkReader {
	b.pendingReaders.Add(1)

	// TODO(npazosmendez)
	return newBucketChunkReader(ctx, nil /*b*/)
}
...
func (b *parquetBucketBlock) ensureIndexHeaderLoaded(ctx context.Context, stats *safeQueryStats) {
	_, span := tracer.Start(ctx, "parquetBucketBlock.ensureIndexHeaderLoaded")
	defer span.End()
