...
func TestLoadingSeriesChunksSetIterator(t *testing.T) {
	...
	for testName, testCase := range testCases {
		t.Run(testName, func(t *testing.T) {
			...
			// Run test
			set := newLoadingSeriesChunksSetIterator(context.Background(), *readers, newSliceSeriesChunkRefsSetIterator(nil, testCase.setsToLoad...), 100, newSafeQueryStats())
	...
	for batchSize := 10; batchSize <= 10000; batchSize *= 10 {
		b.Run(fmt.Sprintf("batch size: %d", batchSize), func(b *testing.B) {
			...
			for n := 0; n < b.N; n++ {
				...
				it := newLoadingSeriesChunksSetIterator(context.Background(), *chunkReaders, newSliceSeriesChunkRefsSetIterator(nil, sets...), batchSize, stats)
