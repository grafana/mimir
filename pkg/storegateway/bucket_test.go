	...
	for name, newTestBucketBlock := range bucketBlockFactories {
		t.Run(name, func(t *testing.T) {
			...
			t.Run("postings selection strategy is respected", func(t *testing.T) {
				...
				refsWithDeferredMatchers, deferredMatchers, err := b.indexReader(omitMatcherStrategy{inverseMatcher}).ExpandedPostings(ctx, matchers, stats)
				require.NoError(t, err)
				if assert.Len(t, deferredMatchers, 1) {
					assert.Equal(t, inverseMatcher, deferredMatchers[0])
				}

				refsWithoutDeferredMatchers, deferredMatchers, err := b.indexReader(selectAllStrategy{}).ExpandedPostings(ctx, matchers[:1], stats)
				require.NoError(t, err)
