		...
		t.Run(fmt.Sprintf("batch size: %d", batchSize), func(t *testing.T) {
			...
			store, err := NewBucketStore(
				...
				mimir_tsdb.BucketStoreConfig{
					...
					PostingOffsetsInMemSampling:           mimir_tsdb.DefaultPostingOffsetInMemorySampling,
					IndexHeader:                           indexheader.Config{},
					IndexHeaderLazyLoadingEnabled:         false, // Lazy index-header loading disabled.
					IndexHeaderLazyLoadingIdleTimeout:     0,
					IndexHeaderEagerLoadingStartupEnabled: false,
				},
				selectAllStrategy{},
...
func TestBucketStore_Series_OneBlock_InMemIndexCacheSegfault(t *testing.T) {
	...
	store := &BucketStore{
		...
		indexReaderPool: indexheader.NewReaderPool(log.NewNopLogger(), false, 0, false, indexheader.NewReaderPoolMetrics(nil), indexheader.LazyLoadedHeadersSnapshotConfig{}),
		metrics:         NewBucketStoreMetrics(nil),
