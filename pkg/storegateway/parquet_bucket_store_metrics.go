...
// ParquetBucketStoreMetrics holds all the metrics tracked by ParquetBucketStore. These metrics
// MUST be monotonic (counter, summary, histogram) because a single metrics instance
// can be passed to multiple ParquetBucketStore and metrics MUST be correct even after a
// ParquetBucketStore is offloaded.
type ParquetBucketStoreMetrics struct {
	...
	indexHeaderReaderMetrics *parquetblock.ReaderPoolMetrics
}

func NewParquetBucketStoreMetrics(reg prometheus.Registerer) *ParquetBucketStoreMetrics {
	var m ParquetBucketStoreMetrics
