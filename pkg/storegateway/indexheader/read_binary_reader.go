...
// NewReadBinaryReader loads or builds new index-header if not present on disk.
func NewReadBinaryReader(ctx context.Context, logger log.Logger, bkt objstore.BucketReader, dir string, id ulid.ULID, postingOffsetsInMemSampling int) (*ReadBinaryReader, error) {
	binfn := filepath.Join(dir, id.String(), block.IndexHeaderFilename)
	br, err := newFileReadBinaryReader(binfn, postingOffsetsInMemSampling)
	if err == nil {
		return br, nil
	}
...
func newFileReadBinaryReader(path string, postingOffsetsInMemSampling int) (bw *ReadBinaryReader, err error) {
	...
	r.symbols, err = index.NewSymbols(symbolsByteSlice, r.indexVersion, 0)
	if err != nil {
		return nil, errors.Wrap(err, "read symbols")
	}
	...
	if r.indexVersion == index.FormatV1 {
		...
		if err := index.ReadOffsetTable(offTableByteSlice, 0, func(key []string, off uint64, tableOff int) error {
			if len(key) != 2 {
				return errors.Errorf("unexpected key length for posting table %d", len(key))
			}
...
func (r *ReadBinaryReader) PostingsOffset(name, value string) (index.Range, error) {
...
func (r *ReadBinaryReader) postingsOffset(name string, values ...string) ([]index.Range, error) {
...
func (r *ReadBinaryReader) LookupSymbol(o uint32) (string, error) {
...
func (r *ReadBinaryReader) LabelValues(name string, filter func(string) bool) ([]string, error) {
	if r.indexVersion == index.FormatV1 {
		e, ok := r.postingsV1[name]
		if !ok {
			return nil, nil
...
func (r *ReadBinaryReader) LabelNames() ([]string, error) {
...
func (r *ReadBinaryReader) Close() error { return r.f.Close() }
