...
type PopulateBinaryReader struct {
	b   index.ByteSlice
	toc *BinaryTOC
...
// NewPopulateBinaryReader loads or builds new index-header if not present on disk.
func NewPopulateBinaryReader(ctx context.Context, logger log.Logger, bkt objstore.BucketReader, dir string, id ulid.ULID, postingOffsetsInMemSampling int) (*PopulateBinaryReader, error) {
	binfn := filepath.Join(dir, id.String(), block.IndexHeaderFilename)
	br, err := newFilePopulateBinaryReader(binfn, postingOffsetsInMemSampling)
	if err == nil {
		return br, nil
	}
	...
	return newFilePopulateBinaryReader(binfn, postingOffsetsInMemSampling)
}

func newFilePopulateBinaryReader(path string, postingOffsetsInMemSampling int) (bw *PopulateBinaryReader, err error) {
...
func (r *PopulateBinaryReader) IndexVersion() (int, error) {
	return r.indexVersion, nil
}

// TODO(bwplotka): Get advantage of multi value offset fetch.
func (r *PopulateBinaryReader) PostingsOffset(name, value string) (index.Range, error) {
...
func (r *PopulateBinaryReader) postingsOffset(name string, values ...string) ([]index.Range, error) {
...
func (r *PopulateBinaryReader) LookupSymbol(o uint32) (string, error) {
...
func (r *PopulateBinaryReader) LabelValues(name string, filter func(string) bool) ([]string, error) {
	if r.indexVersion == index.FormatV1 {
		e, ok := r.postingsV1[name]
		if !ok {
			return nil, nil
...
func (r *PopulateBinaryReader) LabelNames() ([]string, error) {
...
func (r *PopulateBinaryReader) Close() error { return r.c.Close() }
