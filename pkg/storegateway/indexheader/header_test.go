...
func TestReaders(t *testing.T) {
	...
	id1, err := helpers.CreateBlock(ctx, tmpDir, []labels.Labels{
		{{Name: "a", Value: "1"}},
		{{Name: "a", Value: "2"}},
		{{Name: "a", Value: "3"}},
		{{Name: "a", Value: "4"}},
...
func benchmarkBinaryReaderLookupSymbol(b *testing.B, numSeries int) {
	...
	// Create a block.
	id1, err := helpers.CreateBlock(ctx, tmpDir, seriesLabels, 100, 0, 1000, labels.Labels{{Name: "ext1", Value: "1"}}, 124, metadata.NoneFunc)
	require.NoError(b, err)
