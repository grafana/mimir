...
func BenchmarkLabelValuesOffsetsIndexV1(b *testing.B) {
	...
	benchmarkReader(b, bucketDir, metaIndexV1.ULID, func(b *testing.B, br Reader) {
		...
		for i := 0; i < b.N; i++ {
			...
			values, err := br.LabelValuesOffsets(ctx, name, "", func(s string) bool {
				return true
			})
...
func BenchmarkLabelValuesOffsetsIndexV2_WithPrefix(b *testing.B) {
	...
	r, err := NewStreamBinaryReader(ctx, log.NewNopLogger(), nil, blockDir, blockID, true, 32, NewStreamBinaryReaderMetrics(nil), Config{})
	require.NoError(b, err)
		...
		b.Run(lbl, func(b *testing.B) {
			for _, tc := range tcs {
				b.Run(fmt.Sprintf("prefix='%s'%s", tc.prefix, tc.desc), func(b *testing.B) {
					for i := 0; i < b.N; i++ {
						values, err := r.LabelValuesOffsets(ctx, lbl, tc.prefix, tc.filter)
						require.NoError(b, err)
