...
func (cfg *Config) RegisterFlagsWithPrefix(f *flag.FlagSet, prefix string) {
	...
	f.BoolVar(&cfg.LazyLoadingEnabled, prefix+"lazy-loading-enabled", tsdb.DefaultIndexHeaderLazyLoadingEnabled, "If enabled, store-gateway will lazy load an index-header only once required by a query.")
	f.DurationVar(&cfg.LazyLoadingIdleTimeout, prefix+"lazy-loading-idle-timeout", tsdb.DefaultIndexHeaderLazyLoadingIdleTimeout, "If index-header lazy loading is enabled and this setting is > 0, the store-gateway will offload unused index-headers after 'idle timeout' inactivity.")
