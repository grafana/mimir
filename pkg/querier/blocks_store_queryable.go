...
func newStoreConsistencyCheckFailedError(remainingBlocks []ulid.ULID) error {
	return fmt.Errorf("%v. The blocks are: %s", globalerror.StoreConsistencyCheckFailed.Message("the consistency check failed because some blocks were not queried"), strings.Join(convertULIDsToString(remainingBlocks), " "))
}
