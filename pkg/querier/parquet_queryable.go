...
func NewParquetStoreQueryable(
	...
	cache, err := mimir_storage.NewCache[*mimir_storage.ParquetReader]("files", 1000, cacheMetrics)
	if err != nil {
		return nil, err
	}
...
func (q *parquetQuerier) findBlocks(ctx context.Context, userID string, logger log.Logger, minT, maxT int64, matchers []*labels.Matcher) (bucketindex.Blocks, error) {
	...
	if q.queryStoreAfter > 0 {
		...
		if origMaxT != maxT {
			level.Debug(q.logger).Log("msg", "the max time of the query to blocks storage has been manipulated", "original", origMaxT, "updated", maxT)
		}
...
func (q *parquetQuerier) searchWithMatchers(ctx context.Context, reader *mimir_storage.ParquetReader, sts *mimir_storage.Stats, matchers ...*labels.Matcher) ([][]int64, error) {
	start := time.Now()
	defer func() {
...
func (q *parquetQuerier) blocksToParquetReader(ctx context.Context, uname string, blocks bucketindex.Blocks, sts *mimir_storage.Stats) ([]*mimir_storage.ParquetReader, error) {
	start := time.Now()
	defer func() {
		sts.AddOpenParquetReaderDuration(time.Since(start).Milliseconds())
	}()

	spanLog, ctx := spanlogger.New(ctx, "parquetQuerier.blocksToParquetReader ")
	defer spanLog.Finish()
	output := make([]*mimir_storage.ParquetReader, len(blocks))
	mtx := &sync.Mutex{}
	eg := &errgroup.Group{}
	readerCacheMiss := 0
	defer func() {
	...
	for i, block := range blocks {
		...
		eg.Go(func() error {
			...
			if err != nil {
				level.Error(q.logger).Log("msg", "error creating parquet reader", "err", err, "path", name)
				spanLog.SetError()
				return err
			}
			mtx.Lock()
