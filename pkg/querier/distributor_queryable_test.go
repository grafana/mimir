...
func TestDistributorQuerier_Select(t *testing.T) {
	...
	for name, testCase := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			d.On("QueryStream", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(testCase.response, nil)
...
func TestDistributorQuerier_Select_ClosedBeforeSelectFinishes(t *testing.T) {
	...
	d.On("QueryStream", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for responseName, responseType := range responseTypes {
				t.Run(responseName, func(t *testing.T) {
					...
					d.On("QueryStream", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(responseType.combinedResponse, nil)
...
func BenchmarkDistributorQuerier_Select(b *testing.B) {
	...
	d := &mockDistributor{}
	for n := 0; n < b.N; n++ {
		d.On("QueryStream", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(makeResponse(), nil).Once()
	}
...
func (m *mockDistributor) QueryStream(ctx context.Context, queryMetrics *stats.QueryMetrics, from, to model.Time, projectionLabels []string, matchers ...*labels.Matcher) (client.CombinedQueryStreamResponse, error) {
	args := m.Called(ctx, queryMetrics, from, to, projectionLabels, matchers)
