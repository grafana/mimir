...
func TestStoreGatewayStreamReader_HappyPaths(t *testing.T) {
	...
	for name, testCase := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			reader := newStoreGatewayStreamReader(ctx, mockClient, 5, queryLimiter, memoryTracker, &stats.QueryStats{}, metrics, log.NewNopLogger())
...
func TestStoreGatewayStreamReader_AbortsWhenParentContextCancelled(t *testing.T) {
	...
	for name, testCase := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			metrics := newBlocksStoreQueryableMetrics(prometheus.NewPedanticRegistry())
			reader := newStoreGatewayStreamReader(parentCtx, mockClient, 3, queryLimiter, memoryTracker, &stats.QueryStats{}, metrics, log.NewNopLogger())
			cancel()
			reader.StartBuffering()
...
func TestStoreGatewayStreamReader_ReadingSeriesOutOfOrder(t *testing.T) {
	...
	memoryTracker := limiter.NewMemoryConsumptionTracker(ctx, 0, nil, "")
	metrics := newBlocksStoreQueryableMetrics(prometheus.NewPedanticRegistry())
	reader := newStoreGatewayStreamReader(ctx, mockClient, 1, queryLimiter, memoryTracker, &stats.QueryStats{}, metrics, log.NewNopLogger())
	reader.StartBuffering()
...
func TestStoreGatewayStreamReader_ReceivedFewerSeriesThanExpected(t *testing.T) {
	...
	memoryTracker := limiter.NewMemoryConsumptionTracker(ctx, 0, nil, "")
	metrics := newBlocksStoreQueryableMetrics(prometheus.NewPedanticRegistry())
	reader := newStoreGatewayStreamReader(ctx, mockClient, 3, queryLimiter, memoryTracker, &stats.QueryStats{}, metrics, log.NewNopLogger())
	reader.StartBuffering()
