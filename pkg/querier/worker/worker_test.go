...
func TestResetConcurrency(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			w, err := newQuerierWorkerWithProcessor(cfg.QuerySchedulerGRPCClientConfig, cfg, log.NewNopLogger(), &mockProcessor{}, nil, nil)
...
func TestQuerierWorker_getDesiredConcurrency(t *testing.T) {
	...
	for testName, testData := range tests {
		t.Run(testName, func(t *testing.T) {
			...
			w, err := newQuerierWorkerWithProcessor(cfg.QueryFrontendGRPCClientConfig, cfg, log.NewNopLogger(), &mockProcessor{}, nil, nil)
			require.NoError(t, err)
