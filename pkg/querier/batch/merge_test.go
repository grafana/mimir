...
func TestMergeIteratorWithNoChunks(t *testing.T) {
	...
	t.Run("NewGenericChunkMergeIterator with nil chunk slice", func(t *testing.T) {
		iter := NewGenericChunkMergeIterator(nil, labels.EmptyLabels(), nil)
		require.Equal(t, chunkenc.ValNone, iter.Next())
		require.NoError(t, iter.Err())
	})
	...
	t.Run("reused adapter with empty chunk slice", func(t *testing.T) {
		...
		iter := NewGenericChunkMergeIterator(nil, labels.EmptyLabels(), []GenericChunk{mkGenericChunk(t, 0, 100, enc)})
		for iter.Next() != chunkenc.ValNone { //nolint:revive
		}
		require.NoError(t, iter.Err())

		iter = NewGenericChunkMergeIterator(iter, labels.EmptyLabels(), nil)
		require.Equal(t, chunkenc.ValNone, iter.Next())
		require.NoError(t, iter.Err())
	})
}
...
type checkHintTestSample struct {
	t       int64
	v       int
	...
	for _, enc := range []chunk.Encoding{chunk.PrometheusHistogramChunk, chunk.PrometheusFloatHistogramChunk} {
		addFunc := func(chk chunk.EncodedChunk, ts, val int) {
			...
			case chunk.PrometheusFloatHistogramChunk:
				...
				require.Nil(t, overflow)
			default:
				require.FailNow(t, "unhandled encoding type")
			}
		}
				...
				t.Run(tc.name, func(t *testing.T) {
					...
					for i, s := range tc.expectedSamples {
						...
						case chunkenc.ValFloat:
							require.True(t, s.isFloat)
							_, v := iter.At()
							require.Equal(t, float64(s.v), v)
