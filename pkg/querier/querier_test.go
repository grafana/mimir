...
func TestQuerier_QueryIngestersVsQueryIngestersClamped(t *testing.T) {
	now := time.Now()
	testCases := []struct {
		name                  string
		minT, maxT            time.Time
		queryIngestersWithin  time.Duration
		expectedQueryIngester bool
		expectedClampedMinT   time.Time
	}{
		{
			name:                  "query max equal to query-ingesters-within",
			minT:                  now.Add(-6 * time.Hour),
			maxT:                  now.Add(-1 * time.Hour),
			queryIngestersWithin:  1 * time.Hour,
			expectedQueryIngester: false,
			expectedClampedMinT:   now.Add(-1 * time.Hour),
		},
		{
			name:                  "query max after query-ingesters-within",
			minT:                  now.Add(-6 * time.Hour),
			maxT:                  now.Add(-30 * time.Minute),
			queryIngestersWithin:  1 * time.Hour,
			expectedQueryIngester: true,
			expectedClampedMinT:   now.Add(-1 * time.Hour),
		},
		{
			name:                  "query max before query-ingesters-within",
			minT:                  now.Add(-6 * time.Hour),
		...
		{
			name:                  "query min equal to query-ingesters-within",
			minT:                  now.Add(-6 * time.Hour),
			maxT:                  now.Add(-1 * time.Hour),
			queryIngestersWithin:  6 * time.Hour,
			expectedQueryIngester: true,
			expectedClampedMinT:   now.Add(-6 * time.Hour),
		},
		{
			name:                  "query min after query-ingesters-within",
			minT:                  now.Add(-5 * time.Hour),
			maxT:                  now.Add(-1 * time.Hour),
			queryIngestersWithin:  6 * time.Hour,
			expectedQueryIngester: true,
			expectedClampedMinT:   now.Add(-5 * time.Hour),
		},
		{
			name:                  "query min before query-ingesters-within",
			minT:                  now.Add(-6 * time.Hour),
			maxT:                  now.Add(-2 * time.Hour),
			queryIngestersWithin:  7 * time.Hour,
			expectedQueryIngester: true,
			expectedClampedMinT:   now.Add(-6 * time.Hour),
		},
	}
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			queryIngestersResult := QueryIngesters(
				testCase.queryIngestersWithin, now, testCase.maxT.UnixMilli(),
			)
			...
			assert.Equal(t, testCase.expectedQueryIngester, queryIngestersResult)
			assert.Equal(t, testCase.expectedQueryIngester, queryIngestersClampedResult)

			// check clamping is accurate
			assert.GreaterOrEqual(t, clampedMinT, testCase.minT.UnixMilli())
			assert.Equal(t, testCase.expectedClampedMinT.UnixMilli(), clampedMinT)
		})
	}
}
