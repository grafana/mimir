...
// Provenance-includes-location: https://github.com/grafana/backend-enterprise/pkg/enterprise/mimir/labelaccess/middleware.go
...
// NewExtractor returns a propagation.Extractor that extracts LBAC policies from a request carrier
// into the context. Used for the protobuf-based remote execution path.
...
func (l *labelAccessExtractor) ExtractFromCarrier(ctx context.Context, carrier propagation.Carrier) (context.Context, error) {
	...
	// Unlike labelAccessMiddleware above, this extractor does not need to check for unsupported endpoints
	// as it's only used for range queries and instant queries at the time of writing.
