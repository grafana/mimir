...
// Provenance-includes-location: https://github.com/grafana/backend-enterprise/pkg/enterprise/mimir/labelaccess/queryable.go
...
func (l *labelAccessQuerier) mergeMatchers(matchers []*labels.Matcher, lbacMatchers []*labels.Matcher) []*labels.Matcher {
	...
	for _, m := range matchers {
		unique[hashableMatcher{Type: m.Type, Name: m.Name, Value: m.Value}] = struct{}{}
	}
	for _, lbac := range lbacMatchers {
		if _, ok := unique[hashableMatcher{Type: lbac.Type, Name: lbac.Name, Value: lbac.Value}]; !ok {
...
// selectors extracts the LBAC policy selectors for the current tenant from ctx.
