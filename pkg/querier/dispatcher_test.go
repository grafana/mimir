	...
	testCases := map[string]struct {
		...
		"query that returns a range vector": {
			req: createQueryRequestForSpecificNodes(
				...
				1,
				[]string{"FunctionCall: max_over_time(...)", "Subquery: [11s:10s]"}, // Evaluate the subquery expression
			),
			expectedResponseMessages: []*frontendv2pb.QueryResultStreamRequest{
				{
					Data: &frontendv2pb.QueryResultStreamRequest_EvaluateQueryResponse{
		...
		//	// TODO
		//},
		//TODO: request with multiple instant vector operators and batch size larger than 1
