...
func TestSilenceLimits(t *testing.T) {
	...
	require.NoError(t, am.silences.Set(context.Background(), sil1))
	...
	require.EqualError(t, am.silences.Set(context.Background(), sil2), "exceeded maximum number of silences: 1 (limit: 1)")

	// Expire sil1 and run the GC. This should allow sil2 to be inserted.
	require.NoError(t, am.silences.Expire(context.Background(), sil1.Id))
	n, err := am.silences.GC()
	require.NoError(t, err)
	require.Equal(t, 1, n)
	require.NoError(t, am.silences.Set(context.Background(), sil2))

	// Expire sil2 and run the GC.
	require.NoError(t, am.silences.Expire(context.Background(), sil2.Id))
	n, err = am.silences.GC()
	require.NoError(t, err)
	require.Equal(t, 1, n)
	...
	require.EqualError(t, am.silences.Set(context.Background(), sil3), fmt.Sprintf("silence exceeded maximum size: %d bytes (limit: 4096 bytes)", toMeshSilence(t, sil3, 0).Size()))
	...
	require.NoError(t, am.silences.Set(context.Background(), sil4))

	// Should be able to update sil4 without modifications. It is expected to
	// keep the same ID.
	sil5 := cloneSilence(t, sil4)
	require.NoError(t, am.silences.Set(context.Background(), sil5))
	...
	sil6.Comment = "m"
	require.NoError(t, am.silences.Set(context.Background(), sil6))
	...
	require.EqualError(t, am.silences.Set(context.Background(), sil7), "exceeded maximum number of silences: 1 (limit: 1)")

	// sil6 should not be expired because the update failed.
	sils, _, err := am.silences.Query(context.Background(), silence.QState(types.SilenceStateExpired))
	require.NoError(t, err)
	...
	require.EqualError(t, am.silences.Set(context.Background(), sil8), fmt.Sprintf("silence exceeded maximum size: %d bytes (limit: 4096 bytes)", toMeshSilence(t, sil8, 0).Size()))
	...
	require.EqualError(t, am.silences.Set(context.Background(), sil9), fmt.Sprintf("silence exceeded maximum size: %d bytes (limit: 4096 bytes)", toMeshSilence(t, sil9, 0).Size()))
