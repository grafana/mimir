...
func runGenerate() {
	...
	samplesPerSeries := fs.Int("samples-per-series", 1000, "Number of samples per series")
	...
	config, err := parseGenerateConfig(*userID, *seriesCount, *samplesPerSeries, *metricNames, *labelNames, *labelCardinality, *timeRangeHours, *scrapeInterval, *outputPrefix)
	if err != nil {
...
func parseGenerateConfig(userID string, seriesCount, samplesPerSeries int, metricNames, labelNames, labelCardinality string, timeRangeHours int, scrapeInterval time.Duration, outputPrefix string) (*DatasetConfig, error) {
