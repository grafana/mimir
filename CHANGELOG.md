...
* [BUGFIX] Block-builder: Fix silent data loss for jobs spanning exactly one record: the consumption loop exited before the first fetch, uploaded no blocks, and still reported the job to the scheduler as completed, permanently skipping the record. Single-record jobs occur when job planning strands one record of an older time bucket past the last job boundary, e.g. across a scheduler restart. #XXXXX
