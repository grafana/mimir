...
* [CHANGE] Querier/Ruler: `-querier.active-query-tracker-dir` option has been removed. Query tracking is now done via Activity tracker configured by `-activity-tracker.filepath`. Limit for max number of concurrent queries (`-querier.max-concurrent`) is now respected even if activity tracking is not enabled.
...
* [ENHANCEMENT] Compactor: expose low-level concurrency options for compactor: `-compactor.max-opening-blocks-concurrency`, `-compactor.max-closing-blocks-concurrency`, `-compactor.symbols-flushers-concurrency`, used when using `split-and-merge` compaction strategy. #569 #701
