...
* [ENHANCEMENT] Usage-tracker: Make the per-tenant shard count configurable via `-usage-tracker.num-shards` (default 16, range 1-256, previously a fixed compile-time constant). Snapshots now encode the shard count they were written with; a usage-tracker that reads a snapshot written with a different shard count (or an older encoding version) discards it with a warning instead of mis-loading or failing to start, and rebuilds the state from events. All instances must use the same value, and changing it discards existing snapshots. #15888
...
* [ENHANCEMENT] Usage-tracker: Make the per-tenant shard count configurable via `-usage-tracker.num-shards` (default 16, range 1-256, previously a fixed compile-time constant). A higher count lowers lock contention and cleanup-induced tail latency on partitions with very large tenants (>~100M series per partition), at the cost of fixed per-tenant overhead that is wasteful for many small tenants. Snapshots now encode the shard count they were written with; changing the count is not graceful: a usage-tracker discards (with a warning) any snapshot written with a different shard count or an older encoding version instead of mis-loading or failing to start, and rebuilds the state from events. All instances must use the same value. #15888
