...
* [BUGFIX] Ingest storage: Log a warning when the partition reader starts with `-ingest-storage.kafka.client-rack` set while `-ingest-storage.kafka.fetch-concurrency-max` is greater than 0, and document the limitation on the flag. Concurrent fetching builds its own Fetch requests, sends no rack and always reads from the partition leader, so the rack is ignored and rack-aware consumption never engages. Behaviour is unchanged, the combination is now no longer silent. #16605
