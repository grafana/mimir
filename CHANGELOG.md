...
### Grafana Mimir
* [ENHANCEMENT] Ingest storage: Allow MSK IAM SASL authentication to fall back to the AWS SDK default credential chain when no static credentials, file path, or HTTP socket path is configured. A new `-ingest-storage.kafka.sasl-msk-iam-region` flag specifies the AWS region for the fallback. #15848
