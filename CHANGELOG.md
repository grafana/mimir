...
* [CHANGE] Ingester: `-ingester.use-ingester-owned-series-for-limits`, `-ingester.track-ingester-owned-series`, and `-ingester.owned-series-update-interval` are no longer experimental and are now enabled by default. In classic architecture, owned series tracking requires zone-aware replication to be enabled; if running without it, set `-ingester.track-ingester-owned-series=false` and `-ingester.use-ingester-owned-series-for-limits=false`. #15305
