...
* [CHANGE] Store-gateway: label names, label values and their search variants now map errors to gRPC status codes the same way series requests do. An error that already carries a status code keeps it, instead of being reported as `Internal`.
...
* [FEATURE] Store-gateway: add the experimental `-blocks-storage.bucket-store.max-concurrent-blocks` option to limit how many blocks are queried concurrently across all tenants and all in-flight requests, bounding the fan-out a single request can cause. `-blocks-storage.bucket-store.max-concurrent-blocks-queue-timeout` sets how long a block waits for its turn. Disabled by default.
* [FEATURE] Store-gateway: add the experimental `-blocks-storage.bucket-store.gate-label-requests` option to subject label names, label values and their search variants to `-blocks-storage.bucket-store.max-concurrent`, as series requests already are. Disabled by default.
