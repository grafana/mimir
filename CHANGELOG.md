...
* [FEATURE] Querier: experimental streaming label/value search HTTP endpoints `/api/v1/search/{metric_names,label_names,label_values}` (NDJSON responses with per-batch flush), mirroring the Prometheus search API ([prometheus#18573](https://github.com/prometheus/prometheus/pull/18573)). Gated by `-querier.experimental-search-api-enabled` (default false). Supports `match[]`, `search[]`, `fuzz_threshold`, `fuzz_alg`, `case_sensitive`, `sort_by`, `sort_dir`, `include_score`, `start`, `end`, `limit`, `batch_size`, and `name` (for label-values). #15349
