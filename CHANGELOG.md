...
* [ENHANCEMENT] Continuous test: When a range query returns a native histogram that doesn't match the expected one, the logged error now states which fields diverged (count, sum, number of buckets, and/or the specific bucket with its boundaries) and prints values with full float precision. Previously the error always reported a sum mismatch, printed with 6 decimals, so a count or bucket divergence was logged as a sum mismatch between two identical-looking numbers. #16683
