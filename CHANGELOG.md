...
* [ENHANCEMENT] Query-frontend: Allocate a unique ID for incoming queries which is passed through to all sub-requests. This `parent_query_id` will be included in related query-frontend, query-scheduler and querier logs, and in the `Server-Timing` response header when the caller asks for query stats. The ID is a random UUID, so it is globally unique. #16593
