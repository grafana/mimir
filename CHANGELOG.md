...
* [FEATURE] Ingester, Block-builder: Add per-tenant `-ingester.float-chunk-encoding` flag to select the float chunk encoding (`xor` or `xor2`) per tenant. The overrides-exporter can export it as a numeric value (`4` for `xor`, `7` for `xor2`) when `float_chunk_encoding` is added to `-overrides-exporter.enabled-metrics`. #15831
