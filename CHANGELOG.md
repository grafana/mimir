...
* [ENHANCEMENT] Distributor: Add experimental `-distributor.otel-log-target-info-label-name-collisions` option to log, at debug level, OTLP resource attributes that collide after label name sanitization and whose values may get concatenated with `;` in `target_info` labels. #15653
