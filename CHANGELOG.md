...
* [CHANGE] Deprecated the object storage consistency delay support. The config parameters `-blocks-storage.bucket-store.consistency-delay` and `-compactor.consistency-delay` are now ignored and will be removed in Mimir 2.9. The metrics `cortex_compactor_meta_sync_consistency_delay_seconds` and `cortex_blocks_meta_sync_consistency_delay_seconds` have been removed. #4366
