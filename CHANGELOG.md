...
* [ENHANCEMENT] Ruler: Add `-ruler.max-global-rule-evaluation-concurrency` to allow independent rules of a tenant to be run concurrently. The amount of concurrency per tenant is controlled via `-ruler.max-concurrent-rule-evaluations-per-tenant` as a limit. A `-ruler.max-global-rule-evaluation-concurrency` value of `0` can be used to disable the feature for all tenants. By default, the new behaviour is disabled. #8146
