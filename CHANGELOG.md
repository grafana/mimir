...
* [CHANGE] Querier, query-frontend, ruler: The experimental PromQL duration arithmetic helpers `min(...)` and `max(...)` used inside `[...]` range/subquery brackets and `offset` clauses have been renamed to `min_of(...)` and `max_of(...)` to match the upstream Prometheus rename. Queries using the old names now fail at parse time with a syntax error; rewrite them to use the new names. The standalone `min` / `max` PromQL aggregation operators are unaffected. #15593 #15603
