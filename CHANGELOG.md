...
* [CHANGE] Dashboards: Stop collapsing the scheduler-mode rows of the "Compactor" and "Compactor resources" dashboards by default. #16239
...
* [ENHANCEMENT] Add the `compactor_standalone_enabled` config option (enabled by default), gating the compactor dashboard panels and alerts that only apply to compactors running in standalone mode. Disable it once all compactors run in scheduler mode. At least one of `compactor_standalone_enabled` and `compactor_scheduler_enabled` must be enabled. #16239
