...
* [ENHANCEMENT] Distributor, Querier: add cross-server fan-out for the streaming label/value search RPCs introduced in #15233. `Distributor.SearchLabelNames` and `SearchLabelValues` fan out to the ingester replication set; `blocksStoreQuerier` and `distributorQuerier` gain querier-side adapters. Reachable but unused — the user-visible HTTP routes arrive in a follow-up PR behind a feature flag. #15301
