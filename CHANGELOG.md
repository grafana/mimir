...
* [ENHANCEMENT] MQE: Add an extension point for registering extra AST and query-plan optimization passes on the query planner, allowing downstream builds to run query-mutating logic as MQE optimization passes so their effect is visible via the query analysis endpoint. Request headers allow-listed with `-query-frontend.extra-propagated-headers` are now made available to optimization passes at query-planning time. #16248
