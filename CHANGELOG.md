...
* [ENHANCEMENT] Store-gateway: Add `-blocks-storage.bucket-store.max-concurrent-blocks-per-request` to limit how many blocks a single Series/LabelNames/LabelValues request resolves matchers for concurrently. This bounds the peak memory of matcher resolution for queries that span many blocks (for example long time ranges). Disabled (`0`) by default. #15645
