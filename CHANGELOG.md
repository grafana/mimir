...
* [ENHANCEMENT] Distributor: added support for reducing the resolution of native histogram samples if the sample has too many buckets compared to `-validation.max-native-histogram-buckets`. This is enabled by default and can be turned off by setting `-validation.downscale-native-histogram-over-max-buckets` to `false`. #6535
