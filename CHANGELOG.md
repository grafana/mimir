...
* [BUGFIX] Recording rules: Use a 5m rate window instead of 1m in the `mimir_api_*` request duration recording rules (such as `cluster_job_route:cortex_request_duration_seconds:sum_rate`), so they keep returning data with 1m scrape intervals. #16178
