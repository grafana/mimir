...
* [CHANGE] Store-gateway: remove fine-grained chunks caching. The following configuration paramters have been removed `-blocks-storage.bucket-store.chunks-cache.fine-grained-chunks-caching-enabled`, `-blocks-storage.bucket-store.fine-grained-chunks-caching-ranges-per-series`.
