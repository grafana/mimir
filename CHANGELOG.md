...
* [CHANGE] Store-gateway: use mmap-less index-header reader by default and remove mmap-based index header reader. The experimental `-blocks-storage.bucket-store.index-header.stream-reader-enabled` flag is no longer required and has been removed. #4280
