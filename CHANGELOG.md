...
* [ENHANCEMENT] Query-frontend: Allocate a unique ID for incoming queries which is passed through to all sub-requests. This `parent_query_id` will be included in related query-frontend, query-scheduler and querier logs. The ID is unique within a single query-frontend process only. #16593
