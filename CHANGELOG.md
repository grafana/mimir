...
* [ENHANCEMENT] Ruler: When remote rule evaluation is enabled (`-ruler.query-frontend.address` is set), the ruler no longer builds the store-gateway ring client and blocks-store queryable, nor validates or connects to blocks object storage at startup, since it delegates queries to the ruler-query-frontend. #15716
