...
* [ENHANCEMENT] Query-frontend: Add experimental `-query-frontend.retry-after-header.enabled` option to include a `Retry-After` header on HTTP 429 and 503 responses, reusing the distributor's existing backoff mechanism. Disabled by default. Backoff bounds are configurable with `-query-frontend.retry-after-header.min-backoff` and `-query-frontend.retry-after-header.max-backoff`. #16503
