...
* [ENHANCEMENT] Query-frontend: Add experimental `subquery_spin_off_disabled_queries` per-tenant limit to disable subquery spin-off for queries matching the configured PromQL patterns (literal or regex). This keeps subquery spin-off enabled for the tenant while excluding specific problematic queries, which are executed without spinning off subqueries. The new `cortex_frontend_subquery_spinoff_disabled_total` counter tracks how often this happens. #15807
