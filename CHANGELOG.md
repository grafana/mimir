...
* [FEATURE] Distributor: Add experimental support for distributed partition series limiting using HyperLogLog. Enable with `-distributor.partition-series-tracker.enabled`. Limits are configured per-tenant via `max_series_per_partition` in runtime config. #13172
