...
* [FEATURE] MQE: Add experimental support for running splitting, caching and spinning off subqueries from instant queries inside MQE. Enabled with `-query-frontend.use-mimir-query-engine-for-splitting-and-caching-results=true`. #15348 #15393 #15397 #15650 #15720 #15750 #15769 #15783 #15787 #15795 #15804 #15822 #15823 #15846 #15827 #15884 #15887 #15912 #15928 #15930 #15954 #15955 #15978 #15999
...
* [FEATURE] Query-frontend, Querier: Add experimental label presence cardinality endpoint `<prometheus-http-prefix>/api/v1/cardinality/label_presence`. Given a `selector` and a list of `label[]` names, it reports, for the matching active series, the total series count, how many carry every expected label, per-label missing counts, and up to `limit` example non-compliant series. Gated by the same `cardinality analysis` tenant setting as the other cardinality endpoints. #15965
* [FEATURE] Query-frontend, Querier: Add experimental label presence cardinality endpoint `<prometheus-http-prefix>/api/v1/cardinality/label_presence`. Given a `selector` and a list of `label[]` names, it reports, for the matching active series, the total series count, how many carry every expected label, per-label missing counts, and up to `limit` example non-compliant series. The query-frontend shards the request across ingesters and caches the result like the other cardinality endpoints. Gated by the same `cardinality analysis` tenant setting as the other cardinality endpoints. #15965
