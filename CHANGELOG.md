...
* [BUGFIX] Ingest storage: Fix `panic: duplicate metrics collector registration attempted` on startup when ingest storage is enabled in a single process that runs both the ingester and query-frontend (e.g. monolithic `-target=all`). The partition offset client metrics now include a `component` label to disambiguate the multiple instances sharing the registry. #15744
