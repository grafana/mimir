...
* [BUGFIX] Ingest storage: Log a warning at startup, and document the limitation on the flag, when `-ingest-storage.kafka.client-rack` is set while `-ingest-storage.kafka.fetch-concurrency-max` is greater than 0. Concurrent fetching builds its own Fetch requests, sends no rack and always reads from the partition leader, so the rack is ignored on the read path and rack-aware consumption never engages. Behaviour is unchanged, the combination is now no longer silent. #PLACEHOLDER
