...
* [ENHANCEMENT] Ruler: Add `-ruler.max-global-rule-evaluation-concurrency` to allow independent rules of a tenant to be run concurrently. The amount of concurrency per tenant is controlled via `-ruler.max-concurrent-rule-evaluations-per-tenant` as a limit. A `-ruler.max-global-rule-evaluation-concurrency` value of `0` can be used to disable the feature for all tenants. By default, the new behaviour is disabled. A rule is eligible for concurrency as long as it has no rules it depends on nor any rules that depend on it, and its total rule group runtime exceeds 50% of its interval. #8146
  * As part of this work the following metrics have been introduced:
    * `cortex_ruler_global_rule_evaluation_concurrency_current`
    * `cortex_ruler_global_rule_evaluation_concurrency_acquire_total`
    * `cortex_ruler_global_rule_evaluation_concurrency_acquire_failed_total`
    * `cortex_ruler_global_rule_evaluation_concurrency_release_total`
