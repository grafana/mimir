...
* [ENHANCEMENT] Distributor: OTLP endpoint now returns partial success (HTTP 200 with `ExportMetricsPartialSuccess`) instead of HTTP 400 when only some samples are rejected by the `past_grace_period` validation but other samples in the same request are successfully ingested. The `RejectedDataPoints` field reports the number of dropped data points. Non-OTLP write paths (Prometheus remote write, gRPC Push, Influx) still return HTTP 400 / `InvalidArgument` for the same rejections. #15248
