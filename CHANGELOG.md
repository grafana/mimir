...
* [CHANGE] The config field `distributorConfig` has been renamed to `ingesterRingClientConfig`. Config field `ringClient` has been removed in favor of `ingesterRingClientConfig`. #997 #XXX
* [CHANGE] Gossip.libsonnet has been fixed to modify all ring configurations, not only ingester ring. Furthermore it now support migration via multi KV store. #XXX
