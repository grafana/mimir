...
* [CHANGE] Query-scheduler: extracted the tenant-fair request queue from `pkg/scheduler/queue` to `pkg/queue` so it can be reused outside the scheduler. The scheduler now wraps the queue with a thin internal type that owns dimension extraction and per-tenant max-queriers lookup. No user-facing behavior or configuration changes. #15443
* [CHANGE] Ingester: replaced `-ingester.label-values-count-max-concurrency` with a shared tenant-fair worker pool. New flags: `-ingester.label-values-count-workers` (default 0 = GOMAXPROCS) and `-ingester.label-values-count-chunk-size` (default 32). Work for a single label-values-cardinality request is now split into chunks dispatched across a process-wide pool, so two tenants computing cardinality at the same time interleave fairly instead of one starving the other. #15440
