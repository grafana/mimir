...
* [ENHANCEMENT] Store-gateway: new indexed+diff+varint cached postings encoding that uses less CPU on read path. Can be enabled using `-blocks-storage.bucket-store.indexed-diff-varint-postings-cache-encoding-enabled` flag. #370
