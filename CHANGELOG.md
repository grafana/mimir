...
* [FEATURE] Query-frontend: Add the experimental `cortex_query_frontend_max_inflight_http_requests` and `cortex_query_frontend_max_inflight_http_request_age_seconds` metrics, reporting the per-tenant peak number of concurrent in-flight HTTP requests and the greatest age an in-flight request reached since the last scrape. Both reset on each scrape. Enable with `-query-frontend.max-inflight-http-metrics-enabled=true`. #16575
* [FEATURE] Query-frontend: Add the experimental `cortex_query_frontend_max_inflight_dispatched_queries` and `cortex_query_frontend_max_inflight_dispatched_query_age_seconds` metrics, reporting the per-tenant peak number of concurrent queries the frontend had dispatched to query-schedulers and the greatest age a dispatched query reached since the last scrape. Both reset on each scrape. Enable with `-query-frontend.max-inflight-dispatched-metrics-enabled=true`. #16575
