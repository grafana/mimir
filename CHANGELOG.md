...
* [FEATURE] Query-tee: Add per-backend `exclude_tenants` option to the `-backend.config-file` configuration. Requests whose `X-Scope-OrgID` tenants are all excluded are not sent to that backend, while the preferred backend always receives all traffic.
