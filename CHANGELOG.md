...
* [CHANGE] Ingester: Renamed the gRPC protobuf package from `cortex` to `ingesterpb`. gRPC method paths (as seen in metrics labels such as `route` and `operation`) have changed from `/cortex.Ingester/...` to `/ingesterpb.Ingester/...`. #15059
