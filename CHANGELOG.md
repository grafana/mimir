...
* [CHANGE] Memcache connections parameters `blocks-storage.bucket-store.index-cache.memcached.max-idle-connections`, `blocks-storage.bucket-store.chunks-cache.memcached.max-idle-connections` and `blocks-storage.bucket-store.metadata-cache.memcached.max-idle-connections` settings are now configured based on `max-get-multi-concurrency` and `max-async-concurrency`. 
