...
* [ENHANCEMENT] Distributor, Ingester, Querier: Replace the gogoproto/gogoslick protobuf code generator with wiresmith for all of Mimir's own protobuf messages. Wire format and public APIs are unchanged. On the write path, marshalling is ~6-10% faster and unmarshalling allocates up to ~47% fewer bytes; on the query path, the streamingpromql query-plan codec is ~7% faster with ~5-6% fewer allocations. #15632
