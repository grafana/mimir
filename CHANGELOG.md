...
* [CHANGE] Query-frontend: Remove the CLI flags `-querier.max-outstanding-requests-per-tenant` and `-query-frontend.querier-forget-delay` and corresponding YAML configuration. This is part of a change that makes the query-scheduler a required component. A result of this removes the ability to run the query-frontend with an embedded query-scheduler. Instead, a dedicated query-scheduler component must be run. #12200
