...
* [ENHANCEMENT] All: Add support for cluster validation in gRPC calls. When it is enabled, the corresponding gRPC server verifies if a request coming from a gRPC client comes from an expected cluster. This validation can be configured by the following experimental configuration options: #10767
...
* [ENHANCEMENT] Ingester client: Add support to configure cluster validation for ingester clients. Ingester clients propagate a non-empty cluster validation label configured via `-server.cluster-validation.label` in the component that creates the former (distributor, ruler or querier).  Failed cluster validations are tracked by `cortex_client_request_invalid_cluster_validation_labels_total` with label `client=ingester`. #10767
