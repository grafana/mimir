...
* [FEATURE] Block-builder: Add the experimental `-block-builder.verify-blocks-before-upload` flag to verify each block's index before uploading it to object storage. Chunk data isn't checked. If verification fails, the block isn't uploaded and the job fails so that it's retried. Verification time and failures are exported as `cortex_blockbuilder_tsdb_block_verification_duration_seconds` and `cortex_blockbuilder_tsdb_block_verification_failed_total`. Disabled by default. #16480
