...
* [ENHANCEMENT] Ruler: Support HTTP Basic authentication for remote rule evaluation over HTTP(S) via `-ruler.query-frontend.http-client-config.basic-auth-username` and `-ruler.query-frontend.http-client-config.basic-auth-password`. Credentials set in the `-ruler.query-frontend.address` URL userinfo were previously ignored because the remote querier calls the transport directly instead of through `http.Client`. #16000
