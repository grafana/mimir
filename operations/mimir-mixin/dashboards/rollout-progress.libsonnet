...
(import 'dashboard-utils.libsonnet') {
  local config = {
    ...
    group_prefix_clusters: $._config.group_prefix_clusters,
  ...
  'mimir-rollout-progress.json':
    ...
    .addClusterSelectorTemplates(false) + {
      ...
      panels: [
        ...
        $.newStatPanel(|||
          histogram_quantile(0.99, sum by (le) (%(group_prefix_clusters)s,_route:cortex_request_duration_seconds_bucket:sum_rate{%(gateway_job_matcher)s, route=~"%(gateway_write_routes_regex)s"}))
        ...
        $.newStatPanel(|||
          histogram_quantile(0.99, sum by (le) (%(group_prefix_clusters)s_route:cortex_request_duration_seconds_bucket:sum_rate{%(gateway_job_matcher)s, route=~"%(gateway_read_routes_regex)s"}))
        ||| % config, unit='s', thresholds=[
          { color: 'green', value: null },
        ...
        $.queryPanel([|||
          1 - (
            avg_over_time(histogram_quantile(0.99, sum by (le) (%(group_prefix_clusters)s_route:cortex_request_duration_seconds_bucket:sum_rate{%(gateway_job_matcher)s, route=~"%(gateway_write_routes_regex)s"} offset 24h))[1h:])
            /
            avg_over_time(histogram_quantile(0.99, sum by (le) (%(group_prefix_clusters)s_route:cortex_request_duration_seconds_bucket:sum_rate{%(gateway_job_matcher)s, route=~"%(gateway_write_routes_regex)s"}))[1h:])
          )
        ||| % config, |||
          1 - (
            avg_over_time(histogram_quantile(0.99, sum by (le) (%(group_prefix_clusters)s_route:cortex_request_duration_seconds_bucket:sum_rate{%(gateway_job_matcher)s, route=~"%(gateway_read_routes_regex)s"} offset 24h))[1h:])
            /
            avg_over_time(histogram_quantile(0.99, sum by (le) (%(group_prefix_clusters)s_route:cortex_request_duration_seconds_bucket:sum_rate{%(gateway_job_matcher)s, route=~"%(gateway_read_routes_regex)s"}))[1h:])
          )
        ||| % config], ['writes', 'reads']) + {
          yaxes: $.yaxes({
            format: 'percentunit',
