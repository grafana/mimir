          ...
          targets: [
            {
              ...
              expr: '{%s=~"$cluster",namespace=~"$namespace",name=~"query-frontend.*"} |= "query stats" != "/api/v1/read" | logfmt | org_id=~"${tenant_id}" | response_time > ${min_duration}' % $._config.clusterLabel,
              instant: false,
              legendFormat: '',
              range: true,
              refId: 'A',
