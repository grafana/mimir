  ...
  'mimir-ruler.json':
    ...
    .addRow(
      ...
      .addPanel(
        ...
        $.queryPanel(
          'histogram_quantile(0.99, sum by (route, le) (%s_route:cortex_request_duration_seconds_bucket:sum_rate{%s, route=~"%s"}))' % [$._config.group_prefix_clusters, $.jobMatcher($._config.job_names.gateway), ruler_config_api_routes_re],
          '{{ route }}'
        ) +
        { yaxes: $.yaxes('s') }
      )
