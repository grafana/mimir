...
(import 'dashboard-queries.libsonnet') {
  [filename]:
    ...
    .addRowIf(
      ...
      $.row('Gateway')
      .addPanel(
        $.qpsPanelNativeHistogram('Requests / sec', $.queries.gateway.writeRequestsPerSecondMetric, $.queries.gateway.writeRequestsPerSecondSelector)
      )
      .addPanel(
        $.latencyPanelNativeHistogram('Latency', $.queries.gateway.writeRequestsPerSecondMetric, $.queries.gateway.writeRequestsPerSecondSelector)
      )
      .addPanel(
    ...
    .addRow(
      ...
      .addPanel(
        ...
        $.qpsPanelNativeHistogram('Requests / sec', $.queries.distributor.writeRequestsPerSecondMetric, $.queries.distributor.writeRequestsPerSecondSelector) +
      ...
      .addPanel(
        $.latencyPanelNativeHistogram('Latency', $.queries.distributor.writeRequestsPerSecondMetric, $.queries.distributor.writeRequestsPerSecondSelector)
      )
      .addPanel(
        $.timeseriesPanel('Per %s p99 latency' % $._config.per_instance_label) +
        $.hiddenLegendQueryPanel(
    ...
    .addRow(
      ...
      .addPanel(
        ...
        $.qpsPanelNativeHistogram('Requests / sec', 'cortex_request_duration_seconds', '%s,route="/cortex.Ingester/Push"' % $.jobMatcher($._config.job_names.ingester)) +
        if $._config.show_rejected_requests_on_writes_dashboard then {
          targets: [
            {
              legendLink: null,
      ...
      .addPanel(
        $.latencyPanelNativeHistogram('Latency', 'cortex_request_duration_seconds', '%s,route="/cortex.Ingester/Push"' % $.jobMatcher($._config.job_names.ingester))
      )
      .addPanel(
        $.timeseriesPanel('Per %s p99 latency' % $._config.per_instance_label) +
        $.hiddenLegendQueryPanel(
