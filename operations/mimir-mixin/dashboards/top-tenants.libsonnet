        ...
        $.tablePanel(
          [
            |||
              topk($limit, %s)
            ||| % [
              $.queries.ingester.ingestOrClassicDeduplicatedQuery('cortex_ingester_active_series{%s}' % [$.jobContainerMatchers($._config.job_names.ingester, $._config.container_names.ingester)], groupByLabels='user'),
            ],
          ], {
      ...
      .addPanel(
        ...
        $.tablePanel(
          [
            ...
            % { job: $.jobContainerMatchers($._config.job_names.ingester + $._config.job_names.distributor, $._config.container_names.ingester) },
          ], {
            user: { alias: 'user', unit: 'string' },
            Value: { alias: 'samples/s' },
          }
        ...
        $.tablePanel(
          [
            |||
              topk($limit, %s)
            ||| % [
              $.queries.ingester.ingestOrClassicDeduplicatedQuery('cortex_ingester_tsdb_exemplar_series_with_exemplars_in_storage{%s}' % [$.jobContainerMatchers($._config.job_names.ingester, $._config.container_names.ingester)], groupByLabels='user'),
            ],
          ], {
            user: { alias: 'user', unit: 'string' },
            Value: { alias: 'series' },
