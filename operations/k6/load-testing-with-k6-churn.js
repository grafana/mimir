...
/**
 * This is a hacked up version of the k6 script that sends a different series in every request.
 *
 * k6 run load-testing-with-k6-churn.js \
 *  -e K6_WRITE_HOSTNAME="localhost:8080" \
 *  -e K6_READ_HOSTNAME="localhost:8080" \
 *  -e K6_TENANT_ID="anonymous" \
 *  -e K6_USERNAME="anonymous" \
 *  -e K6_WRITE_REQUEST_RATE="1" \
 *  -e K6_WRITE_SERIES_PER_REQUEST="1" \
 *  -e K6_READ_REQUEST_RATE="1" \
 *  -e K6_SCRAPE_INTERVAL_SECONDS="1" \
 *  -e K6_HA_REPLICAS="1" \
 *  -e K6_DURATION_MIN="240"
 *
 * The above will churn through roughly 14 series per second, for ~100k in 2 hours (a compaction period)
 */
 ...
 * @constant {number}
 */
const TOTAL_SERIES = WRITE_REQUEST_RATE * WRITE_SERIES_PER_REQUEST * 14;
/**
...
export function write() {
    try {
        ...
        const min_series_id = (iteration*WRITE_SERIES_PER_REQUEST) + ((iteration % (TOTAL_SERIES / WRITE_SERIES_PER_REQUEST)) * WRITE_SERIES_PER_REQUEST);
        const max_series_id = (iteration*WRITE_SERIES_PER_REQUEST) + (min_series_id + WRITE_SERIES_PER_REQUEST);
