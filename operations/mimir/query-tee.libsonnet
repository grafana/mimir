{
  ...
  query_tee_container:: if $._config.query_tee_enabled then
  ...
  newQueryTeeDeployment(name, container, nodeAffinityMatchers=[])::
    deployment.new(name, 2, [container]) +
    $.newMimirNodeAffinityMatchers(nodeAffinityMatchers) +
    (if !std.isObject($._config.node_selector) then {} else deployment.mixin.spec.template.spec.withNodeSelectorMixin($._config.node_selector)),

  query_tee_deployment: if $._config.query_tee_enabled then
    $.newQueryTeeDeployment('query-tee', $.query_tee_container),

  query_tee_service: if $._config.query_tee_enabled then
