{
  _config+:: {
    ...
    query_scheduler_service_discovery_mode: if $._config.deployment_mode == 'read-write' then 'ring' else super.query_scheduler_service_discovery_mode,

    // Overrides-exporter is part of the backend component in the Mimir read-write deployment mode.
    overrides_exporter_enabled: if $._config.deployment_mode == 'read-write' then false else super.overrides_exporter_enabled,
  },

  // Mimir read-write deployment mode makes some strong assumptions about what must enabled enabled and disabled.
  check_compactor_max_concurrency: if $._config.deployment_mode != 'read-write' || $._config.compactor_max_concurrency == 1 then null else
    error 'please set compactor_max_concurrency to 1 when using Mimir read-write deployment mode',

  check_ingester_multi_zone: if $._config.deployment_mode != 'read-write' || $._config.multi_zone_ingester_enabled then null else
    error 'please set multi_zone_ingester_enabled to true when using Mimir read-write deployment mode',

  check_store_gateway_multi_zone: if $._config.deployment_mode != 'read-write' || $._config.multi_zone_store_gateway_enabled then null else
    error 'please set multi_zone_store_gateway_enabled to true when using Mimir read-write deployment mode',

  check_querier_autoscaling: if $._config.deployment_mode != 'read-write' || !$._config.autoscaling_querier_enabled then null else
    error 'please set autoscaling_querier_enabled to false when using Mimir read-write deployment mode',

  check_ruler_remote_evaluation_enabled: if $._config.deployment_mode != 'read-write' || !$._config.ruler_remote_evaluation_enabled then null else
    error 'please set ruler_remote_evaluation_enabled to false when using Mimir read-write deployment mode',

  check_overrides_exporter_enabled: if $._config.deployment_mode != 'read-write' || !$._config.overrides_exporter_enabled then null else
    error 'please set overrides_exporter_enabled to false when using Mimir read-write deployment mode',

  check_memberlist_ring: if $._config.deployment_mode != 'read-write' || $._config.memberlist_ring_enabled then null else
