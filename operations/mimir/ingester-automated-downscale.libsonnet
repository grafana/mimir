{
  ...
  _config+: {
    ...
    ingester_automated_downscale_ingester_zones: 3,
    ingester_automated_downscale_replicas_per_zone: std.ceil($._config.multi_zone_ingester_replicas / $._config.ingester_automated_downscale_ingester_zones),

    // How long to wait before terminating an ingester after it has been notified about the scale down.
    ingester_automated_downscale_delay: if 'querier.query-ingesters-within' in $.querier_args then
  ...
  local downscale_enabled_in_at_least_one_zone = $._config.ingester_automated_downscale_zone_a_enabled ||
                                                 $._config.ingester_automated_downscale_zone_b_enabled ||
                                                 $._config.ingester_automated_downscale_zone_c_enabled,

  // Validate the configuration.
  assert !$._config.ingester_automated_downscale_enabled || $._config.multi_zone_ingester_enabled : 'ingester downscaling requires multi_zone_ingester_enabled in namespace %s' % $._config.namespace,
  assert !$._config.ingester_automated_downscale_enabled || $._config.multi_zone_ingester_replicas > 0 : 'ingester downscaling requires multi_zone_ingester_replicas > 0 in namespace %s' % $._config.namespace,
  assert !$._config.ingester_automated_downscale_zone_b_enabled || $._config.ingester_automated_downscale_zone_a_enabled : 'ingester downscaling must first be enabled in zone-a in namespace %s' % $._config.namespace,
  assert !$._config.ingester_automated_downscale_zone_c_enabled || $._config.ingester_automated_downscale_zone_a_enabled : 'ingester downscaling must first be enabled in zone-a in namespace %s' % $._config.namespace,
  ...
  local updateIngesterZone(enabled, statefulsetName, primary=true) = overrideSuperIfExists(
    ...
    if !enabled then {} else (
      ...
      statefulSet.mixin.metadata.withAnnotationsMixin({
        ...
        'grafana.com/rollout-delayed-downscale': $._config.ingester_automated_downscale_delay,
  ...
  ingester_zone_a_replica_template:
    if !downscale_enabled_in_at_least_one_zone then null
    else $.replicaTemplate('ingester-zone-a', $._config.ingester_automated_downscale_replicas_per_zone, 'name=ingester-zone-a'),

  ingester_zone_a_statefulset: updateIngesterZone($._config.ingester_automated_downscale_zone_a_enabled, 'ingester_zone_a_statefulset', true),
  ingester_zone_b_statefulset: updateIngesterZone($._config.ingester_automated_downscale_zone_b_enabled, 'ingester_zone_b_statefulset', false),
  ingester_zone_c_statefulset: updateIngesterZone($._config.ingester_automated_downscale_zone_c_enabled, 'ingester_zone_c_statefulset', false),
}
