...
* [ENHANCEMENT] Gateway: add `gateway.lifecycle` to set lifecycle hooks on the gateway container. A `preStop` sleep keeps NGINX serving while the Pod's endpoint removal propagates, so requests routed to a terminating Pod are not refused and retried. #16680
