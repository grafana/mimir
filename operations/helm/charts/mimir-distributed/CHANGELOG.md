...
* [ENHANCEMENT] Add configurable readiness and liveness probes to the memcached cache StatefulSets (`chunks-cache`, `index-cache`, `metadata-cache`, `results-cache`) and the memcached-exporter sidecar, defaulting to a TCP probe on the memcached port and an HTTP probe on the exporter. This gives PodDisruptionBudgets accurate pod-health visibility during rollouts and node upgrades, and clears the GKE "StatefulSet with a PDB has no readiness probe" recommendation. Configurable via `memcached.readinessProbe`/`livenessProbe` and `memcachedExporter.readinessProbe`/`livenessProbe`. #15676
