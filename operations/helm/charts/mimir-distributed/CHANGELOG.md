...
* [FEATURE] Add an `ipv6-only` values preset for running the chart on an IPv6 only Kubernetes cluster. It sets `instance_enable_ipv6` on every ring, `memberlist.bind_addr` to `["::"]`, the HTTP and gRPC listen addresses to `::`, and the bundled Kafka listeners to `[::]`. No chart template changes are needed, so nothing changes for existing installations. #16606
...
* [BUGFIX] Ingester: document that the rack set by `ingester.zoneAwareReplication.autoIngestStorageClientRack` is ignored while `-ingest-storage.kafka.fetch-concurrency-max` is greater than 0, because concurrent fetching always reads from the partition leader. With the default fetch concurrency the flag on its own does not reduce cross-AZ read traffic. No rendered output changes. #16605
...
* [CHANGE] Upgrade Mimir to [3.2.0](https://github.com/grafana/mimir/blob/release-3.2/CHANGELOG.md). #15839
* [CHANGE] Querier: Reduce the default concurrency of queriers, `querier.max_concurrent`, to 8. #15984
* [CHANGE] Kafka: Mount the data claim at `/var/lib/kafka/data` with `subPath: data` rather than at `/var/lib/kafka`. The `apache/kafka` JVM image declares `/var/lib/kafka/data` as an image volume, and containerd satisfies an uncovered image volume with node-local storage, so the Kafka log was written below the claim and lost whenever the pod moved to another node. The default `apache/kafka-native` image declares no such volume and already kept its log in the claim's `data` directory, so the new mount resolves to the same data. #16658
