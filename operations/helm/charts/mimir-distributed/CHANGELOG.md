...
* [BUGFIX] Hash only the data of the ConfigMap or Secret in the `checksum/config` and `checksum/alertmanager-fallback-config` pod annotations, so that a chart version bump on its own no longer restarts every pod. #PRNUM
