...
* [BUGFIX] Ingester: document that `ingester.zoneAwareReplication.autoIngestStorageClientRack` only affects the read path when `-ingest-storage.kafka.fetch-concurrency-max` is set to 0. With the default fetch concurrency the ingesters read from the partition leader and the rack is ignored, so the flag on its own does not reduce cross-AZ traffic. No rendered output changes. #PLACEHOLDER
