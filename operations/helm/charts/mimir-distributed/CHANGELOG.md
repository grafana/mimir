...
* [FEATURE] Add an `ipv6` values preset for running the chart on an IPv6 only Kubernetes cluster. It sets `instance_enable_ipv6` on every ring, `memberlist.bind_addr` to `["::"]`, the HTTP and gRPC listen addresses to `::`, and the bundled Kafka listeners to `[::]`. No chart template changes are needed, so nothing changes for existing installations. #16606
