...
* [FEATURE] Add `global.ipFamily` to run the whole installation on IPv6 with a single value. Setting it to `IPv6` sets `instance_enable_ipv6` on every ring, `memberlist.bind_addr` to `["::"]`, the HTTP and gRPC listen addresses to `::`, makes the gateway listen on IPv6 only, and makes the bundled Kafka listen on `[::]`. The default `IPv4` keeps the current behaviour and `mimir.structuredConfig` still overrides the derived values. #16606
