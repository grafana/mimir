...
* [ENHANCEMENT] Add ability to manage PrometheusRule for metamonitoring with Prometheus operator from the Helm chart. The accompanying alerts are enabled by default but can be disabled with `prometheusRule.mimirAlerts` set to `false`. To disable the default rules, set `mimirRules` to `false`. #2134 #2609
