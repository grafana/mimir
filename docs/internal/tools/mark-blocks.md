...
`mimirtool blocks mark` is a command that assists in uploading or removing block markers to a specified backend.
...
When `--mark-type deletion` is provided, `mimirtool blocks mark` uploads a `DeletionMark` which tells the compactor that the block provided should be deleted.
This is a **destructive operation** (although it doesn't happen immediately, the default deletion delay is 12h, see `-compactor.deletion-delay` value), proceed with caution.
...
$ go run ./cmd/mimirtool blocks mark --mark-type deletion --tenant tenant-1 --details "Corrupted blocks" --blocks "01FSCTA0A4M1YQHZQ4B2VTGS2R,01FSCTA0A4M1YQHZQ4B2VTGS7Z"
...
When `--mark-type no-compact` is provided, this command uploads a `NoCompactMark` which tells the compactor that the marked blocks should not be compacted.
The `--details` flag can be used to provide an explanation of why the block is marked as such (who marked it? issue link?).
...
$ go run ./cmd/mimirtool blocks mark --mark-type no-compact --tenant tenant-1 --details "Blocks with out of order chunks" --blocks "01FSCTA0A4M1YQHZQ4B2VTGS2R,01FSCTA0A4M1YQHZQ4B2VTGS7Z"
