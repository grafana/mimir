...
- **Configurable TLS minimum version and cipher suites**
  We added the two new flags `-server.tls-min-version` and `-server.tls-cipher-suites`, they can be used to define the minimum TLS version and the supported cipher suites in all HTTP and gRPC servers in Mimir.
