...
We also recommend running a dedicated Memcached cluster for each type of cache since each is used and will scale
differently: query results, metadata, index, and chunks. This also ensures that each cache is isolated from the
others. This helps ensure that one type of cache entry isn't crowding out other entries resulting in degraded
performance.

The metadata cache is used to cache information about files in object storage, the contents of auxiliary files
such as bucket indexes, or discovered information about object storage such as lists of tenants. This results
in relatively low CPU and bandwidth usage.

The query results cache is used to cache responses for queries. Entries in this cache tend to be small and only
a few are fetched at a time. This results in relatively low CPU and bandwidth usage.

The index caches store portions of the TSDB index fetched from object storage. Entries in this cache vary in size
from a few hundred bytes to several megabytes. Entries are fetched both individually and in batches. A single query
may fetch many entries from the cache. This results in higher CPU usage compared to other caches.

The chunks caches store portions of timeseries samples fetched from object storage. Entries in this cache tend to be
large (several kilobytes) and are fetched in batches by the store-gateway components. This results in higher bandwidth
usage compared to other caches.
