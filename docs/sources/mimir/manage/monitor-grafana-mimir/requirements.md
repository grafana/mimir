...
For precompiled rules and alerts to function, configure your Prometheus or Grafana Alloy instance to scrape metrics from Grafana Mimir at an interval of `1m` or shorter (the mixin default `scrape_interval`). If you scrape less often, recompile the mixin with `_config.scrape_interval` set to match your scrape interval so `rateInterval()` windows stay large enough.
