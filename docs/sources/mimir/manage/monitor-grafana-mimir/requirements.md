...
For precompiled rules and alerts to function, configure your Prometheus (or Grafana Alloy) scrape interval to `1m` or shorter — `1m` is the Prometheus default and the mixin default `scrape_interval`. If you scrape less often, recompile the mixin with `_config.scrape_interval` set to match your scrape interval so `rateInterval()` windows stay large enough.
