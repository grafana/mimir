...
| `MIMIR_SIGV4_ASSUME_ROLE_ARN`       | `--sigv4-assume-role-arn`       | Sets the AWS role ARN to assume before SigV4 request signing.                                                                                                                          |
| `MIMIR_SIGV4_EXTERNAL_ID`           | `--sigv4-external-id`           | Sets the AWS external ID to use when assuming `MIMIR_SIGV4_ASSUME_ROLE_ARN`.                                                                                                           |
| `MIMIR_SIGV4_USE_FIPS_STS_ENDPOINT` | `--sigv4-use-fips-sts-endpoint` | If `true`, uses the FIPS AWS STS endpoint when assuming a role for SigV4 request signing.                                                                                              |
