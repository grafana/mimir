...
- parent_query_id — a unique ID allocated to the user query. This will be available in other query component logs processing the related sub-requests
...
- status, err - indicates success or failure with an error message indicating the failure reason
- parent_query_id - identifies the user query this evaluation belongs to, matching `parent_query_id` on the query-frontend's `query stats` line for that query.
