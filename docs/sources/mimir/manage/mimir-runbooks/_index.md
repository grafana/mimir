...
- System time of your Prometheus instance has been shifted backwards. If this was a mistake, fix the system time back to normal. Otherwise, wait until the system time catches up to the time it was changed. Using [node exporter](https://github.com/prometheus/node_exporter) metrics identify clock skew on any hosts using this promQL query: `abs(node_time_seconds - timestamp(node_time_seconds))`.
