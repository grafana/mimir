...
For those looking for a simple way to get started, Grafana Mimir can also be run in [monolithic mode]({{< relref "../deployment-modes/index.md#monolithic-mode" >}}), with all components running simultaneously in one process, or in [read-write mode]({{< relref "../deployment-modes/index.md#read-write-mode" >}}), which groups components into their read and write paths.
