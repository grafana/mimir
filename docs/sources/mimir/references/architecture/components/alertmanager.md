...
The Mimir Alertmanager is an optional component that accepts alert notifications from the [Mimir ruler](../ruler/).
...
To run Mimir Alertmanager as a part of [monolithic deployment](../../deployment-modes/#monolithic-mode), run Mimir with the option `-target=all,alertmanager`.
{{< /admonition >}}
...
| `queryFromGeneratorURL` | `generator_url`                               | Returns the URL decoded query from `GeneratorURL` of an alert set by a Prometheus, Mimir Ruler, or Loki Ruler. Example: `{{ queryFromGeneratorURL (index .Alerts 0).GeneratorURL }}`                              |
...
You must configure the [ruler](../ruler/) with the addresses of Alertmanagers via the `-ruler.alertmanager-url` flag.
...
1. Enable [`utf8-migration-logging-enabled`](../../../../configure/configuration-parameters/#alertmanager) and set [`log_level`](../../../../configure/configuration-parameters/#server) to `debug`. You must restart Mimir for the changes to take effect.
...
1. To enable UTF-8, set [`utf8-strict-mode-enabled`](../../../../configure/configuration-parameters/#alertmanager) to `true`. You must restart Mimir for the changes to take effect.
...
1. You can disable [`utf8-migration-logging-enabled`](../../../../configure/configuration-parameters/#alertmanager) and set [`log_level`](../../../../configure/configuration-parameters/#server) back to its previous value.
