...
- **case_sensitive** - _optional_ - whether `search[]` terms and `search_expr` leaves are case-sensitive. Defaults to `true`.
- **fuzz_alg** - _optional_ - the fuzzy-matching algorithm used to score `search[]` terms and positive `search_expr` leaves. One of `subsequence` (the default), `jarowinkler`, `substring_left` (plain substring match, scoring higher for matches further left in the value), or `substring` (plain substring match, always scoring 1.0). `substring_left` and `substring` apply no fuzzy matching and ignore `fuzz_threshold`.
- **fuzz_threshold** - _optional_ - the minimum match score a candidate must reach for a `search[]` term or positive `search_expr` leaf, as an integer between `0` and `100`. Defaults to `0`. A higher score indicates a better relevance match. `100` is an exact match. With `fuzz_alg=substring`, every match scores `1.0`, so `100` is the only meaningful value: it is the default, and any other explicit value returns an HTTP 400.
- **sort_by** - _optional_ - the result ordering. Either `alpha` (the default), which sorts alphabetically, or `score`, which sorts by descending relevance score. Using `score` requires at least one `search[]` term or a `search_expr`, and is rejected with `fuzz_alg=substring` since every match scores identically.
...
- **cursor** - _optional_ - an opaque token from a previous response's `next_cursor` trailer field, used to fetch the next page of results. Only supported for `sort_by=alpha`; a response for `sort_by=score` never emits `next_cursor`. When `cursor` is present, it is the sole source of every other search parameter — supplying `cursor` together with any other query parameter returns an HTTP 400. A cursor is a resume position, not a consistent snapshot: results added before the resume position between pages are never returned, and results deleted between pages are skipped. During a rolling upgrade, if a backend component does not yet support cursor-based resume, the affected page's trailer carries a `warnings` entry saying so, and that page's results can be incomplete until the upgrade completes.
