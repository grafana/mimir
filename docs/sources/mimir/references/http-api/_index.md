...
- **fuzz_alg** - _optional_ - the fuzzy-matching algorithm used to score `search[]` terms and positive `search_expr` leaves. Either `subsequence` (the default) or `jarowinkler`.
- **fuzz_threshold** - _optional_ - the minimum match score a candidate must reach for a `search[]` term or positive `search_expr` leaf, as an integer between `0` and `100`. Defaults to `0`. A higher score indicates a better relevance match. `100` is an exact match.
