...
- Query-frontend
  ...
  - Support for cluster validation when fetching runtime config from an HTTP endpoint via `-runtime-config.client.cluster-validation.label` (also inheritable from `-common.client-cluster-validation.label`).
