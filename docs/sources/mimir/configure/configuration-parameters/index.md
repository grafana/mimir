...
# range period (2h by default). A value of 0s means that all blocks are
# considered regardless of their upload time.
...
tsdb:
  ...
  # creation time of the block. This should be larger than the TSDB block range
  # period (2h by default) and -querier.query-store-after, and large enough to
  # give store-gateways and queriers enough time to discover newly uploaded
  # blocks.
