...
# mimirtool before enabling this mode as otherwise some tenant configurations
# might fail to load. Tenants with incompatible configurations can be identified
# in Mimir server logs searching for lines containing `Alertmanager is moving to
# a new parser for labels and matchers, and this input is incompatible.`. Tenant
# configurations that are valid but contain ambiguous matchers can be found
# searching for log lines containing `Matchers input has disagreement`. Each log
# line includes the invalid input, a suggestion on how to fix the input, and the
# ID of the affected tenant. Mimir must be run with debug level logging enabled.
