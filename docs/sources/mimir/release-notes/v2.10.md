...
- **Improved the performance of _label values with matchers_** calls when number of matched series is small. If you're using Grafana to query Grafana Mimir make sure you configure the right version in your Prometheus datasource configuration.
...
- **Reduce wasted effort spent computing results that won't be used** by making queriers cancel the requests sent to the ingesters in a zone upon receiving first error from that zone.
- **Added the ability to clean up the object storage by the compactor** when no blocks left in the bucket index by removing the bucket index, markers and debug files. This cleanup can be enabled by using `-compactor.no-blocks-file-cleanup-enabled` option.
...
- **Per-tenant results cache TTL**. (YAML keys `results_cache_ttl`, `results_cache_ttl_for_out_of_order_time_window`).
