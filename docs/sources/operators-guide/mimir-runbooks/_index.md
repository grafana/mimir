...
1. Find out which Mimir component operated on the block last (e.g. uploaded by ingester/compactor, or deleted by compactor)
   ...
   1. If there is a "compaction job finished" with "success=`false`" and later a "compaction job finished" with "success=`true`", it means that the compaction was retried successfully. Note: this produces different block IDs so you won't necessarily see the skipped block being uploaded again.
