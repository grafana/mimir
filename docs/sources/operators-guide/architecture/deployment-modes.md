...
![Mimir's monolithic mode](../../images/monolithic-mode.svg)
...
![Mimir's horizontally scaled monolithic mode](../../images/scaled-monolithic-mode.svg)
...
In microservices mode, each Grafana Mimir process is invoked with its `-target` parameter set to a specific Grafana Mimir component (for example, `-target=ingester` or `-target=distributor`). To get a working Grafana Mimir instance, you must deploy every required component. For more information about each of the Grafana Mimir components, refer to [Architecture]({{<relref "../architecture/_index.md" >}}).
...
![Mimir's microservices mode](../../images/microservices-mode.svg)
