...
1. The query-frontend places the query (or _queries_ if splitting or sharding of the initial query occured) in an in-memory queue, where it waits to be picked up by a querier.
