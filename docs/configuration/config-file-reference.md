...
bucket_store:
  ...
  # Probability in [0,1] interval of migrating an existing diff+varint cache key
  # to indexed+diff+varint encoding. 0 means that diff+varint cache entries will
  # be untouched. 1 or more means that any diff+varint encoded entry found will
  # be immediately stored in indexed+diff+varint encoding. Can be used as a know
  # to reduce load on MC during migration.
