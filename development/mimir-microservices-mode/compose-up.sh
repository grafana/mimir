...
CGO_ENABLED=0 GOOS=linux go build -mod=vendor -tags=netgo,stringlabels,localvalidationscheme -gcflags "all=-N -l" -o "${SCRIPT_DIR}"/mimir "${SCRIPT_DIR}"/../../cmd/mimir
...
if [ "$(yq '.services.query-tee' "${SCRIPT_DIR}"/docker-compose.yml)" != "null" ]; then
  ...
  CGO_ENABLED=0 GOOS=linux go build -mod=vendor -tags=netgo,stringlabels,localvalidationscheme -gcflags "all=-N -l" -o "${SCRIPT_DIR}"/../../cmd/query-tee "${SCRIPT_DIR}"/../../cmd/query-tee
