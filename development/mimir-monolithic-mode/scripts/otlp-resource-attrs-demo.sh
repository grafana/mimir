...
#
# Cloud mode (skips docker-compose, flush, and WAL replay phases):
...
#   MIMIR_URL         Mimir base URL (default: http://localhost:8101)
...
if [ "$CLOUD_MODE" = true ]; then
    print_phase 3 "WAL replay verification (SKIPPED)"
    echo -e "${YELLOW}Skipped in cloud mode: requires docker-compose restart${RESET}"
else
