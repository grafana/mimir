...
- **D-08:** `SingleUTCDayVerifier` computes the UTC-day bound from `meta.MinTime` and `meta.MaxTime`: pass iff `meta.MinTime / 86_400_000 == (meta.MaxTime - 1) / 86_400_000`. In deep mode, also inspects the min/max sample timestamps derived from the block's index (via `pkg/storage/tsdb/block.GatherBlockHealthStats` or similar primitive — planner to pick specific call) and confirms those also fall within the same UTC day.
