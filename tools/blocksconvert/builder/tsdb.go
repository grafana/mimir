...
// Provenance-includes-location: https://github.com/cortexproject/cortex/blob/master/tools/blocksconvert/builder/tsdb.go
...
func addSeriesToIndex(indexWriter *index.Writer, sl *seriesList, unsortedChunksReader *chunks.Reader, chunksWriter *chunks.Writer) (tsdb.BlockStats, error) {
	...
	for s, ok := it.Next(); ok; s, ok = it.Next() {
		...
		if err := indexWriter.AddSeries(storage.SeriesRef(seriesRef), l, cs...); err != nil {
