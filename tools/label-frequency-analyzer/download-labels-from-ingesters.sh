...
OUTPUT_DIR="label-names-and-values-dump"
...
query_ingester() {
  ...
  grpcurl \
    ...
    -H "X-Cluster: $K8S_NAMESPACE" \
    -proto pkg/ingester/client/ingester.proto \
    -import-path "$SCRIPT_DIR/../.." \
    -import-path "$SCRIPT_DIR/../../vendor" \
    -plaintext \
    localhost:${LOCAL_PORT} "cortex.Ingester/LabelNamesAndValues" | jq -r '.items[].labelName' > "$OUTPUT_DIR/$POD"
  STATUS_CODE=$?
...
# Get list of pods.
PODS=$(kubectl --context "$K8S_CONTEXT" -n "$K8S_NAMESPACE" get pods --no-headers | grep ingester-zone-a | awk '{print $1}')
