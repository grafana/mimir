...
./run.sh --url "${URL}" --tenant-id "${TENANT_ID}" --tsv-no-header \
  --query "sum by(job) (rate(container_cpu_user_seconds_total[1m]))" --time-ranges "10d" --shards "16"

./run.sh --url "${URL}" --tenant-id "${TENANT_ID}" --tsv-no-header \
  --query "histogram_quantile(0.99, sum by(le) (rate(apiserver_request_duration_seconds_bucket[1m])))" --time-ranges "6H 24H 7d" --shards "16"

./run.sh --url "${URL}" --tenant-id "${TENANT_ID}" --tsv-no-header \
  --query "histogram_quantile(0.99, sum by(verb, resource, le) (rate(apiserver_request_duration_seconds_bucket[1m])))" --time-ranges "6H 24H 7d" --shards "16"
