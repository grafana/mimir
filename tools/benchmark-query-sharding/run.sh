...
for TIME_RANGE in $TEST_TIME_RANGES; do
  ...
  for SHARD_SIZE in $TEST_SHARDS; do
    benchmark_query_with_multiple_runs "$TEST_QUERY" "$TIME_RANGE" "yes" "${SHARD_SIZE}"
  done

  # Sharding disabled.
  benchmark_query_with_multiple_runs "$TEST_QUERY" "$TIME_RANGE" "no" ""
done
