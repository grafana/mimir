...
func generateFirstTokens(zoneId, zonesCount, tokensPerInstanceCount int) ring.Tokens {
	tokens := make([]uint32, 0, tokensPerInstanceCount)
	for t := 0; t < tokensPerInstanceCount; t++ {
		token := uint32(math.Pow(2, 32)*(1.0-(3.0*float64(t)+float64(zoneId)))/float64(zonesCount*tokensPerInstanceCount)) - 1
