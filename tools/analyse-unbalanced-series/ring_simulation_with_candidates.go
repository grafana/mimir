...
func generateRingWithoutZoneAwareness(logger log.Logger, numTokensPerInstanceScenarios []int, replicationFactor, instancesPerZone int, zones []tokdistr.Zone, seedGeneratorProvider func(zones []tokdistr.Zone, replicationFactor, tokensPerInstance int, maxTokenValue tokdistr.Token) tokdistr.SeedGenerator) error {
	result := make([][]float64, 0, iterations)
	...
	for _, numTokensPerInstance := range numTokensPerInstanceScenarios {
		for it := 0; it < iterations; it++ {
			level.Info(logger).Log("tokensPerInstance", numTokensPerInstance, "iteration", it)
			replicationStrategy := tokdistr.New(replicationFactor, make(map[tokdistr.Instance]tokdistr.Zone, initialInstanceCount), nil, nil)
			tokenDistributor := tokdistr.NewTokenDistributor(numTokensPerInstance, len(zones), maxTokenValue, replicationStrategy, seedGeneratorProvider(zones, replicationFactor, numTokensPerInstance, maxTokenValue))
			var stat *tokdistr.Statistics
			for i := 0; i < instancesPerZone; i++ {
				for j := 0; j < len(zones); j++ {
...
func main() {
	...
	generateRingWithZoneAwareness(logger, numTokensPerInstanceScenarios, 1, 30, []tokdistr.Zone{tokdistr.SingleZone}, seedGenerator)
	// generate ring with different tokens per instance with replication and zone-awareness enabled
	generateRingWithZoneAwareness(logger, numTokensPerInstanceScenarios, replicationFactor, 30, []tokdistr.Zone{tokdistr.SingleZone}, seedGenerator)
	// generate ring with different tokens per instance with replication and zone-awareness enabled
