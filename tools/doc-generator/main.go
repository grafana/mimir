...
// Type used to help us control which entries are written in each block
type filterFunc func(entry *parse.ConfigEntry) bool
...
func generateBlocksMarkdown(blocks []*parse.ConfigBlock) string {
	md := &markdownWriter{}
	md.writeConfigDoc(blocks, nil)
	return md.string()
}

func blockContainsFieldCategory(block *parse.ConfigBlock, fieldCategoryFilter *filterFunc) bool {
	if block == nil {
		return false
	}

	// Find blocks that have fields with the given category
	for _, entry := range block.Entries {
		if (*fieldCategoryFilter)(entry) {
			return true
		}
	}
...
func generateBlockMarkdownFromFieldCategory(blocks []*parse.ConfigBlock, fieldCategory string) string {
	// Define a filtering function that filters based on a field category
	// This will recursively search nested blocks for entries that belong to the field category
	var fieldCategoryFilter filterFunc
	fieldCategoryFilter = func(entry *parse.ConfigEntry) bool {
		if entry.FieldCategory == fieldCategory {
			return true
		}
		if entry.Block != nil {
			for _, nestedEntry := range entry.Block.Entries {
				if fieldCategoryFilter(nestedEntry) {
					return true
				}
			}
		}
	...
	var blocksWithFieldCategory []*parse.ConfigBlock

	for _, block := range blocks {
		if blockContainsFieldCategory(block, &fieldCategoryFilter) {
			blocksWithFieldCategory = append(blocksWithFieldCategory, block)
		}
	}

	md := &markdownWriter{}
	md.writeConfigDoc(blocksWithFieldCategory, &fieldCategoryFilter)
	return md.string()
}
...
func main() {
	...
	data := struct {
		...
		ExperimentalFieldsBlock  string
		DeprecatedFieldsBlock    string
	}{
		...
		ExperimentalFieldsBlock:  generateBlockMarkdownFromFieldCategory(blocks, "experimental"),
		DeprecatedFieldsBlock:    generateBlockMarkdownFromFieldCategory(blocks, "deprecated"),
