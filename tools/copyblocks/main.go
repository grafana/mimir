...
func loadMetaJSONFile(ctx context.Context, bkt bucket, tenantID string, blockID ulid.ULID) (metadata.Meta, error) {
	objectName := tenantID + delim + blockID.String() + delim + block.MetaFilename
	r, err := bkt.Get(ctx, objectName)
	if err != nil {
		return metadata.Meta{}, errors.Wrapf(err, "failed to read %v", objectName)
	}
	...
	if err != nil {
		return metadata.Meta{}, errors.Wrapf(err, "read %v", objectName)
	}
	if closeErr != nil {
		return metadata.Meta{}, errors.Wrapf(err, "close reader for %v", objectName)
	}
...
func listBlockMarkersForTenant(ctx context.Context, bkt bucket, tenantID string, destinationBucket string) (map[ulid.ULID]blockMarkers, error) {
	markers, err := bkt.ListPrefix(ctx, tenantID+delim+bucketindex.MarkersPathname, false)
	if err != nil {
		return nil, err
	}
