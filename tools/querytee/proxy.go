...
func (cfg *ProxyConfig) RegisterFlags(f *flag.FlagSet) {
	...
	f.BoolVar(&cfg.SkipPreferredBackendFailures, "proxy.compare-skip-preferred-backend-failures", false, "If true, the proxy will set the 'skip' result on comparisons where the preferred backend returned a 5xx status code. This is useful in the case where the secondary backend is more reliable.")
...
func (p *Proxy) Start() error {
	...
	for _, route := range p.routes {
		...
		router.Path(route.Path).Methods(route.Methods...).Handler(
			NewProxyEndpoint(p.backends, route, p.metrics, p.logger, comparator, p.cfg.LogSlowQueryResponseThreshold, p.cfg.SecondaryBackendsRequestProportion).
				WithSkipPreferredBackendFailures(p.cfg.SkipPreferredBackendFailures),
		)
	}
