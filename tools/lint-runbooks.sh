...
for ALERT in $ALERTS; do
  if ! grep -q "# ${ALERT}$" "${SCRIPT_DIR}/../docs/sources/operators-guide/runbooks/_index.md"; then
...
for ID in $ERROR_IDS; do
  ...
  if ! grep -q "# ${ID}$" "${SCRIPT_DIR}/../docs/sources/operators-guide/runbooks/_index.md"; then
