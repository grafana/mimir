...
// BenchmarkAmplifyWriteRequest benchmarks the amplification of RW1 and RW2 requests
// with various series counts and amplification factors.
func BenchmarkAmplifyWriteRequest(b *testing.B) {
	seriesCounts := []int{1000}
	ampFactors := []float64{2.0, 5.0}
	...
	for _, numSeries := range seriesCounts {
		...
		for _, factor := range ampFactors {
			b.Run(fmt.Sprintf("RW1/series=%d/factor=%.1fx", numSeries, factor), func(b *testing.B) {
				for i := 0; i < b.N; i++ {
					result, err := AmplifyWriteRequest(rw1Compressed, factor, nil)
					if err != nil {
						b.Fatal(err)
					}
					_ = result
			...
			b.Run(fmt.Sprintf("RW2/series=%d/factor=%.1fx", numSeries, factor), func(b *testing.B) {
				for i := 0; i < b.N; i++ {
					result, err := AmplifyWriteRequest(rw2Compressed, factor, nil)
					if err != nil {
						b.Fatal(err)
					}
					_ = result
