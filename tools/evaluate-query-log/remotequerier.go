...
const (
	...
	queryRangeEndpointPath = "/api/v1/query_range"
...
func (q *RemoteQuerier) QueryRange(ctx context.Context, query string, start time.Time, end time.Time, step time.Duration, logger log.Logger) ([]byte, error) {
	args := make(url.Values)
	args.Set("query", query)
	args.Set("start", start.Format(time.RFC3339Nano))
	args.Set("end", end.Format(time.RFC3339Nano))
	args.Set("step", step.String())
	body := []byte(args.Encode())

	req := httpgrpc.HTTPRequest{
		Method: http.MethodPost,
		Url:    q.promHTTPPrefix + queryRangeEndpointPath,
		Body:   body,
		Headers: []*httpgrpc.Header{
			{Key: textproto.CanonicalMIMEHeaderKey("User-Agent"), Values: []string{userAgent}},
			{Key: textproto.CanonicalMIMEHeaderKey("Content-Type"), Values: []string{mimeTypeFormPost}},
	...
	resp, err := q.sendRequest(ctx, &req)
	if err != nil {
		level.Warn(logger).Log("msg", "failed to remotely evaluate query expression", "err", err, "qs", query)
		return nil, err
	}
	if resp.Code/100 != 2 {
		return nil, httpgrpc.Errorf(int(resp.Code), "unexpected response status code %d: %s", resp.Code, string(resp.Body))
	}
	level.Debug(logger).Log("msg", "query expression successfully evaluated", "qs", query)
