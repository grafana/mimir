...
func TestOOOIngestion(t *testing.T) {
	...
	series, _, expectedMatrix := generateFloatSeries(floatSeriesName, nowTS)
	res, err := c.Push(series)
	require.NoError(t, err)
	require.Equal(t, http.StatusOK, res.StatusCode)

	// Push out-of-order sample.
	series, expectedVector, _ = generateFloatSeries(floatSeriesName, oooTS)
	res, err = c.Push(series)
	require.NoError(t, err)
	require.Equal(t, http.StatusOK, res.StatusCode)
	...
	// Query float series.
	rangeResult, err := c.QueryRange(floatSeriesName, nowTS.Add(-time.Minute), nowTS.Add(time.Minute), time.Minute)
	require.NoError(t, err)
	...
	// Push in-order sample.
	series, _, expectedMatrix = generateHistogramSeries(intHistogramSeriesName, nowTS)
	res, err = c.Push(series)
	require.NoError(t, err)
	require.Equal(t, http.StatusOK, res.StatusCode)

	// Push out-of-order sample.
	series, expectedVector, _ = generateHistogramSeries(intHistogramSeriesName, oooTS)
	res, err = c.Push(series)
	require.NoError(t, err)
	require.Equal(t, http.StatusOK, res.StatusCode)
	...
	rangeResult, err = c.QueryRange(intHistogramSeriesName, nowTS.Add(-time.Minute), nowTS.Add(time.Minute), time.Minute)
	require.NoError(t, err)
	require.Equal(t, model.ValMatrix, rangeResult.Type())
	require.Equal(t, expectedMatrix, rangeResult.(model.Matrix))
	...
	// Push in-order sample.
	series, _, expectedMatrix = generateHistogramSeries(floatHistogramSeriesName, nowTS)
	res, err = c.Push(series)
	require.NoError(t, err)
	require.Equal(t, http.StatusOK, res.StatusCode)

	// Push out-of-order sample.
	series, expectedVector, _ = generateHistogramSeries(floatHistogramSeriesName, oooTS)
	res, err = c.Push(series)
	require.NoError(t, err)
	require.Equal(t, http.StatusOK, res.StatusCode)
	...
	rangeResult, err = c.QueryRange(floatHistogramSeriesName, nowTS.Add(-time.Minute), nowTS.Add(time.Minute), time.Minute)
	require.NoError(t, err)
	require.Equal(t, model.ValMatrix, rangeResult.Type())
	require.Equal(t, expectedMatrix, rangeResult.(model.Matrix))
}
