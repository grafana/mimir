...
func runQueryFrontendTest(t *testing.T, cfg queryFrontendTestConfig) {
	...
	for u := 0; u < numUsers; u++ {
		...
		if userID == 0 && cfg.queryStatsEnabled {
			...
			require.NoError(t, err)
			require.Regexp(t, "querier_wall_time;dur=[0-9.]*, response_time;dur=[0-9.]*, bytes_processed;val=[0-9.]*, samples_processed;val=[0-9.]*, equivalent_samples_read;val=[0-9.]*$", res.Header.Values("Server-Timing")[0])
		}
